...
const MANIFEST = {
    ...
    // GLEXPORT-1:报表包与总账导出那一族的拒绝。后缀集合【现读】
    // packErrorCodes.ts 的那个 Set —— 加一条拒绝、忘了写文案,构建当场红。
    'pack.errors.':         { kind: 'enum', values: () => tsSet('app/finance/packErrorCodes.ts', 'PACK_ERROR_CODES') },
    ...
    // AP-RECON-1 Batch B:残留的类别接表上的 CHECK —— 迁移给残留表加一类,两个语言会同时被要求补标签。
    'finance.listVsLedger.residueClass.': { kind: 'enum', values: () => sqlEnum('db/tables/list_ledger_residue.sql', 'residue_class') },
