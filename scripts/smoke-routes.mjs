...
const EXPECTED_SKIPS = new Set([
    ...
    // SO-3b:线上还没有一张发货单(机制与屏幕先于第一次真发货落地 —— 第一次
    //【真的】发货是 Tim 的决定,不是一次走查)。发出第一张的那天,这条断言会
    // 逼人把它从这里删掉。它这一次就【当场响了】:新路由一加进来就变成 skip,
    // 而"新路由悄悄没被跑过"与"真的没数据"在屏幕上一模一样。
    '/sales/shipments/[id]/pdf',
