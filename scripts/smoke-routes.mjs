...
const EXPECTED_UNREACHABLE = {
    ...
    // ════════════════════════════════════════════════════════════════════
    // ★★【UI-FIX-1 ⑦(2026-09-02):这一条【留着】,但它的【理由】要换掉】★★
    // ════════════════════════════════════════════════════════════════════
    ...
    // 旧注释说的是"入口挂在 pricing 模块里,operations 没有那个模块"。
    // ⑦ 之后那句话【已经不成立】—— 入口现在挂在工具底下,operations 有工具。
    ...
    // **对【人】来说,⑦ 确实给了 operations 一个入口** —— 点顶栏「工具」,菜单里
    // 就有「金属行情」,可点。**对【这个爬虫】来说没有**,因为那张菜单不在 HTML 里。
    // 这条断言度量的是后者。**它没有说谎,但它的"走不到"比人的"走不到"宽** ——
    // 这个差别是 IA-BUILD-1 把二级搬进点开式菜单之后【整体】出现的,不是这一刀造成的,
    // 也不该靠删断言来抹掉。**要缩小这个差别,得让二级出现在服务端 HTML 里
