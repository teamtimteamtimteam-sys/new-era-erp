...
// ==========================================================================
// 【瞄准 · AIM】
//   我读的是      :命令行传进来的**两棵构建产物树**里的 CSS(postcss 真解析)。
//   我声称管的是   :「187 个已上线页面一个像素都没变」是量出来的。
//   两者不同之处   :★★ **没有任何东西在跑我。** 我要两个 `<before-dir> <after-dir>`,
//                   而 `npm run build` 只产出其中一棵、另一棵没有任何机制保留 ——
//                   所以我**接不进构建链**(NARROW-COVERAGE-1 实测:不带参数 exit 2)。
//                   我不是一支被忘掉的检查,我是一支**差分工具**:BRAND-1 用过一次,
//                   之后再没有人调用过。`package.json` 里对我零引用。
//                   ☞ 一支存在却从不运行的证明工具,它的覆盖率是 **0** ——
//                     这是本族里最彻底的一种「名字对,覆盖窄」。已交回给 Tim 裁定。
