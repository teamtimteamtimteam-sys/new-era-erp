...
if (added.length === 0) {
    console.log('✓ 没有【新增】的直连遮蔽表读取。')
    process.exit(0)
}

console.log('✗ 新增了直连遮蔽表的读取:')
for (const a of added) {
    const [file, tbl] = a.k.split(' :: ')
    console.log(`     ${file}`)
    console.log(`       读的是 ${tbl},应当读 ${MASKED.get(tbl)}   (在册 ${a.was} 处,现在 ${a.now} 处)`)
    for (const h of hits.filter((x) => x.file === file && x.table === tbl)) {
        console.log(`       第 ${h.line} 行: ${h.text.slice(0, 90)}`)
    }
}
console.log('')
console.log('【怎么改】把 .from(\'<表>\') 换成 .from(\'<表>_masked\')。')
