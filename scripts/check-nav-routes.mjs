...
const EXCEPTIONS = new Map([
    ...
    // ★ COD-2(2026-09-08):两条【不是给本系统用户走的】路由 ★
    ['/verify/cod/[token]',
        '销毁证书的核验页。**本系统第一条不需要会话的业务路由**(PUBLIC_PATHS 里加的那一条),' +
        '地址由 COD-1 烧进了每一张证书的二维码。它的访客是【送料方】,不是这套系统的用户 —— ' +
        '所以它不在导航里,而且它连应用外壳都不画(route 直接回一页自包含 HTML)。'],
    // ★【SEARCH-1 · S6:判据 ② 收紧之后,这一条露了出来】★
    //   `/my-reviews` 本来就在这张表里(「我自己的东西,不属于九个模块」),
    //   于是它底下那一页【没有任何注册表条目覆盖得到】。此前动态段是一句
    //   无条件 continue,所以没有人问过;现在动态段也要被覆盖,它因此要一条理由。
    //   理由与它的父一字不差:本人的一份绩效评估,它的读者是它的主语本人。
    ['/my-reviews/[id]',
        '本人的一份绩效评估。父路由 /my-reviews 就在这张表里,理由相同 —— ' +
        '它是「我自己的东西」,不属于九个模块中的任何一个,入口是头像菜单。'],
    ['/me/avatar',
        '头像字节的代理路由(UI-1d 写下的替代方案 (b),COD-2 落地)。它不是一个页面,' +
        '是 <img src> 指向的那个地址:桶转私有之后,头像的字节从这里出去,' +
        '而【地址里不再出现任何 uid】—— 服务谁由会话说了算。'],
    // ★★【SEARCH-5(2026-09-19):关联记录那两条地址】★★
    //   它们的入口**只有搜索面板** —— 不是菜单去处,也不该是。给它们一条
    //   FUNCTIONS 条目会让它们出现在「找页面与动作」那一节里,而一条
    //   `/documents/[key]` 的条目在那一节里读起来是一个**没有主语的动词**。
    //   ⚠ 两条各写一行(不是一条盖住两条):它们是**两种形状**,
    //     而 S6 收掉的正是"一条例外覆盖无穷多种组合"那张通行证。
    ['/related/[subject]/[id]/[target]',
        '一条单据与另一种单据之间的关联记录(「NMC Cathode Foil 的产出批」)。' +
        '入口只有搜索面板里那一行分组行 —— 它不是一个菜单去处:它要先有一个主语' +
        '才有意义,而菜单里没有主语。三段全是动态段,所以它自己要一条例外。'],
    ['/documents/[key]',
        '一种单据自己的列表(「化验单」)。SEARCH-5 之前,5 种单据的命中链接指向' +
        '一张结构上永远 0 行的列表(?q= 过滤的是别的表的单据号,前缀不重叠);' +
        '它们现在落在这里。入口只有搜索面板的命中行 —— 它【不该】进菜单:' +
        '这 5 种单据的行在它们自己的上下文里(某一批的化验单、某个客户的催收)' +
        '才读得懂,一份全表列表是搜索的落点,不是一条日常去处。'],
...
//
// ════════════════════════════════════════════════════════════════════════════
// ★★【SEARCH-1 · S6:判据 ② 收紧了 —— 前缀覆盖不再是一张通行证】★★
// ════════════════════════════════════════════════════════════════════════════
//
// 【此前它是什么】「要么在注册表里、**要么在某条条目之下**、要么在例外表里」。
//   ★ 中间那一条 = **前缀覆盖 = 一个新长出来的埋着的屏幕【不会让这道闸变红】。**
//   SEARCH-0 §Q2 实测了这条口子的大小:`app/` 下 232 条路由,注册表 83 条,
//   **67 条只靠前缀覆盖**,其中 49 条是动作族,★ **18 条是【同级的屏幕】** ——
//   它们各自是一整页功能,而注册表里没有它们的名字。Tim 举的那个例子
//   (`/inbound/receive`,「field receiving」)就是其中第 10 条。
//
// 【为什么补完那 18 条【不够】】SEARCH-0 的原话:「不收紧,这 18 条补完之后,
//   第 19 条照样会安静地长出来。」☞ **一道闸如果只在那件事已经存在之后才是绿的,
//   而看不见第二十个,它一文不值。**
//
// ── 新规则(Tim 的 S6,逐字)────────────────────────────────────────────────
//   一条**静态**路由,只要它不是**动作族**(`/new` `/export` `/pdf` `/bulk` `/import`),
//   **就必须有自己的注册表条目**。
//   · **动态段 `[id]`** —— 保留前缀覆盖。一个 id 不是架构里的一层,它是同一层上
//     的一行(与 `gen-deep-routes.mjs` 的判据逐字同源)。
//   · **动作族** —— 保留前缀覆盖。它们是【对当前这一层的动作】,不是更深的一层
//     (同样与 `gen-deep-routes.mjs` 的 `LEAF_VERBS` 同一条道理,只是这里的名单更长:
//     那支管面包屑,只排除 new/edit;这一支管"要不要有名字",所以 export/pdf/bulk/import
//     一并排除 —— 实测树上动作族叶子词恰好就是这五个)。
//   · **例外表** —— 一个字没动,继续管着登录页、首页、本人档案那一族。
//
// ★【而这两族【仍然要被覆盖】,不再是不查】★ 此前动态段是一句无条件 `continue`
//   (那一行的注释说「由下一行判」,而它其实什么都没判)。现在它们也要落在某条条目
//   之下,否则同样红 —— 实测收紧时抓到一条真的:`/my-reviews/[id]`,
//   它的父 `/my-reviews` 在例外表里,所以没有任何条目覆盖得到它。
//   它现在自己也在例外表里,带着理由。
const entrySet = new Set(entryHrefs)
const isCoveredByEntry = (r) => entryHrefs.some((h) => r === h || r.startsWith(h + '/'))
/** 动作族的叶子词。**它们是对当前这一层的动作,不是更深的一层。** */
const ACTION_LEAVES = new Set(['new', 'export', 'pdf', 'bulk', 'import'])
const lastSeg = (r) => r.split('/').filter(Boolean).pop() ?? ''
const isActionFamily = (r) => ACTION_LEAVES.has(lastSeg(r))
let buriedFound = 0
for (const r of routes) {
    if (entrySet.has(r)) continue
    if (EXCEPTIONS.has(r)) continue
    const covered = isCoveredByEntry(r)
    // 动态段与动作族:**前缀覆盖仍然算数**,但覆盖不到就红(此前它们根本不查)。
    if (r.includes('[') || isActionFamily(r)) {
        if (!covered) {
            problems.push({
                arm: '② 路由要么在注册表里,要么在例外表里',
                msg: `${r} 是${r.includes('[') ? '一条动态路由' : '一条动作路由'},而它【不在任何一条注册表条目之下】—— 它是一页谁都走不到的页面。给它的父级一条 FUNCTIONS 条目,或者在 EXCEPTIONS 里写下它为什么不需要。`,
            })
        }
        continue
    }
    // ★ 这里是 S6 收紧的那一格:一条静态、非动作族的路由,【前缀覆盖救不了它】。
    buriedFound += 1
    problems.push({
        arm: '② 路由要么在注册表里,要么在例外表里',
        msg: covered
            ? `★ ${r} 是一个【埋着的屏幕】:它是一整页功能,而注册表里没有它的名字 —— ` +
              `它今天只靠父条目的前缀被"覆盖"着,于是搜索找不到它、菜单点不到它,而在这道闸收紧之前【没有任何东西会说出来】。\n` +
              `      修法:给它一条 FUNCTIONS 条目(navKey + permission;它页面守卫用的那个码就是它的 permission)。` +
              `它如果不该出现在菜单里,就带上 parent: '<父条目的 href>' —— 那条条目仍然被搜索看得见,而菜单一个字不变。`
            : `${r} 既不是注册表条目、也不在某条条目之下、也没有登记成例外 —— 它是一页【谁都走不到】的页面。要么给它一条 FUNCTIONS 条目,要么在 EXCEPTIONS 里写下它为什么不需要。`,
    })
}
...
console.log(
    ...
    // ★ 判据 ② 收紧之后,这个数【应当恒为 0】—— 把它印出来,是因为一个
    //   「0 个埋着的屏幕」只有在它旁边写着分母时才读得出它量过没有。
    `埋着的屏幕 ${buriedFound} 个(静态 ${routes.filter((r) => !r.includes('[')).length} 条路由里)`,
)
