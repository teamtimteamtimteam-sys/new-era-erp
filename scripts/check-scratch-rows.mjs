...
// ==========================================================================
// 【瞄准 · AIM】
//   我读的是      :线上库:一次性账号、授权、以及按命名规则认出来的 ZZ-*/smoke-* 业务行,
//                   加上一个**两小时**的年龄门槛与「还有谁在引用它」那一列。
//   我声称管的是   :**报告**滞留的临时行 —— 它刻意【不清扫】。
//   两者不同之处   :★ **我陈述一个判断,但不依据它动手。** 归属与年龄正是一次清扫
//                   无法安全知道、而报告可以照直说出来的东西:`sweepScratch` 就是
//                   这么在 2026-08-10 删掉了另一个会话正在用的账号。
//                   ★ 而我按【命名规则】认残骸 —— 一个不守命名规则的残骸我看不见,
//                     一个名字像残骸的**真行**我会误报(实测:`ZZ-SMOKE-PROBE` 物料
