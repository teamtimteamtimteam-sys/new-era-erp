...
//                   scrollWidth / getBoundingClientRect / getComputedStyle。
//   我声称管的是   :390px 上今天有多少页可用(U1 不横向平移 · U2 账簿不被裁掉)。
//   两者不同之处   :**我走的是硬导航,而人走的是软导航。** AGENTS.md 那条具名法则
//                   (「判据必须走人真的走的那条路」第五例)量到过:根布局在客户端
//                   换页时**不重画**,于是脚本每次拿到的是一棵人从来见不到的树。
//                   ☞ 所以我的数对「一次全新打开这一页」成立,
//                     对「一个人点着链接走到这一页」**未经验证**。
