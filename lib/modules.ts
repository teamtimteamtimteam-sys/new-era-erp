...
// ════════════════════════════════════════════════════════════════════════════
// ★★【UI-1a ③:顶栏画哪几个 —— 这【不是】 MODULES 的子集那么简单,值得整段说清】★★
// ════════════════════════════════════════════════════════════════════════════
//
// 【委托书原文是「SEVEN modules, not nine」,而照字面做会弄坏三件事】
// 直觉的做法是把 'tools' 与 'settings' 从上面的 MODULES 里删掉。**实测不行** ——
// MODULES 不只是"顶栏的标签表",它是【一条路由归谁所有】这件事的真源:
//
//   ① 面包屑会死在 8 条深路由上。lib/navTrail.ts:breadcrumbTrail 里写着
//      `MODULES.find(m => m.id === active.id)`,找不到就 `return []`。
//      /tools/pricing/formulas、/tools/pricing/metal-prices/[id]/edit 等 8 条
//      **整条面包屑消失**,而屏幕上没有任何东西说为什么。
//   ② 一场 console.error 暴雨。ModuleBar 里那条"属主一个都进不去就报矛盾"的
//      守卫会在【每一个】 /tools/* 与 /settings/* 页面上对【每一个人】喊
//      「注册表与守卫不一致」—— 而它们其实是一致的,是我们把真源改窄了。
//      **一条永远在喊的警报,等于没有警报。**
//   ③ ★ 手机上,工具与设置会【连门都没有】★ —— 而这一条最严重。
//      手机抽屉渲染的是同一个 modules 数组(ModuleBar 的 sm:hidden 分支),
//      而替代的两扇门(工具下拉、头像下拉)只长在桌面那一行上。删两条 = 手机上
//      **五个小工具与七张设置页整批够不着**,一处缺席,没有任何东西说出来。
//      ModuleBar 自己的抬头早就写下了禁止这件事的那句话:
//      **「顶栏挪走什么,抽屉就得接住什么。」**
//
// 【所以:退出的是【那一行】,不是【注册表】】(Tim 裁定,2026-09-05,采纳了这条更正)
//   · MODULES 保持九条 —— 属主、面包屑、活动模块判定一个字不动;
//   · 桌面顶栏那一行画下面这七条;
//   · 手机抽屉画【同样这七条】,再加两个【具名的区】(工具 / 设置)。
//
// ★★【UI-1c(2026-09-05):那条"手机抽屉画九条"的时限【兑现了】,不再顺延】★★
//   UI-1a 写的是「手机抽屉继续画九条,直到 UI-1b 重做手机顶栏」;UI-1b 把它
//   顺延到 UI-1c;**UI-1c 做了那件事。** 抽屉现在遍历 BAR_MODULE_IDS 这七条,
//   工具与设置各自是一个【标题 + 平铺条目】的区(ModuleBar 的 sheet 分支)。
//   【为什么值得改】九条并排时,工具与设置读起来像第八、第九个业务模块 ——
//   而它们不是:一个装小工具,一个装系统配置。变的【只有渲染】:
//   MODULES 仍然九条,上面 ①②③ 那三条后果一条都没有重新引发。
//
// ★【手机与桌面的形态差别还在,而它【不再是欠账】】★
//   桌面:工具走 ToolsMenu 那张下拉,设置走头像下拉里的七行子菜单;
//   手机:两者都是抽屉里的一个区。
//   **两侧都从同一个 modules 数组派生、同一份 allowed、同一套措辞与记号**
//   (common.restricted + dashboard.restrictedHint + data-module-restricted),
//   所以这是【两种画法】,不是【两份清单】。完整论证见
//   docs/information-architecture.md §23.3 与 §23.7。
export const BAR_MODULE_IDS: readonly string[] = [
    'purchasing', 'logistics', 'operation', 'sales', 'finance', 'inventory', 'hr',
]

/** 退出顶栏那一行、但【仍然在注册表里】的模块。工具进工具下拉,设置进头像下拉。 */
export const TOOLS_MODULE_ID = 'tools'
...
export const FUNCTIONS: readonly FunctionEntry[] = [
    ...
    //
    // 【它是什么】首页那一整块「当前待办」搬了过来:34 支跨模块的等待信号 +
    //   HR 提醒。为什么是【搬】而不是【拆回各模块】—— 信号天生跨模块,拆回去就
    //   丢掉了"一眼看完所有该操心的事"这件事本身;给它一个自己的去处,人就是
    //   想看的时候去看,而不是一登录就被推一脸。
    //
    // ★【判据【刻意】恒真,与 /tools/calendar 逐字同理 —— 不要"顺手"关上】★
    //   这一页没有自己的权限模型:**每一支按它【自己家】那个模块的可见性
    //   出现或不出现**(视图末尾那个 WHERE + 页面的 allows(),两道同码)。
    //   给这一条挂一个模块码,就等于替 34 支【各不相同】的判据挑一个代表 ——
    //   挑谁都错:挑 finance,一个只做仓库的人就看不见他自己的盘点提醒;
    //   挑得宽,又会让菜单上出现一条点进去空空如也的条目。
    //   **本条不挡人,挡人的是每一支。** 一个进得来却一支都看不见的读者,
    //   在页面上拿到一句【具名的话】(reminders.nothingVisible),不是一张白页。
    //
    // 【顺序:任务 · 日历 · 单位换算 · 提醒 · 定价】—— 提醒排在定价【之前】,
    //   因为定价是这份菜单里唯一一条【自己还带着孩子】的条目(它那一页列出
    //   公式/计价器/金属行情),把一条平的条目插到它后面会读成它的第四个孩子。
    { href: '/tools/reminders', navKey: 'reminders.title', modules: ['tools'], permission: { all: [] } },
