 ...
 *
 * ════════════════════════════════════════════════════════════════════════
 * ★★【UI-1b(2026-09-05)复裁:'/me' → '/' 。那条"暂定"到期了。】★★
 * ════════════════════════════════════════════════════════════════════════
 *
 * UI-1a 写在这里的是:「这是【暂定】的……Revisit once UI-1b gives home real
 * content」。**UI-1b 给了。** 所以这不是推翻一条裁定,是那条裁定自己约好的复议。
 *
 * 【UI-1a 选 /me 的理由,逐字是这一句】「首页此刻近乎空白,而那上面唯一的东西
 *   是一个刻意不能用的搜索外壳。」**那个前提今天不成立了**:首页有一个彩色字标、
 *   一行叫得出他名字的问候语,以及在节日窗口里换成节日画的那套机制。
 *
 * 【为什么是【首页】而不是继续 /me —— 判据是"这一样东西还有没有别的路"】
 *   · /me 【每一页都够得着】:头像下拉里的「我的档案」,桌面与手机都在。
 *   · 首页也够得着(顶栏那个字标是 <Link href="/">,TopNav.tsx:113/197)——
 *     **但它的内容是【有时效的】**:一句属于此刻这个钟点的问候语,
 *     一张只在两天窗口里出现的节日画。
 *   · **一句要靠人主动去点才看得到的问候语,不是问候语;一张要自己去找的
 *     节日画,不是 Doodle,是彩蛋。** Tim 给这一行写下的目的是
 *     「给首页增加一些温度」—— 而没有人到达的首页,温暖不了任何人。
 *   于是判据落在:两页都只有一次点击的距离,而只有一页的内容【错过就没了】。
 *
 * 【那个"搜索框还是不能用"的反对意见,量过之后没有分量】它在被点之前就在
 *   屏幕上写着「尚未启用」(app/page.tsx 的 badge)——**没有人会被它骗**。
 *   一个诚实标着"还没建"的框,与一张空页,不是同一件东西。
 *
 * 【它对六个人都成立】首页不看任何 module.* 权限;零权限的账号在那里还会
 *   多读到一句解释自己状况的话(OPS-15 那个牌子)。UI-1a 立的那条判据
 *   ——「落地页必须是【这个人】的,不是某一个部门的」——**首页现在也满足它**:
 *   它叫得出他的名字,而没有员工档案的人那一行【不画】,不编一个假名字。
 *
 * ★ 仍然只有这一个地方。★ app/login/actions.ts 与 lib/supabase/middleware.ts
 *   都调 landingPath();改落点永远只改下面这一行。
 */
