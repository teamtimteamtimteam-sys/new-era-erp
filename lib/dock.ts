...
// 它只能指向【注册表里已经有的】二级条目。
//
// ★★【UI-FIX-1(2026-09-02)更正:这里原本写着「scripts/check-dock.mjs 把这条钉死」——
//    ★ 那个脚本【不存在】★,查过 scripts/ 底下没有这个文件,也从来没有过。】★★
// 【所以这一条今天由【两半】守着,而只有一半是机制:】
//   * **人手加的那一侧【是】机制**:app/components/nav/dockActions.ts 对每一个进来的
//     地址核 `FUNCTIONS.some(f => f.href === href)`,核不上就拒绝。这一半是真的。
//   * **下面这份默认候选清单【没有任何构建期检查】。** 一个被某一刀删掉的地址留在
//     这里,defaultDock() 会【静默地跳过它】(`fn ? allows(...) : false`)——
//     不报错,只是默认 dock 少一条,而没有人会发现。
// 【本刀手工核过一次:16 条候选全部仍在 FUNCTIONS 里(⑦ 搬家只换属主、没删条目)。】
// **而"手工核过一次"正是这里需要一个闸的理由** —— 按本仓库那条规矩,
// 一句"记得核一下"是【它该被做成机制】的证据。做那个闸不在 UI-FIX-1 的范围里,
// 所以它作为一条具名的缺口留在这里,而不是继续冒充一个已经存在的检查。
