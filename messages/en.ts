const en = {
    ...
    finance: {
        ...
        reverseUseSourcePathPayment: 'This entry was posted by a payment. A payment is reversed through a reversal request on the payment itself (open the payment and use "Request reversal"), so the CFO approves it first.',
        reverseUseSourcePathTransfer: 'This entry was posted by a bank transfer. A transfer is reversed through a reversal request on the transfer itself (Finance → Bank, "Request reversal" on its row), so the CFO approves it first.',
        reverseUseSourcePathWht: 'This entry was posted by a withholding tax remittance. A remittance is reversed through a reversal request on the remittance itself (Finance → Withholding tax, "Request reversal" on its row), so the CFO approves it first.',
        paymentRequests: {
        ...
        errors: {
            ...
            APPROVAL_NOT_AUTHORISED: 'Only the level-{0} approver ({1}) can decide this. Ask someone who holds that role.',
            APPROVALS_NOT_ENABLED: 'Approvals are not in force, so there is nothing to decide — this request was approved automatically when it was raised.',
            JE_REVERSE_USE_SOURCE_PATH: 'Entry {0} was posted by a payment, a bank transfer, a withholding tax remittance, a payroll posting, the pricing of a goods receipt, an invoice or a credit note, so it cannot be reversed here. Use "Request reversal" on the payment, the transfer (Finance → Bank) or the remittance (Finance → Withholding tax), or "Request unposting" on the payroll period (HR → Payroll); the CFO approves it first. A receipt\'s price is corrected by repricing it on the receipt (Inbound). An invoice or credit note entry is corrected by a void or credit note request on the invoice (Finance → Invoices).',
