const en = {
    logistics: {
        ...
        createBlocked: 'Creating a container is not available yet. The gapless CTR- numberer is deliberately not executable by application sessions (only a SECURITY DEFINER function may call it, exactly as ship_order calls the shipment numberer), so a create_container function has to land on the database side first. Containers created directly on the database appear here immediately.',
