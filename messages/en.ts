const en = {
    gst: {
        ...
        noCodedLines: 'NO LEDGER LINE CARRIES A TAX CODE YET — documents do not feed the F5: invoicing still uses the old scalar rate and posts no journal, and expenses have no tax-code field. So an F5 produced now will be zero in every box, and that does NOT mean there were no supplies. The wiring is blocked on a statutory question (is a supply reported in the invoice period or the sale period) — see docs/known-issues.md.',
        ...
        tiesOk: 'The return ties: output tax read from the tax account agrees with output tax recomputed from the supply values.',
        tiesBroken: 'THE RETURN DOES NOT TIE. Output tax read from the tax account disagrees with output tax recomputed from the supply values — do not file this until the difference is explained.',
