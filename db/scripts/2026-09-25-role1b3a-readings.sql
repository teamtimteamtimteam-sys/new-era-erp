-- db/scripts/2026-09-25-role1b3a-readings.sql
-- ROLE-1 Batch 3a · 前后读数(只读;整支一笔事务,最后 ROLLBACK)。与 Batch 4b 那一支同形,多读:
-- 两个新码的持有人、stocktake_counts(迁移前不存在 → NULL)、盘点单与工单按状态、正式化验张数、
-- 1220 / 5200 两个科目。本刀新增两个码,所以码表要证明【只多了裁定的那四行】。
--
...
SELECT status, count(*) AS work_orders FROM work_orders GROUP BY 1 ORDER BY 1;
...
SELECT p.code, string_agg(r.code, ' ' ORDER BY r.code) AS holders
 ...
 WHERE p.code IN ('data.view_prices', 'data.view_purchase_prices', 'action.price_receipts', 'module.inbound.edit',
                  'module.stocktakes.edit', 'module.stocktakes.view', 'action.stocktake_count', 'action.stocktake_post')
