-- db/scripts/2026-09-24-payrollapr1-readings.sql
-- PAYROLL-APR-1 · 前后读数(只读;整支一笔事务,最后 ROLLBACK)。与 ROLE-1 Batch 2b 那一支同形,换上本刀的对象。
--
...
SELECT a.code, round(COALESCE(sum(l.debit - l.credit), 0), 2) AS balance_debit_positive
 ...
 WHERE a.code IN ('1100', '2000', '2200', '2300', '2400') GROUP BY a.code ORDER BY a.code;
