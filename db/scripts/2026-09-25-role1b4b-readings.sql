-- db/scripts/2026-09-25-role1b4b-readings.sql
-- ROLE-1 Batch 4b · 前后读数(只读;整支一笔事务,最后 ROLLBACK)。与 Batch 4a 那一支同形,多读:
-- receipt_price_requests(迁移前不存在 → NULL)与它在途的张数、payroll_requests 在途、定价状态分布、
-- 已应用化验与含量行数。本刀不新增权限码,所以码表只用来证明【一条都没变】。
--
...
SELECT (SELECT count(*) FROM assay_results WHERE applied_at IS NOT NULL AND deleted_at IS NULL) AS assays_applied,
