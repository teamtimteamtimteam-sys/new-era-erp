...
BEGIN
    ...
    WITH u AS (INSERT INTO auth.users (id, email_confirmed_at) VALUES (gen_random_uuid(), now()) RETURNING id),
         r AS (INSERT INTO roles (code, name_en, name_zh, is_active) VALUES ('fx79-b3b-releaser', 'f', 'f', true) RETURNING id),
    ...
    INSERT INTO roles (code, name_en, name_zh, is_active)
      VALUES ('fx79-apr2-rel', 'f', 'f', true) RETURNING id INTO r_apr2;
