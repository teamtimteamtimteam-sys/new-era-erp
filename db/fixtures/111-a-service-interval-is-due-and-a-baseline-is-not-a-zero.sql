...
-- * 【行为那一半】在这里:支的清单是【恰好】那三十二支(多、少、改名都按名报红),
--   (这个数此前写着「三十」,而数组里当时已经是 31 —— 过期的说明与错的断言
--    让下一个读的人付一样的代价,所以 WHT-1 顺手改正,并把这句话留在这里。)
...
DECLARE
    ...
    v_expected text[] := ARRAY['allocation_stale','ap_over_90','ar_over_90',
        ...
        'invoice_request_pending',  -- APR-5a(2026-09-25):第三十九支
        'journal_request_pending',  -- APR-6(2026-09-25):第四十二支
        'leave_pending','margin_cost_not_allocated','metal_quote_stale',
        'orders_unfulfilled','output_unsold_aging',
        ...
        'terms_request_pending',  -- APR-8(2026-09-26):第四十四支
        'warehouse_request_pending',  -- APR-7(2026-09-25):第四十三支
...
BEGIN
    ...
    IF v_types <> v_expected THEN
        RAISE EXCEPTION 'FIXTURE 111F1 失败:进入 F1 —— 支的清单应当【恰好】是这四十四支 %,实得 %。多一支 = 有人加了臂而没有加规格行(docs/dashboard-arm-inventory.md 的规矩);少一支或改了名 = 本刀的拼接动了不该动的地方;而 dashboard.item.* 的 i18n 键集合【现读同一份清单】,所以两边必须一起动', v_expected::text, v_types::text;
    END IF;
