...
BEGIN
    ...
    WITH u AS (INSERT INTO auth.users (id, email_confirmed_at) VALUES (gen_random_uuid(), now()) RETURNING id),
         r AS (INSERT INTO roles (code, name_en, name_zh, is_active) VALUES ('fx74-b3b-releaser', 'f', 'f', true) RETURNING id),
    ...
    -- ROLE-1 Batch 3b:建工单从本刀起是 action.wo_create(改 / 取消 / 关闭仍认 processing.edit);
    --   G 臂的"编辑写得了"由这个角色演,所以它两个都持。下达仍然不给(G 臂验只读的人放行不了)。
    VALUES (r_edit, 'module.processing.edit'), (r_edit, 'module.processing.view'), (r_edit, 'action.wo_create');
