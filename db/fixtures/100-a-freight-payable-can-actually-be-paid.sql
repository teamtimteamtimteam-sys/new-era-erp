...
BEGIN
    ...
    -- ★ PAY-REQ-1(Tim 的 Q2(c),2026-09-23):运费单从此【生不出】已付的那一种 ——
    --   'paid' 在建单那一刻就按名拒。于是这一臂从"已付的单不许再付"改成"已付的单根本
    --   建不出来"。record_payment_internal 运费支里 payment_status = 'unpaid' 那一条
    ...
    EXCEPTION WHEN OTHERS THEN v_denied := true; v_msg := SQLERRM;
    END;
    IF NOT v_denied OR v_msg <> 'FREIGHT_PAID_AT_CREATION_REFUSED' THEN
        RAISE EXCEPTION 'FIXTURE 100B2 失败:运费单生下来就已付,应按名拒 FREIGHT_PAID_AT_CREATION_REFUSED,实得 denied=% msg=%',
            v_denied, COALESCE(v_msg, '(收下了)');
    END IF;
