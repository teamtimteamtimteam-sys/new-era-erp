...
--
-- 【一个刻意的、写下来的丑陋处】F3② 之后资产成本是 30,000 + 70,000 + 70,000,
-- 因为【冲销一笔追加模式的资本支出不会退回成本】(本刀实测,记在
-- docs/known-issues.md)。这个数在下面被【点名断言】,不是被绕开:
-- 修好那个缺陷的人会拿到一条红,红在正确的那一行上,并且消息直接告诉他改哪里。
BEGIN;
DO $$
DECLARE
...
BEGIN
    ...
    VALUES ('ZZFIX105-M', 'fixture 105 material', 'other') RETURNING id INTO v_mat;
    ...
    -- 【刻意断言一个"不好看"的数】30,000 + 70,000(已冲销,却没退回)+ 70,000。
    SELECT cost_base INTO v_cost FROM fixed_assets WHERE id = v_asset;
    IF v_cost <> 170000 THEN
        RAISE EXCEPTION 'FIXTURE 105F3 失败:资产成本实得 %,而本 fixture 写下时它是 170,000 —— 30,000 定金 + 70,000(已冲销【但没有退回成本】)+ 70,000 重开。那 70,000 是一个【已记录的缺陷】(docs/known-issues.md「冲销一笔追加模式的资本支出,不会退回资产成本」)。如果你刚刚修好了它,正确的期望值是 100,000:改这一行,并删掉 known-issues 里那一条', v_cost;
    END IF;
