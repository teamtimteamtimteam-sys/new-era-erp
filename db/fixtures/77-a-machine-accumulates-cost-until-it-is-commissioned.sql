...
BEGIN
    ...
    -- 三次都是这样(PAYEE-1a、EQP-1b-ii、GST-2),所以它是规律不是意外:regprocedure 找不到
    -- 那个签名就当场报错,于是"签名动了"永远不会静悄悄地过去。改签名的人在这里
    -- 补一个类型,顺带确认自己知道这份 fixture 在注入什么。
    'public.record_expense(date,text,numeric,text,numeric,text,text,uuid,text,text,jsonb,uuid,uuid,text)'::regprocedure);
