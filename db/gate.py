...
★★【--offline 是【多出来的一相】,不是"门,但快一点"】★★(VERIFY-1,2026-09-05)
  ...
  python3 db/gate.py --offline     # 迁移【之前】跑,44 秒,全程不碰线上
...
【为什么值得多跑一相】实测(--offline 那一次 2026-09-05,同一台机器同一个下午;
整门那个区间见下面 ★ 那一段,跨多次运行):
  * 整门 `GATE_OWN_EXIT=0` **2026-09 实测 180–700s,随机器负载而定**;
    本相位 `--offline` **44s**。
    差额几乎全是对线上的往返 —— 光重建 + 193 支 fixture 本身只要 13s
    (单独量过:建集群 0s · 重建 7s · fixture 6s),剩下的 31s 是重建侧的
    列权限/读者/跨模块/导入模板目录查询与每支 fixture 的泄漏指纹。
    ★【为什么这里是一个区间,不是一个数】★(CONSEQ-1,2026-09-09)
      这里原先写着一个 **310s**,并声明它是量出来的 —— 它当时确实是。
      此后同一道门实测到 697s · 571s · 452s · 183s。**问题不在那个数过期了,
      在于这件事上的单点数【一定会】过期**:它量的是一台机器某个下午的负载,
      而不是这道门的成本。区间与月份一起写,读的人才知道自己在看什么;
      而下面那条"写下来的成本必须是量过的成本"因此更立得住,不是被削弱。
      ☞ 观测落在区间外时【把区间放宽】,不要把观测丢掉。
  * C-2 被门抓到的五件事里【四件不需要新库】:镜像列序、一句 `?? []`、
    两支 fixture 漏填必填列、fixture 146 的位置参数错位。
    它们全都落在这 44 秒能回答的范围里,却等到迁移之后才被问。
  * 于是盈亏平衡点也是一个区间:44/700 ≈ **6.3%** 到 44/180 ≈ **24.4%** ——
    **每 16 刀到每 4 刀里有 1 刀被它抓到一次,这一相就回本。**
    区间两端都在"值得跑"这一侧,所以这个结论不因负载而翻转 ——
    而那正是把它写成区间之后才看得出来的事。C-2 那一刀的命中率是 100%。
