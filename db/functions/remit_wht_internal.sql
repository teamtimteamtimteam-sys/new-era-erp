-- db/functions/remit_wht_internal.sql
-- PAY-REQ-1 · Batch B(2026-09-23):remit_wht 的函数体搬到这里,拿掉了写权限检查,
-- 多了一个参数 p_expected_amount(申请冻结的数,付款时推导值不同就按名拒),
-- 返回值多了 entry_id(试跑按它读这次会过账的本位币额)。原来的抬头照录如下。
--
