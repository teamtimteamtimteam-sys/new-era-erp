...
-- FIN-25(2026-08-06):再加工 —— 材料两路(进料批 unit_price → 首挂贷 1200;
-- 产出批 unit_cost_base → 首挂贷 1220 解除上游);差额路两路材料同贷 5000。
-- 第四处置:被下游耗掉的份额 → 5000 停车,传导靠过期旗逐级(不递归)。
-- 无价投料允许不拒,cost_incomplete 打标传染,上游补齐后重跑即清。
