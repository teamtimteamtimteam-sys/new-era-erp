...
--
-- ★★【AP-RECON-1 Batch B(2026-09-24)更正:下面这段"实测"【不成立】,留着是为了让人看见它错在哪】★★
--   它说三个分项把差额"逐分钱解释干净"。算术上是对的 —— 余额确实是 0.00 ——
--   但三个分项是按【机制】分的(起单 / 结算 / 重估),于是【缺陷被吸进了分项里】,
--   而不是留在余额里。AP-RECON-1 grilling(2026-09-24)量到:应付的"起单差异"
--   48,854.98 = 49,204.00 切换前的单据 − 4,032.00 注销仍欠钱的收货(当时是活缺陷)
--   − 20.70 带税费用单的税(当时是活缺陷)+ 3,703.68 截在今天的 2027 运费原件 ——
--   四项之和逐分等于它,而它报 reconciled = true。
--   ☞ 所以:这支函数回答的是"差额按机制分成了哪几块",【不】回答"差额里有没有没人解释过的东西"。
--     后一个问题归 list_ledger_reconciliation(全账、不截日,只允许逐单据登记的残留 +
--     重估 + 挂账三种具名的差,其余一律未解释)。本函数的签名与返回键不动(Tim AP-RECON-1 Q8)
--     —— 冻在 management_packs 里的包读它;管理包那一版的改基排在队列里。
--
-- 【实测(2026-08-28),三个分项把差额【逐分钱】解释干净,余额 0.00】(★ 见上:不成立)
...
--
-- ═══════════════════════════════════════════════════════════════════════════
-- 【INV-VAL-1(2026-08-31):第三、第四条腿 —— 存货】
--   AR/AP 两条腿的【算术与注释都原样保留】(fu5 记着我第一次是怎么弄丢注释的)。
--   后面接上 inventory_control_reconciliation 给出的
--   inventory_raw(1200)与 inventory_fg(1220)。
--
--   · 四条腿现在都带一个 `variances` 数组(名目 + 金额),因为存货的六个名目
--     与 AR/AP 的三个完全不同,一个通吃的循环才渲染得了四条腿;
--     既有的三个键【原样保留】,于是已经冻在 management_packs 里的包
--     与 CSV 导出一行都不用改。
--   · 存货明细侧【只答得出此刻】:as-at 早于今天时按名拒
--     (BUSINESS_DATES_INCOMPLETE / CAPITALISATION_AFTER_AS_OF),
--     数字为 NULL、reconciled 为 NULL —— **答不上来不是对不上**。
--     照答会返回一个自信的 0.00,那正是 INV-VAL-0 在"2026-06-30 的存货"上量到的。
-- ═══════════════════════════════════════════════════════════════════════════
