-- PROC-COST-1 fu3(2026-08-31):零成本的放电单【不写载体行】
--
-- 【为什么这是一条真的缺陷,而不是洁癖】
-- 载体行是【第七过期源】。主刀里,状态改变型分支无条件先删后插 —— 于是一张
-- **一分钱成本都没有**的放电单也会写下一行 amount_base = 0 的载体,
-- 而那一行的 created_at 会把【吃过那批料的下游加工单】标成过期。
--
-- 那张下游单被要求重跑,而重跑出来的数与它现在的数**一模一样** ——
-- 因为什么都没变。这正是本仓库成文禁止的那种旗:
-- fixture 54 为金属含量钉过同一条("weight 拆分不读含量,含量变了它不过期 ——
-- 无条件举旗是喊狼来了,而没人看的旗和没有旗是同一样东西")。
--
-- 【DELETE 仍然无条件执行,那一半不能省】一次 300 → 0 的重分摊
-- (成本条目全被软删)必须真的把那一行拿掉,否则基函数还会读到 300。
-- 也就是说:**删是无条件的,插是有条件的** —— 两者不对称,而那是对的。
--
-- 【为什么不在主刀里就写对】主刀把注意力放在"钱要落到哪里",而这一条问的是
-- "什么都没有的时候要不要留一行" —— 它是在写第七过期源的 fixture 时才显形的。
