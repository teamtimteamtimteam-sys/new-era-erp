-- PRICE-1 fu1:index_period_average 少了一次【按名】的权限拒绝。
--
-- 【它是写 fixture 时抓出来的】本函数是 SECURITY INVOKER,而 index_market_calendar 有 RLS。
-- 于是一个没有 module.pricing.view 的读者,看到的日历是空的 —— 他拿到的是
-- QP_CALENDAR_NOT_COVERED,读起来是「数据缺了」,而实情是「你没权限」。
-- **两者必须分得开**(lib/permissions.ts 的全部理由:null 已经有别的含义了)。
-- 处置:进函数先问一次权限,按名拒 PRICING_PERMISSION_DENIED。
--
-- 顺带钉死另一件事:本函数【不是】SECURITY DEFINER。一支 definer 函数
-- 没有权限检查是本仓库点名过的陷阱;这里两头都占住 —— 它是 invoker,
-- 而且自己还问了一次。
