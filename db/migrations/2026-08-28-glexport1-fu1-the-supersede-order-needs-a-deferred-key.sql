-- db/migrations/2026-08-28-glexport1-fu1-the-supersede-order-needs-a-deferred-key.sql
-- ════════════════════════════════════════════════════════════════════════════
-- GLEXPORT-1 fu1:重出一份包时,两条约束把顺序逼死了 —— 外键必须延迟检查。
--
-- ★【探针当场抓到的,两次】★ 在 live 上跑一次【回滚掉的】重出探针,连撞两条:
--   ① `idx_management_packs_live_month` 是【部分唯一索引】
--      ((period_month) WHERE superseded_at IS NULL)—— 一个月只许有一份在册。
--      于是"先插新的、再标旧的"在那一瞬间有两份在册,撞唯一索引。
--   ② 把顺序倒过来(先标旧的、再插新的)之后,`superseded_by` 指向的是一行
--      【还没有插进去的】记录,撞外键。
--
--   **两条约束各自都是对的,而它们合起来只允许一个顺序:
--     先标旧行 → 再插新行,并把外键延迟到提交时检查。**
--
-- 【为什么不是把唯一索引去掉】那条索引正是"一个月一份在册"这件事的机制,
--   而它是本表最要紧的不变量之一。为了让写入顺序好写而拆掉一条不变量,
--   是拿正确性换方便。
-- 【为什么不是把外键去掉】superseded_by 指向的那一行必须真的存在 ——
--   一条指向不存在记录的取代关系,读起来和"没有被取代"一模一样。
--
-- 【本刀无数据变更】只改一条外键的检查时机,外加替换那支函数的语句顺序。
--   覆盖它的备份是 2026-08-28-2159(BACKUP_EXIT=0,pg_restore --list 已验)。
-- ════════════════════════════════════════════════════════════════════════════

BEGIN;

ALTER TABLE public.management_packs
    DROP CONSTRAINT management_packs_superseded_by_fkey;
ALTER TABLE public.management_packs
    ADD CONSTRAINT management_packs_superseded_by_fkey
        FOREIGN KEY (superseded_by) REFERENCES public.management_packs (id)
