-- INV-VAL-1-fu4:把「此刻」做成【真正的默认值】,而不是在 TS 里casting 一个谎
--
-- fu3 让 p_as_of 为 NULL 时取 CURRENT_DATE,但签名上它仍是必填,于是
-- supabase gen types 把它生成成 `p_as_of: string`,应用侧要传 null 就得写
-- `null as unknown as string`。**那是一个骗过类型检查的强制转换** ——
-- 类型说"这里一定有个字符串",而运行时那里是 null。下一个读这行代码的人
-- 会以为传 null 是个 hack,而它其实是这支函数被设计出来的用法。
--
-- 所以默认值写在它该在的地方:函数签名上。DEFAULT 不改变函数标识
-- (签名仍是 (date)),CREATE OR REPLACE 就够,不用 DROP —— 也就不会
-- 连带把 EXECUTE 授权丢掉。
--
-- 【顺带记一条判据】需要在 TS 里 cast 才能调用的 RPC,通常说明 SQL 侧的契约
-- 没写完整。改 SQL,不要改 cast。
