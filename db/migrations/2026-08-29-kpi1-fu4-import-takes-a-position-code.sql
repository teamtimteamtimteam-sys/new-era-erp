    ...
    FOR v_i IN 0 .. v_n - 1 LOOP
        ...
        FOR v_ref, v_bad_key IN
            SELECT * FROM (VALUES
                ...
                -- KPI-1:职位也按编号给。**不加这一条,带职位的员工导入就做不了** ——
                -- position_id 是一个 uuid,而本段开头那句话正是"操作员不可能手打 uuid"。
        ...
        LOOP
            IF v_row ? v_ref AND coalesce(btrim(v_row ->> v_ref), '') <> '' THEN
                ...
                ELSIF v_bad_key = 'position_id' THEN
                    SELECT id INTO v_uuid FROM positions WHERE code = btrim(v_row ->> v_ref);
                ELSE
