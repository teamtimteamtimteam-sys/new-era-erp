-- db/migrations/2026-08-06-fin25c-clear-movement-ctx.sql
--
-- FIN-25c:commit/rollback 用完 movement_ctx 不清 —— 同事务内残留的
-- 'processing:...' 会让 FIN-25 的投入腿守卫对后续裸 INSERT 放行。
-- price_ctx 的注释早把道理写明了(「用毕即清,免得同事务内后续的直改被误放行」),
-- movement_ctx 当时不需要清 —— 投入腿守卫触发器是 FIN-25 才建的。fixture 19F 实测撞出。
