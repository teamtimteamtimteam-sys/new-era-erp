...
BEGIN
    ...
    v_by_source := jsonb_build_object('material', round(v_material, 2));
    FOR v_ct IN
    ...
    ELSE
        ...
        LOOP
            IF v_ct.d <> 0 THEN
                v_cap_lines := v_cap_lines || jsonb_build_object(
                    'account_code', CASE WHEN v_ct.src = 'material' THEN '5000' ELSE fin_cost_account(v_ct.src) END,
        ...
        LOOP
            ...
            v_other := GREATEST(0, v_leg.qty - v_leg.remaining_qty - v_leg.sold_cogs - v_leg.sold_nocogs);
            v_l1220 := round(v_delta * (v_leg.remaining_qty + v_leg.sold_nocogs) / v_leg.qty, 2);
            v_l5000 := round(v_delta * v_leg.sold_cogs / v_leg.qty, 2);
        ...
        IF v_d5000 <> 0 THEN
            v_cap_lines := jsonb_build_array(jsonb_build_object('account_code', '5000',
                ...
                'line_memo', 'sold share — COGS catch-up')) || v_cap_lines;
        END IF;
        IF v_d5200 <> 0 THEN
