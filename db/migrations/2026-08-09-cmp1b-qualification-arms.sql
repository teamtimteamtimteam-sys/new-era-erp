-- CMP-1b:看板补两支资质臂(qualification_expiring / qualification_missing)
-- 与 OPS-19 不同,这两支【今天每个状态都可达】:live 有一张 2024 年就过期的证书,
-- 也有没有任何证书的供应商 —— 不存在"造不出可判别状态"的问题。
   ...
   FROM ( SELECT 'awaiting_assay'::text AS item_type,
        ...
        UNION ALL
         SELECT 'qualification_expiring'::text AS item_type,
            ...
            (ct.name_en || ' — ' || s_1.legal_name) AS subject,
          ...
          WHERE sc.deleted_at IS NULL AND s_1.deleted_at IS NULL
            AND ct.disposition <> 'ignore'
    ...
    UNION ALL
         SELECT 'qualification_missing'::text AS item_type,
           ...
           FROM suppliers s_2
          WHERE s_2.deleted_at IS NULL AND s_2.status = 'active'
