-- FRT-1 fu1(2026-08-11):单据先落地,分录号后补 —— 顺序是被外键逼出来的
--
-- 第一版照抄了 record_expense 的顺序(先过分录,再插单据,单据一次性带上分录号)。
-- 那条顺序在这里【不成立】:freight_allocations 的外键指向本单,而分摊必须在
-- 过账【之前】算完 —— 拆账的金额就是分录的行。于是第一版一跑就撞外键:
--   freight_allocations_freight_document_id_fkey ... is not present in freight_documents
--
-- 【记一笔,因为"照着旁边那个函数写"是这个仓库的默认动作】它多数时候是对的,
-- 而这一次不对的原因很具体:expenses 没有子表,freight 有。子表把顺序钉死了。
