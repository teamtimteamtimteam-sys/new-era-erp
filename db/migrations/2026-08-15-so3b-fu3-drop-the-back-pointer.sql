-- db/migrations/2026-08-15-so3b-fu3-drop-the-back-pointer.sql
-- SO-3b 收尾:撤掉那个【指回发货行】的冗余外键 —— 它让镜像循环依赖
--
-- ═══════════════════════════════════════════════════════════════════════════
-- 【verify_rebuild 当场抓到的】主刀给 sales_order_reservations 加了
-- shipment_line_id → shipment_lines,而 shipment_lines.reservation_id 本来就
-- 指着 sales_order_reservations。两张表互指 ⇒ 表镜像之间【循环依赖】,
-- 重建排不出建表顺序:
--     表镜像之间存在循环依赖,无法排序:[sales_order_reservations, shipment_lines]
-- 判词【可重建性】因此直接红 —— 而线上完全正常(ALTER 是逐条加的,没有顺序问题)。
-- 这正是那个判词存在的理由:线上跑得通,不等于这个仓库建得出库。
--
-- 【撤掉哪一边:撤冗余的那一边】shipment_lines.reservation_id 是 UNIQUE,
-- 映射本来就是一对一 ——"这条预留被哪一行发货消耗了"反查得到,不需要第二个
-- 指针。留着它是两处真相里多出来的那一处(与 sales_records.movement_id 被
-- SO-2b 撤掉是同一条:一处真相,或者不做)。
-- consumed_at / consumed_by 保留 —— 它们记的是【事实本身】,不是指针。
--
-- 【破窗】纯删一列冗余列 + 一条 CHECK;没有任何读者(它是本刀刚加的)。
-- 镜像:db/tables/sales_order_reservations.sql、
--       db/functions/{ship_order,guard_sales_order_reservation_append_only}.sql。
-- ═══════════════════════════════════════════════════════════════════════════

BEGIN;

ALTER TABLE public.sales_order_reservations
    DROP CONSTRAINT sales_order_reservations_consume_complete,
    DROP COLUMN shipment_line_id;
