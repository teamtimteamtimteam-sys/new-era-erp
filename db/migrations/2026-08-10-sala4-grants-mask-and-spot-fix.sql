-- SAL-A4:三件收尾 —— 遮蔽表加列的两半、报价函数的现货分支修复
--
-- 1)【给遮蔽表加列 = 同一迁移里做两件事】(AGENTS.md 那一节,colgrant 顶出来的):
--    price_source 是标签,不敏感 → 列级授权 + 遮蔽视图原样;
--    price_provenance 【含价格】(USD 单价、汇率、行情行)→ 不授权,只进遮蔽视图、
--    按 data.view_prices 置空 —— 与 unit_price/amount_base 同一档。
-- 2)price_output_sale 的现货分支炸在【未赋值 record 的字段访问】上:
--    CASE WHEN p_formula_id IS NOT NULL 救不了它 —— PL/pgSQL 对未赋值 record 的
--    字段访问在求值前就抛。换标量,顺带少一次整行取数。fixture 38 D 臂顶出来的。
-- NOTE: apply with ./db/apply_migration.sh
BEGIN;

GRANT SELECT (price_source) ON public.sales_records TO authenticated;
...
CREATE OR REPLACE VIEW public.sales_records_masked WITH (security_invoker = off) AS
 SELECT id,
    ...
    created_by,
    cogs_entry_id,
    -- SAL-A:出处。source 是标签,原样;provenance 含价格(USD 单价、汇率、行情行),
    -- 与 unit_price 同档 —— 按 data.view_prices 置空。
    price_source,
        CASE
