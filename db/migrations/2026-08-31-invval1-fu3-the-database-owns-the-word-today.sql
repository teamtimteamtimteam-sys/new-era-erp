-- INV-VAL-1-fu3:实时快照的「今天」只能由数据库说了算
--
-- 【发现方式:量了一下 DB 的时区,而不是假设它是 UTC】
--   线上 current_setting('TimeZone') = **Asia/Singapore**,而应用侧算日期用的是
--   new Date().toISOString().slice(0,10) —— 那是 **UTC** 日期。
--   今天 17:55 SGT 两者相同,所以随手一测是绿的;但每天 00:00–08:00 SGT,
--   UTC 还停在前一天,应用会把"昨天"当作 as-at 送进来,于是 fu1 加的那条
--   AS_OF_NOT_RECONSTRUCTABLE 会把这张页面拒掉,**每天八个小时**。
--
-- 【修法不是放松判词,是把"今天"的权威还给数据库】p_as_of 为 NULL 时取
--   CURRENT_DATE。这与 gl_control_reconciliation 的 AS_OF_REQUIRED 不矛盾:
--   那一条防的是【历史时点被默默填成今天】(无中生有);这一支是实时快照,
--   「此刻」就是它的含义,不是一个被编造出来的时点。差别写进了函数体。
