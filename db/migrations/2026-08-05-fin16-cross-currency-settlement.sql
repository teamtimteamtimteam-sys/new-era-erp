...
BEGIN
    ...
    LOOP
        ...
        ELSIF v_po_id IS NOT NULL THEN
            ...
            -- 【为什么这一支要单独折算,而不是跟着上面三支一起"放开就完了"】
            -- 另外三支结算的是【有敞口的单据】:核销额本来就以单据币种计,敞口
            -- 逐笔递减到零,币种天然对齐。预付没有敞口(v_doc_value 为 NULL,
            -- 下面的 ALLOC_EXCEEDS 整段跳过),它唯一的栏杆就是这条 1.5 倍上限。
            -- 而 estimated_total_usd 【存的是单据自己的币种】(create_purchase_order
            -- 直接累加行金额,全程不乘汇率 —— 名字里的 _usd 是 FIN-1a 留下的旧名,
            -- 与内容不符,已记入 docs/known-issues.md)。
            -- 所以放开币种之后,若不折算,就成了"拿付款币种的数去比单据币种的上限":
            -- SGD 8,000 打给一张 USD 6,000 估算的 PO,朴素比较是 8,000 对 9,000 —— 过;
            -- 折成 USD 约 6,450 对 9,000 —— 也过,但两个数从此不再是同一个单位,
            -- 换个汇率就会朝【放行】的方向出错。栏杆必须两边同币种。
