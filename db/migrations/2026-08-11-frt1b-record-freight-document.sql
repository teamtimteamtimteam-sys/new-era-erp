-- FRT-1 第二部分(2026-08-11):运费单据的写入口 —— 分摊、拆账、过账
--
-- 抬头的完整理由在第一部分。这里只记它没说完的三件事。
--
-- 【一 · 三种口径各自的拒绝,而不是各自的猜测】
--   weight  —— 混装批次【单位不一致】就拒绝。跨不同单位的"按重量分"不是近似,
--               是没有意义;今天全是 kg,所以这条拒绝今天不会响,而它挡的是明天。
--   value   —— 任一批次 unit_price 为空就【点名拒绝】(线上 13 批里有 3 批是这样)。
--               给未计价批次一个零份额,等于把它的运费悄悄摊给别人。
--   stated  —— 逐批列明,而且必须【正好加总到单据金额】。差一分就拒。
--
-- 【二 · 拆账比例取【过账那一刻】,并记在行上】
-- 迟到的运费是主路径:收货 → 加工 → 卖出 → 发票才到。所以每一批各按它【此刻】的
-- remaining_qty / quantity 拆:在库份额进 1200,已耗份额进 5000。
-- 收货即到的运费在这里是 ratio = 1,同一条路径的边界情形。
--
-- 【三 · 贷方是货代 —— 这一段是新写的,不是从 reprice 抄的】
-- 借方的拆账算术与 reprice_split 同源(同一个问题),贷方不是:
-- 既有的每一条差额路径都贷 2000 且意指【材料供应商】。这里贷的是【货代】,
-- 由 freight_documents.supplier_id 指明,应付账龄按 doc_kind='freight' 认领它。
-- 照抄 reprice 的贷方会让分录照样平、照样不报错,而钱记在了错的人头上。

BEGIN;

CREATE OR REPLACE FUNCTION public.record_freight_document(
    p_doc_date date,
    p_supplier_id uuid,
    p_amount numeric,
    p_currency text,
    ...
    p_allocations jsonb DEFAULT NULL,   -- [{inbound_batch_id, amount_base?}]
...
BEGIN
    ...
    INSERT INTO freight_documents (id, code, doc_date, supplier_id, amount_ccy, currency,
        ...
        notes, journal_entry_id, created_by, updated_by)
    VALUES (v_doc_id, v_code, p_doc_date, p_supplier_id, p_amount, p_currency,
        ...
        p_notes, (v_je->>'entry_id')::uuid, v_user, v_user);
