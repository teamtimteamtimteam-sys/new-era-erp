...
SELECT
    ...
    r.code                                  AS run_code,
    po.unit_cost_base,
    -- 成本 = 单位成本 × 【已售】数量(收入也是已售那部分的)
    round(po.unit_cost_base * s.qty_sold, 2) AS cost_current_base,
    -- 【NULL 传播就是第二条规矩本身】unit_cost_base 为 NULL 时这两列自然为 NULL
    round(s.revenue_base - po.unit_cost_base * s.qty_sold, 2) AS margin_base,
    CASE WHEN s.revenue_base <> 0
         THEN round((s.revenue_base - po.unit_cost_base * s.qty_sold) / s.revenue_base * 100, 1)
    END                                     AS margin_pct,
    -- 算不出来时【说出是哪一种算不出来】—— 让"没有成本依据"成为一行的内容,
    -- 而不是让这一行消失
    CASE WHEN po.id IS NULL           THEN 'no_run'::text
         WHEN po.unit_cost_base IS NULL THEN 'no_unit_cost'::text
         ELSE 'ok'::text
    END                                     AS margin_status,
    COALESCE(po.cost_incomplete, false)     AS cost_incomplete,
    (r.allocated_at IS NOT NULL
     AND c.last_cost_change IS NOT NULL
     AND c.last_cost_change > r.allocated_at) AS is_stale,
    s.cogs_posted_base,
    -- 【总账口径与管理口径不同,不是错】见文件头第三条
    (s.cogs_posted_base IS NOT NULL
     AND po.unit_cost_base IS NOT NULL
     AND s.cogs_posted_base <> round(po.unit_cost_base * s.qty_sold, 2)) AS cogs_differs
FROM output_batches ob
    JOIN materials m ON m.id = ob.material_id
    -- 【主表是 output_batches】—— 没有加工单的批次必须留在结果里
    LEFT JOIN processing_outputs po ON po.output_batch_id = ob.id
    LEFT JOIN processing_runs r ON r.id = po.run_id
    -- 过期判定:与 processing_run_allocation_status 同一份定义(三个源),就地算 ——
    -- 理由见文件头第四条;fixture 31 D 臂断言两者一致
    LEFT JOIN LATERAL (
        SELECT max(x.ts) AS last_cost_change
        FROM (
            SELECT GREATEST(e.created_at, e.updated_at) AS ts
