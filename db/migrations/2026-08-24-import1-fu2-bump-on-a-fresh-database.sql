-- 2026-08-24-import1-fu2-bump-on-a-fresh-database.sql
-- IMPORT-1-fu2:序列推进在【全新重建的库】上根本没有跑 —— 由 fixture 124 抓出来。
--
-- 【缺陷】`pg_sequences.last_value` 在一条**从来没有被取过号**的序列上是 **NULL**。
-- 一个刚重建出来的库正是这种状态 —— **而生产就是一次重建**。
-- 于是那个比较是 `14001 > NULL` = NULL,IF 永不成立,**推进被静默跳过**。
--
-- 【它为什么在线上看不出来】线上的 material_code_seq 早就被取过号(76),
-- 所以 last_value 有值,推进照常发生。**这个缺陷只在【要紧的那种库】上出现,
-- 而且不报任何错** —— 正是本仓库反复点名的那一族:一个在开发机上永远绿的判据。
--
-- 【抓住它的是谁】fixture 124 的 A 臂,而它本来只是一条"前提读得到吗"的检查。
-- 那条前提臂在重建库上读到 NULL 就停了 —— **一条前提臂救了它要证明的那件事本身。**
