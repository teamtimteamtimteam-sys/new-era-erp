...
CREATE INDEX pricing_formulas_code_trgm ON public.pricing_formulas USING gin (code extensions.gin_trgm_ops);

-- SEARCH-2b · 迁移 C:「最近编辑过」要的那一条 —— `updated_by = auth.uid()`
-- 按 updated_at DESC 取前 5(T3)。SEARCH-0 §Q5 实测:这两列上此前一条索引都没有。
CREATE INDEX pricing_formulas_recents ON public.pricing_formulas (updated_by, updated_at DESC);
...
--   delete_pricing_formula。直连写由 trg_pricing_formulas_direct_write 按名拒(零行也拒)。
