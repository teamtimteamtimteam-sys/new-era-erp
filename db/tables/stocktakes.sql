...
CREATE INDEX stocktakes_code_trgm ON public.stocktakes USING gin (code extensions.gin_trgm_ops);

-- SEARCH-2b · 迁移 C:「最近编辑过」要的那一条 —— `updated_by = auth.uid()`
-- 按 updated_at DESC 取前 5(T3)。SEARCH-0 §Q5 实测:这两列上此前一条索引都没有。
CREATE INDEX stocktakes_recents ON public.stocktakes (updated_by, updated_at DESC);
