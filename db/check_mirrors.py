...
RUNTIME_CONFIG_TABLES = [
    ...
    # ★ UI-1b:首页那 72 句问候语 —— **这张表存在的全部理由就是"改一句不用部署"**。
    #   逐行比对它,等于把它变回一份要走迁移才能改的文案文件,也就等于取消它。
    "home_greetings",
    # ★ UI-1b:23 个节日画与它们的窗口 —— 加一个节日 = 丢一张图进源目录 + 加一行,
    #   与 certificate_types 同一条。**注意它【不是】 public_holidays**:
    #   那张是工作日事实,这张是装饰,两者不共用一行(理由在两张表的抬头)。
    "festival_doodles",
...
DEFINER_NO_CHECK_ALLOWED = {
    ...
    # APR-5a:贷项 / 作废申请的内层算子 —— 靠的是调不到(zzz_function_grants.sql)。
    "void_invoice_internal": "EXECUTE revoked from PUBLIC/authenticated/anon",
    "create_credit_note_internal": "EXECUTE revoked from PUBLIC/authenticated/anon",
    ...
    # APR-7:注销 / 回滚 / 证书作废申请的内层算子与读者 —— 靠的是调不到(zzz_function_grants.sql)。
    "soft_delete_inbound_batch_internal": "EXECUTE revoked from PUBLIC/authenticated/anon",
    "soft_delete_output_batch_internal": "EXECUTE revoked from PUBLIC/authenticated/anon",
    "rollback_processing_run_internal": "EXECUTE revoked from PUBLIC/authenticated/anon",
    "warehouse_request_submit_internal": "EXECUTE revoked from PUBLIC/authenticated/anon",
    "warehouse_request_execute_internal": "EXECUTE revoked from PUBLIC/authenticated/anon",
    "warehouse_request_dry_run": "EXECUTE revoked from PUBLIC/authenticated/anon",
    "warehouse_request_touches": "EXECUTE revoked from PUBLIC/authenticated/anon",
    "warehouse_request_freezing": "EXECUTE revoked from PUBLIC/authenticated/anon",
    "warehouse_request_snapshot": "EXECUTE revoked from PUBLIC/authenticated/anon",
