...
## ★★★ 一刀的终端活【到推送为止】—— 部署是 Tim 自己看的(Tim 2026-09-12 立的常设规矩)

> ### ★ 规矩本身
> **一刀的终端工作【结束在推送那一刻】。**
> 推送之后:**报出 `HEAD` · `origin/main` · `git ls-remote` 三个【完整 40 字符】SHA,
> 确认三个逐字相同,然后【停下来】。**
>
> ★★ **它【不】等部署、【不】查部署状态、【不】把 deployment id 绑到 SHA、
> 【不】复现一次失败的构建、也【不】去调查一次失败的构建 —— ★★ 哪怕有人告诉它部署失败了。**
> ★ **Vercel 面板由 Tim 自己看;如果有日志,由 Tim 把日志贴过来。**

**理由 —— 它有一次实测的价钱,照直记:**

★ **这台机器够不到 Vercel。** 五项逐条实测全部 ABSENT(`vercel` CLI · devDependencies ·
`.vercel/project.json` · `VERCEL_TOKEN` · `~/.vercel`)—— 上面那一节已经把这五项钉死过两次。
☞ **于是"在这里调查一次失败的构建"实际上意味着:一次冷 clone + 一次冷 `npm ci` +
一次冷 `npm run build`,而它【仍然】以"请 Tim 把日志贴过来"收场。**

★★ **2026-09-12 这件事真的整趟跑了一遍(BUGFIX-1a):**
构建红了 → 冷 clone 到 `/tmp` → `npm ci` → `npm run build` → ★ **全绿**(`REPROBUILD_OWN_EXIT=0`)→
七条假设逐条排除(缺声明的依赖 · 原生包 · `@source` 指到未提交目录 · 脚本没提交 ·
build 串里的隐形字符 · 大小写 · CRLF)→ ★★ **一条都没有命中。**
病因是 **Tim 把日志贴回来之后第一眼就看见的那一行**:
`check-generated-css:覆盖断言失败 …… 判据数出 922,独立计数是 0`。
☞ ★★ **那一整趟冷复现买到的东西是【零】**,而它花掉的时间是真的。

> ### ☞ 判词
> ★★ **一个够不到真源的进程,不该去调查那个真源上发生的事。**
> 它能做的只有猜,而 **一条写进报告的猜测,读起来和一条裁定一模一样** ——
> 这正是本文件反复付账的那一件事。
> ★ **它该做的是:把推送报干净,然后把问题交给够得到真源的那个人。**

**与上面那一节的关系 —— 两条不冲突,它们管的是不同的问题:**
上面「**部署这件事的真源是 Vercel,不是 GitHub 的部署记录**」管的是
**"部署这件事该问谁"**;这一条管的是 **"这个进程该不该去问"** —— ★★ **答案是:不该。**

* ☞ **一刀的终端流程里【不再跑】那条部署等待**(`scripts/wait-for-deploy.sh` /
  `db/wait_for.sh --label "部署"`)。它们作为工具没有问题,**变的是谁来跑**。
* ★ **破窗**(迁移提交 → 部署成功)那个字段**仍然要填**,★ **但它的终点读数从 Tim 那里来** ——
  他在 Vercel 面板上看到成功的那一刻,**不是**这台机器发出的一次查询。
  ☞ 交回报告里要**把这件事写出来**:一份**转述**和一次**测量**在报告里长得很像,而它们不是一回事。

### ★ 同一天立的第二条 —— 一行以 `!` 开头的输入,是【原样交给 shell 执行】的

★ **`!` 那一行会被逐字当成命令跑,所以那一行里【只能有命令本身】。**
☞ **说明、理由、附加的指示要另发一条消息** ——
写在 `!` 那一行里,shell 会试着把它当命令执行,而**它不会到达该读它的人**。
...
> ☘ **CONFIRM-1(2026-09-06)之后,这一族有了一个更窄、更好认的名字:见下面
> 「★★★ 委托书里的【数】来自上一份报告,而不是来自一次测量」。**
> 那一节说的不是「委托书有时会写错」,而是一个**有出处、可预测、连着三刀命中**
> 的具体机制 —— 因此处置也更硬:不是「多留神」,是**开工前必须重量一遍**。
...
## ★★★ 委托书里的【数】来自上一份报告,而不是来自一次测量(CONFIRM-1,2026-09-06)

**Tim 自己指出并要求记下来的,而且要求按【一个失效模式】记,不要按【三次更正】记。**
「三次更正」读起来像三次运气不好;**一个失效模式有出处、有方向、可以预防。**

### 机制,一句话

> **一刀的报告里写下一个数 → 那个数被抄进下一刀的委托书 → 下一刀把它当成前提,
> 而不是当成一条待核对的断言。中间【没有任何一步重新量过】。**

数在这条链上**只会变硬,不会变准**:报告里它带着上下文(「当时的 grep」「被
`head` 截断了」),抄进委托书就只剩一个光秃秃的整数,而整数看起来像事实。
...
一条**描述**错了,读的人常常能看出别扭(「upload panel 从来不碰字节?真的吗」)。
一个**整数**错了,看起来和对的整数一模一样 —— 它没有可以被怀疑的表面。
而委托书里的数恰恰是用来**定工期、划范围、排优先级**的:
CONFIRM-1 按 56 处报的价,实际是 40 处;UI-1d 按 30MB 报的价,实际是 0。
**报错价的那一刀,Tim 是照那个价点的头。**

### 处置(两条,都是开工前的,不是收尾时的)

1. ★ **委托书里的每一个数,开工前当场重量一遍,并把重量的结果写进报告** ——
   包括**量下来是对的**那些(写「零条」也要写)。这条已经是本文件上一节的要求;
   这里把它收窄成一句可执行的:**凡是委托书里出现的数字,一个都不许直接引用。**
2. ★★ **量的时候,先问那个数【数的是什么】。** CONFIRM-1 的 56 是
   `grep -c window.confirm` —— 它同时数了注释,又漏了没有 `window.` 前缀的那一处。
   **一个数错的常见原因不是数错了,是它数的东西和它的名字对不上。**
   所以见「★ 一句注释可以污染将来对它自己的计数」那一条:
   **注释与调用点分开数,两个数都报出来。**

### ★★ 一条【只活在聊天里】的裁定,在终端这边读起来就是【还没答】(INPUT-2,2026-09-10)

**这是同一族的另一张脸,而它比「数抄错了」更贵:**
Tim 在 2026-09-09 已经裁了输入框的高度(32px)。**那条裁定没有进 `docs/variant-c-spec.md`。**
于是 INPUT-1 开工时读到的是 spec §9 Q3 —— 「**输入框的目标高度是 32px 还是 44px?仍然未决,
而且它是本族最贵的一个数字**」—— **它照着那句话停了下来,一个控件都没有转。**
一整刀的工时,买的是一次「这件事其实已经定了」。

> **判据一句话:一条裁定如果只存在于对话里,对终端这边【就是不存在】。**

**处置(两条,一条给委托书,一条给刀):**

1. ★ **委托书引用一条裁定时,要指出它【写在哪里】** —— 文件与小节。
   指不出来的,那一条就还不是一条裁定,是一句转述。
2. ★★ **一刀如果要应用一条【spec 里还没有的】裁定,就在【同一个提交里】把它写进 spec。**
   不许「先施工,回头补文档」——**回头那一步是这个仓库反复丢掉的那一步**,
   而它丢掉之后,下一刀读到的仍然是那句「仍然未决」。

### 报告那一侧也要配合 —— 这是本条唯一能治本的地方

> **一个数写进报告时,把【它是怎么量出来的】和它写在一起。**

「40 处」是没用的;「`grep -rn 'window\.confirm(' app/` 去掉注释行之后 40 处,
另有 16 处注释、1 处没有 `window.` 前缀」是有用的 ——
**因为下一个人抄走它的时候,会连同它的量法一起抄走,于是他重量得起来。**
一个不带量法的数,注定在下一份委托书里变成一句无法核对的断言。

> ★★ **APR-4(2026-09-23,Tim 要求记下):交回报告里的「Measured」必须指出量它的那条查询**
> —— 读的是哪张表或视图、以什么身份、那条查询住在哪个文件里。APR-ROUTE-1 Batch B 写着
> 「**Measured:** `cfo` 只有两个码」,而它的走证脚本里**一条读 `cfo` 码的查询都没有**,那句话只是一句注释;
> 实测是五个码(含 `module.purchasing.view`)。它把一个假决定送到了 Tim 桌上。
> ☞ **这一族的新脸:没有测量的断言不是从上一份报告抄来的,而是【就写在报告里、标着 Measured】。**
> 写不出那条查询的「Measured」,改写成「推断」。
...
> ☞ 最便宜的解药是给那个容器 **`flex-wrap`**:`/logistics/lanes` 实测,
> 加上它并把 INPUT-2 那三个宽度类拿掉,390px 整页溢出 **+4 → 0**,
> 而桌面那一档**逐字未变**(那三个宽度类挑的数,恰好就是标准自己算出来的)。

★ **这一条同时是「★★★ 覆盖率本身必须是一条断言」那一节的又一个实例,而且是新的一张脸:**
那个检出器**带着覆盖断言**(它报了 `flexHosts` / `nowrapHosts` / `hostsWithBand2`,
证明自己**确实在看**),而它仍然报了一个**假的零** ——
**因为它看得见自己在看,却看不见自己【看的是错的性质】。**
☞ 覆盖断言证明的是"我扫过了",**不是**"我问对了问题"。两者都要被怀疑一次。

### ★★ 一个【证明了自己在看】的检出器,仍然会报出一个假的零 —— 因为它看的是错的性质(INPUT-2b,2026-09-11)

**一次覆盖断言答的是「我扫到了多少个候选」,它答不了「我问的是不是那个决定结果的性质」。**
INPUT-2b 那个检出器问的是「有没有写宽度类」与「并排够不够两个」——
两个都是**看得见、数得出、而与结果无关**的性质;真正决定一个控件会不会撑破页面的,是
「它的**内在尺寸**由内容决定,而它坐在一个**不换行**的容器里」。**两条断言都为真,结论仍然是假的。**

> ### ☞ 处置,一句可执行的
> **拿一个【同一机制】的已知正例去试那个检出器,不要拿一个相似的。**
> 「相似」是指看起来像的那一类(另一个 flex 行 / 另一颗下拉);
> **「同一机制」是指它会经由【你正在赌的那条因果】而变的那一个**
> —— 这里就是「一个不换行的容器里、一个内在尺寸由内容决定的原生控件」。
> 检出器在这个正例上必须**响**;它不响,那个零就不是零,是瞎。
...
> ☞ 它与「委托书里的数来自上一份报告」是一对,但**治法不同**:那一条的解药是【重量一遍】,
> 而**这一条重量也治不了** —— 你会量出一个正确的 38,然后以为前一刀的 25 变坏了。
> **真正的解药是把分母一起抄走。**

**处置(两条):**

1. ★ **报告里写一个数,就把它的【总体】写在同一句话里** ——
   不是「25 颗装不下」,是「**22 条路由 / 65 颗首屏下拉里,25 颗装不下**」。
   ☞ 这与本文件「一个数写进报告时,把【它是怎么量出来的】和它写在一起」是同一条的下半句:
   **量法说的是"怎么数的",分母说的是"从哪儿数起的"。两个都要。**
2. ★★ **两个口径的数并排写,不要挑一个,也不要把后一个当成前一个的更正。**
   FONT-1 在 `docs/forward-queue.md` 那条下拉条目里就是这么记的:
   **「INPUT-3:22 条路由上的 25 颗」与「FONT-1:141 条路由 · 168 颗里的 38 颗(真被截 21)」并排**,
   而**两条都标着它们各自的分母**。
...
> **内容**(那颗按钮上写的字)与**结构**(它住在 `tbody tr` 里)才是它自己的。
> ☞ 与本文件「判据必须走【人真的走的那条路】」是邻居,而这一条更窄:
> **判据不但要看得见那个性质,那个性质还得是【不会被别的刀顺手改掉】的那一个。**

**处置:**

1. ★ **写一条判据时,问一句「它认的这个东西,归谁管?」** 归**样式**管的(颜色、字号、间距)
   迟早会被一刀改掉;归**内容**或**结构**管的稳得多。
2. ★★ **一刀如果要改一个【量具正在认的】东西,两件事必须在【同一个提交里】** ——
   不许「先施工,回头补量具」。**回头那一步是这个仓库反复丢掉的那一步。**
...
> ☞ 它与本文件「判据必须走【人真的走的那条路】」同族:那一条说判据要走真路,
> **这一条说【修法】也要走一次真路** —— 在**判它合法的那一刻**就走。

**处置(两条):**

1. ★ **一份勘察报告写「修法是 X」时,必须带上一次【X 用上去之后的读数】** ——
   哪怕只是在 DevTools 里注入一次。**没有读数的修法是一个猜测,不是一条建议。**
   ☞ 成本极低:round 3 把四种候选(`max-w-full` / `w-full` / 两个一起 / 父元素 `min-width:0`)
   **在同一棵 DOM 上逐个注入再量**,两条路由一共花了不到两分钟,而它**直接否掉了委托书首选的那一个**。
2. ★★ **修法之前先量【元凶的 min-content】。** 一个容器压不到它的 min-content 以下;
   而当那个 min-content 由**一个单独的、不可分割的子元素**决定时,
   **换行、`flex-shrink`、`min-w-0` 全部无效** —— 这是 INPUT-2b 那条教训的另一面。
...
1. ★★ **凡是「把某个样式值归一」的刀,收尾必须有一次【读渲染结果】的普查**,
   而不能只有一次「源码里还剩几处」的重扫。**源码扫描证明不了屏幕上的事。**
2. ★ **写这一类扫描器时,把 `className` 与内联 `style` 两条路【都扫】,而且两个数【分开报】** ——
   合成一个数会让下一个人不知道哪一条路上还有多少。
...
> ① 问的是「把这张表的**所有**字换掉会怎样」;② 问的是「把**第一张**表换掉会怎样」。
> ☞ **两次都不是漏了一行代码,是【判据的范围】与【它声称的范围】对不上** ——
> 与本文件「一个数【数的东西和它的名字对不上】」是同一条法则在**证明**上的投影。

**处置(三条):**

1. ★★ **一次撤销证明,撤的范围必须【逐字等于】这一刀改过的范围** —— 不多不少。
   多撤一分,它证的就是另一件事;少撤一分,它会把一次干净的增长报成"证不住"。
2. ★ **永远 `querySelectorAll`,永远遍历。** 「这一页只有一个」是一个**会过期的**假设。
3. ★★ **撤销之后要再撤销一次(unrevert),并核对读数回到【改后】那个值** ——
   一次留下残留的证明,会把它自己的残留算进下一条路由。
   ☞ 而这两次错**都是被【证明自己说"证不住"】暴露出来的**:
   **一个会说"我证不住"的证明,比一个总是说"通过"的强。**
...
> 而这个符号**不在那个整数里** —— 它在那句话的动词里,于是它**穿不过**「把委托书里的数重量一遍」那道闸:
> 你把 1 量出来了,1 是对的;**错的是 1 在哪一边。**

**处置(两条,都是开工前的):**

1. ★ **凡是「差 N 就会 X」这类措辞,重量的时候要同时报出【X 现在是真还是假】** ——
   不是只报那个 N。一个阈值读数必须带着它**在阈值的哪一侧**。
2. ★★ **一条停止条件,开工前先问一次「它今天是不是已经开着」。**
   一个**已经开着**的停止条件,和一个**快要开**的,需要的是两种完全不同的处置:
   前者要的是**一条裁定**(例外、或改判据),后者要的只是**小心**。
   ☞ 这一族此前的经验都在说后者(「留出余量」),而这一次是前者 —— **没有人问过那一句。**
...
> 三枚控件住在同一份源码里,而它们的样子由**各自那一行的 className** 决定 ——
...
1. ★ **按【渲染出来的那一个】清点,不按文件清点。** 一个组件里有几个同类控件,
   就要有几行读数 —— 哪怕它们在源码里挨着。
2. ★★ **「它们是同一批」这句话本身要被量一次。** 判据最便宜:**把每一处的 className 原样列出来**。
   ☞ 这一条与「一次勘察只看得见代码碰巧给它起的名字」是一对:
   那一条说**名字**会骗人,这一条说**容器**会骗人。

## ★★ 一份基线文档是一张【快照】,不是一道闸(INPUT-2b,2026-09-10)

**一份基线文档记的是一批【读活数据的页面】在某一天的读数。** 页面的整页宽度吃活数据 ——
...
1. ★ **停止条件的参照点 = 这一刀自己的改前读数**(同一天、同一支量具、同一棵树)。
   基线文档那一份**另报一行差值**,让它继续可读、可追,但不让它决定停不停手。
2. ★ **报告里那一行差值要说清它【是什么】**:是「基线那天到今天之间活数据变了」,
   还是「这一刀改的东西推动了它」。**两者在数字上长得一模一样,而只有前者不该停手。**
...
### ★★★ 而【写】那一半更坏:被 RLS 挡下的 UPDATE 不是错误,它是一次成功的空操作(ALERT-1,2026-09-08)

上面那一段说的是【读】。写这一半从来没有人写下来,而它咬得更深:

```
UPDATE materials SET deleted_at = now() WHERE id = $1;
→ rows_affected = 0 · 不抛异常 · error 为 null
```

**这不是数据库的怪癖,是 RLS 的定义。** UPDATE 策略上的 `USING` 决定
【哪些行看得见、改得动】,`WITH CHECK` 只对**通过了 USING 的行**校验新值。
本仓库八张受影响的表全部写成 `USING (p) WITH CHECK (p)` —— **两侧同一个谓词**。
...
* **客户端**:乐观更新的回滚判据写成【没确认落地就回滚】,不是【出错才回滚】。
  后者要求服务端永远记得报错;前者不要求任何人记得任何事。
* **动作层**:每一条 `.update()` 后面跟 `.select('id')`,**零行不许报告成功**。
  ALERT-1 的 `lib/action-refusal.ts` 只此一份实现。
* **数据库(还没做,已立案 `ALERT1-SILENT-WRITES-DB-SIDE`)**:让拒绝 RAISE。
  只要闸不在库里,「记得加 `.select()`」就是一条**忘掉也不会变红**的规矩。

☞ 同族既有条目:`BTN4-REMOVENODE-SILENT-NOOP`(`removeNode` 删零行仍返回成功)。
☞ **这一条与上面那段读的法则是同一条**:*RLS 不报错,它只是让语句什么都碰不到。*
   读的那一半给你 NULL,写的那一半给你 success —— **两边都长得像通过。**
...
## ★★ 判据必须走【人真的走的那条路】—— 这一族的第五例(CONFIRM-1,2026-09-06)

> ☞ **第六例是【判据看不见自己有没有在看】,它单独占一节** —— 见下面
...
> **一条判据必须【走人真的走的那条路】。**
> 「看得见,不是在不在」是必要的,**不是充分的** —— 一条用 `page.goto` 驱动的
> 可见性判据,对着这棵同样坏掉的树照样会绿。

**落地时的两条,都便宜:**
* **软导航就要【点】,不许 `goto`。** 点之前等【水合收尾】(`__reactFiber$…`),
  不然那次点击不会被 `<Link>` 接管 —— 它会悄悄变成一次硬导航,判据于是假绿。
* ★ **「我真的走了软导航吗」这件事本身也要证。** 导航前在 `window` 上盖一个记号,
  导航后它还在 = 文档没有重新加载。**没有这个记号,一次悄悄退化成硬导航的点击
  会让判据假绿 —— 那正是本探针要抓的缺陷,穿着探针自己的衣服回来。**

## ★★ 一次勘察只看得见【代码碰巧给它起的名字】(CONFIRM-1,2026-09-06)

**上一条与下一条是同一条法则的两面,所以它们挨着放:**

| | 症状 | 方向 |
|---|---|---|
| 注释提到了那个机制 | `grep -c window.confirm` 把 16 句散文数了进去 | **数大了**(40 → 56) |
| 组件没有自称是对话框 | 按文件名 `modal\|dialog\|overlay` + `role="dialog"` 搜,`ReasonPrompt` **一次都没出现** | **看不见**(8 个调用点整个消失) |
...
* 它渲染出一个**遮罩/展开层**吗 —— `fixed inset-0` · `{open && (` · `useState(false)` 配一个 `<button>` 触发;
* 它**挡在一个动作前面**吗 —— 一个 `confirmLabel` / `onConfirm` / `action(` 的 prop;
* 它**等一个人点下去**吗 —— 一个不由数据驱动、只由点击驱动的 `open` 状态。

按名字搜是免费的,所以先跑;**但一次声称"树里没有 X"的结论,
必须有一遍按行为的搜索垫在下面** —— 否则它说的只是"没有东西叫 X"。
...
**上一条是这条的反面,所以它们挨着放。** 上一条说的是"按名字搜会漏";
这一条说的是**有时候根本不必搜** —— 换一个枚举对象,漏不掉就成了结构性的。
...
> 问题是「**有没有**哪一处满足 P」时,先问 **P 的取值住在哪里**。
> 住在一份**枚举得完的东西**里(消息目录 · 枚举类型 · 路由表 · 权限清单 ·
> token 表 · 迁移目录),就**枚举那个空间**,再把命中的项反查回调用点。
> 只有当 P 真的只住在调用点里(布局 · 控制流 · 手写 className)时,
> 才回去数调用点 —— 那时才需要上面两条法则和第六张脸兜着。

★ **一次对着闭合集合的勘察不需要"我搜得够不够全"这个问句** ——
而那个问句正是本仓库前面每一次漏抓的入口。

## ★ 一句注释可以污染将来对它自己的计数(CONFIRM-1,2026-09-06)
...
* 注释里**不写那个字面 token** —— 写「原来走的是原生确认框,消息键 `xxx.deleteConfirm`」。
  说清楚是哪一处,而不留下一个会被数进去的词。
* **勘察一个机制时,先把【注释】与【调用点】分开数,并把两个数都报出来。**
  只报一个总数的勘察,读者无从知道它数的是代码还是散文。

☞ 这条注释污染的下游 —— **一个解析器同样会被代码的词汇误导,而它不会说自己漏了** ——
  已经提成本文件的一条独立法则:见「★★★ 覆盖率本身必须是一条断言」。
...
* **批量改写【不许用正则找调用点】** —— 用 AST。注释不进 AST,这是结构性的免疫,
  不是"小心一点"。ALERT-1 的计数脚本 `scripts/survey-native-dialogs.mjs` 走的正是 AST
  (所以它的数从一开始就是对的);**而它的改写脚本没有,于是只有改写那一半出了事。**
* **一个数要么带着它的量法,要么带着它的日期。** 「19 处」在 BTN-4 之前是对的。
  写下一个数的时候写清楚它是【什么时候、用什么口径】量的,下一刀才知道该不该重量。

## ★★★ 覆盖率本身必须是一条断言 —— 一个瞎掉的检查必须说「我瞎了」,不许说「干净」(CONFIRM-1,2026-09-06)

**这是判据失效那一族的第六张脸,而它与前五张【不同类】,所以它自己占一节。**

前五例都是「判据看不见它要证的那个性质」,或者「判据被指向了一个人永远不会
到达的状态」。这一例问的是更下面的一层:
...
| 世界 A | 世界 B |
|---|---|
| 解析器扫过 43 个调用点,一个坏的都没有 | 解析器**悄悄跳过了** 5 个调用点,剩下 43 个没问题 |
| `EXIT 0 — 每一处确认都点得出它在确认什么。` | `EXIT 0 — 每一处确认都点得出它在确认什么。` |

**两句话逐字相同。** 而世界 B 里,那 5 处可以是任何东西。
**一个瞎掉的检查不只是没用 —— 它还发一张「干净」的证明,
于是它比没有检查更坏:它替读者把怀疑花掉了。**
...
> **凡是「扫一遍树、抽出 N 处、逐处判」的检查,`N` 本身必须是一条断言,
> 不是一个印出来给人看的数。**
>
> 做法:**用一个与解析器【无关】的办法数一遍同样的东西,两个数对不上就自己红。**
> 这里是「非注释行里的 JSX 开标签数」,与抽取 `subject` 走的是两条独立路径。
>
> **故障注入必须包含【把解析器自己弄瞎】那一格。** 弄瞎之后它印的是
> `0 处 subject / 43 个 JSX 开标签` —— **它没有报「干净」,它报了「我瞎了」。
> 这就是这条断言存在的全部理由。**

### 它管的不只是这一个脚本

本仓库已经为这个形状记过好几笔,而此前每一笔都写在【它自己那个对象】上:
「一次失败不是一个空集」(`mustRows` / `restRows`)· 「解析出 0 个后缀的
resolver 必须失败」(`check-i18n`)· 「零必须是一次测量,不是一次缺席」
(`xmodule` 的 `check_xmodule_views()` 返回它检查过的数量,任一为零即失败)·
「`security_invoker` 拼作 `'on'` 或 `'true'`,只匹配一个就是检查了 15 张里的 1 张
而报告干净」。

**四条各自被学过一次,而它们是同一条。** 写在这里是为了让第五次不必再学:

> **给本仓库将来写的每一次普查、每一道棘轮、每一个自制解析器:
> 先问它数出来的那个 N 是不是一条断言。不是,就先把它变成断言,再谈判据。**

★ 并且顺带说清一件容易反着做的事:**这条法则【不是】"把 N 写进基线"。**
一张记着「上次是 43」的基线文件,在解析器瞎掉那天会安静地跟着变成 0 并被刷新。
**要的是同一次运行里的第二条独立路径**,不是上一次运行的记忆。
...
**处置:** 门槛改成 **一条都不许有**,并且【当场演示它能红】——
注入一个真的语法错,3 条诊断即退出 3 并点名文件。
☞ 一般化:**健康检查的阈值默认应当是零。** 要设一个非零的门槛,
   得先说出「为什么这个数量的坏是可以接受的」;说不出来,就说明那个门槛
   是为了让检查过关而设的,不是为了描述健康。

## ★★ 按行切开再匹配,会废掉一个含 `\n` 的字符类(BTN-1 · BTN-2 · BTN-3,三次)

**给这个陷阱一个名字,免得它继续被重新发现:**

> ### **SPLITTING BY LINE BEFORE MATCHING DEFEATS A CHARACTER CLASS THAT INCLUDES `\n`.**

`/<tag[\s>]/` 看起来完全正确 —— `\s` 当然包含换行。而 `.split('\n')` 已经把换行
**吃掉了**,于是多行开标签的那一行

```
        <button                     ← 这一行到此为止,后面一个字符都没有
            type="button"
```
...
**改法一律是 `(?=[\s>]|$)`** —— 行尾也算一个合法的边界。
**或者根本不要按行切**:对整份源码跑正则时,`\n` 真的是 `\s`,这个陷阱不存在
(BTN-3 实测:同一条 `/<button[\s>]/` 对整份源码给出 205,与正确写法一字不差)。
☞ 所以**这个陷阱住在 `.split('\n')` 里,不住在字符类里**。
一个"我把字符类写对了"的结论,在按行切的代码里仍然是错的。
...
> 它说的是下面两件事之一,而两件都要紧:
> **① 你注入的不是你以为的那个故障;② 这条断言本来就看不见这个性质。**
> 两件都必须查到底 —— 一格"注入了但没红"被当成"那大概不重要"放过去,
> 等于把这条断言的**唯一一次**体检结果丢掉。

**判据:每写一格注入,先说出【它应该红在哪一行】,再跑。**
跑完是绿的,那么在你修被测对象之前,先修那一格。
☞ 与上一节是一对:上一节说**覆盖率要写成断言**;这一节说**注入本身也要被核对** ——
一条从来没有真的红过的断言,和一条不存在的断言,退出码相同。

## ★★ 一个【同步的 process.exit】会掐死同一个信号上的异步清理(LEAK-1,2026-09-06)

**UI-1d 那一条(下面一节)说的是"清理要接到每一条出口上"。它做到了,而它仍然漏。
漏的原因不是"异步处理器不保证跑完"那么笼统 —— 是一个【每次都同一个赢家的竞争】。**

```js
// 模块层,先注册
for (const sig of ['SIGINT','SIGTERM']) process.on(sig, async () => { await cleanup(); … })
// main() 里,后注册(liveLock.acquireOrExit 内部)
...
> `{ ownExit: false }`,把退出交给 `scripts/ephemeral.mjs` 的 `installExitHooks`
> —— 它跑完清理、收子进程、放锁,才退。

★ **而这一层【第一版又写错了一次】,错法值得记:** `installExitHooks` 只放了锁、
**没收子进程**,实测 SIGTERM 之后 `next dev` 与 chrome 双双 `ppid=1` 活了下来。
**在信号这一处消灭了孤儿,同时在另一处造出两个** —— 与本文件记的 `run_detached`
那一课(上限处消灭一个孤儿、键盘上造出另一个)是同一个形状。
**收尾的顺序也是判据的一部分:先跑完清理的网络往返,再收子进程。**

## ★★ 一份【先于它要清的东西落盘】的清理计划(LEAK-1,2026-09-06)

**SIGKILL 捕获不到,所以任何"接到出口上"的清理都有一个它够不着的洞。**
补法不是再加一个信号,是**换一个不依赖本进程的凭据**:

1. 造出任何一次性东西**之前**,把删它的那几步同步写进 `.ephemeral/<pid>.json`;
2. 正常退出与可捕获的信号照计划清理,清干净了才删计划文件;
3. **SIGKILL / 断电:②跑不到,而①还在盘上** —— 下一支脚本开跑时照计划补删。

★ **顺序要写成【声明】,不能靠追加顺序。** 计划按"造出来的先后"追加,
而删必须按依赖的反序(评估 → 员工 → 授权 → 账号);按追加顺序执行会先删员工、
再删指着它的评估,外键当场拒绝 —— **于是清理"失败"了,而失败的那几步里就有那条 admin 授权。**

★ **SIGKILL 之后【当场】还剩什么,要照直说:** 线上那些行 + 一份盘上的计划,
它们不会自己消失。**这个设计没有消除那个窗口,只是缩短到"下一支脚本开跑"为止。**
不要把它写成"SIGKILL 也干净"。
...
> 这里的后果是「一条 user_roles 没被删掉」,而按后果搜只要一句
> `grep -c user_roles <每一支会造账号的脚本>` —— **有 INSERT 没有 DELETE 的那一支当场露头。**

★ 与 CONFIRM-1 那条「一次勘察只看得见代码碰巧给它起的名字」是一对:
那一条说**名字**会误导搜索,这一条说**立案的因果链**会误导搜索。
...
> 一支量具有**读取**、**归一**、**分桶**三层,而**每一层都可以单独瞎掉** ——
> 更坏的是:**读取那一层的成功会让人以为后面两层也成了。**
>
> **处置(三条,都很便宜):**
> ① 分桶的判据要写成**一个函数**,它的入参**就是读取层的全部输出** ——
>    读到了却没进参数表的字段,当场看得见;
> ② 每加一种新的写法(内联 style、`cn()`、变量),**去问一句「分桶那一层认不认它」**;
> ③ ★ **抽样手核的样本要按【写法】分层,不要只按【模块】分层** ——
>    按模块抽,一百个样本可能一个内联 style 都抽不到。
...
> ### ☞ 判词
> ★ **一个 class 不知道自己的主人是谁。**
> 「这一刀不碰 `table-style.ts`」是一条**关于文件**的裁定;
> 而一条 `.text-[15px]{…}` 是一条**关于屏幕上每一个戴着这个类的元素**的规则 ——
> **两者的边界不重合,而不重合的那一块正好是关掉的那一族。**
>
> **处置:**
> ① 注入 / 规则的选择器要把关掉的那一族**显式排除**
>    (`:not(table *):not(input):not(select):not(textarea):not(button):not(option)`),
>    而且**排除项要逐条写出来给人看**,不要藏在一个变量里;
> ② ★★ **加一条【越界自证】**:注入之下**逐条读**那几族元素的值,
>    它们必须**一个都没有动** —— FONT-2 第二版就是这么证的
>    (`<td>/<th>` 与每一颗控件的字号直方图,15px 那一档 1014 → **1014**);
> ③ **同一条教训对【落地】那一侧一样成立**:这一刀真正改的是**代码点**,
>    于是它必须再问一次「这个代码点最终渲染到哪里去」——
>    ★ FONT-2 实测到**两个方向**都会错:一个写在 `<td>` 的 `render:` 回调里的 `<p>`,
>    渲染进的其实是**确认对话框**;而一个写在普通调用点的 `<span>`,
>    渲染进的其实是**一个格子**。**写在哪里 ≠ 渲染到哪里。**
...
> 而那个数一旦被抄进队列条目,它的**层名就掉了**,读起来就成了一条对**这一刀**的预测。
> ☞ 与「一个数被抄走时,它的【分母】掉了」是同一条法则的另一张脸:
> **这一次掉的不是分母,是【它是在什么条件下量出来的】。**
>
> **处置:**
> ① 一个上界层量出来的数,**写进队列时必须把「上界」两个字一起写进去**,
>    并且写清楚它顶掉了什么(「用 `!important` 连元素自己的字号类一起顶掉」);
> ② ★ **下一刀读到一条点名的预测时,先花两分钟在同一棵 DOM 上注入【自己这一刀真的要下的那条规则】再量一次**
>    —— FONT-2 就是这么发现它不成立的,代价是两分钟;
> ③ 与 FONT-1 那条「一次『有合法修法』的判断,和一次真的去量那个修法有没有效果,是两件事」并排读:
>    **两条都是同一句话 —— 一条没有被这一刀自己量过的断言,不是这一刀的读数。**

---

## ★★★ 一条【承重的契约只写在注释里】—— 换掉它描述的那个东西,没有任何一道闸会红(BUGFIX-1b,2026-09-12)

`lib/action-refusal.ts` 的 `refuseFromCoded` 靠一条契约分界,而那条契约**逐字写在注释里**:
...
1. ★ **动一族共享行为之前,先 `git grep` 那族行为的【名字】与它的【判据形状】** ——
   这里是 `!== raw` / `=== raw`,四处命中、两处承重。**判据形状比名字好搜**:
   名字会变,`!== <入参>` 这个形状不会。
2. ★★ **改完之后,把那一族的注释【读一遍】,并且问每一句:它还是真的吗?**
   本刀因此改了 **59 处**注释,其中 **6 处**不是"过期",是**它的论证反过来指着新代码**
   (例:「一句看不懂的英文比一句编出来的中文强」—— 那句话在只有两个选项时是对的,
   而这一刀造出了第三个)。☞ **一条论证过期,比一个数字过期更难发现:数字会被重量,
   论证会被照着做。**
3. ★ **把契约从注释里【搬出来】,哪怕只搬一半。** 本刀的搬法是给共用兜底多导出一支
   `fallbackTextFor(raw)`,让 `refuseFromCoded` **拿同一串再问一次**「走兜底会说什么」——
   同输入同语言 ⇒ 同输出,于是那个比对是**精确的**,不是启发式的。
   ⚠ **它仍然没有闸**(登记在 `docs/known-issues.md` 的 `BUGFIX1B-REFUSE-FROM-CODED-CONTRACT`)——
   ★ **而本刀【故意】没有现写一道**:一个刚被自己绊倒的人写出来的闸,没有人验过。
...
**处置(已落地):** `app/globals.css` 用 `@import "tailwindcss" source(none)` + 逐个 `@source`
把扫描源限死在 `app/` 与 `lib/`;`scripts/check-generated-css.mjs` 进 `npm run build`,
**把生成 CSS 的解析 warning 当失败**。★ **顺带一条要记住的维护规则:以后新开一个会写类名的
顶层目录,必须在那里加一行 `@source`** —— 漏了它,那个目录的类一条都不会生成,而页面会安静地少掉样式。
...
**它为什么特别难发现:** cast 写在**查询这一侧**,而错在**列名那一侧**;两者在同一行里,
而读代码的人会把那句 cast 读成"这里的类型太麻烦",不会读成"这里有一道检查被关掉了"。
**处置:一句 cast 要么带一行写明它关掉了什么、为什么可以关,要么就不写。**
★ 本刀顺带加了一道**不依赖类型**的闸(`scripts/check-select-columns.mjs`,读源码文本),
理由正是:**再有人加一句 cast,`tsc` 会再次失明,而那支闸不会。**
...
配了一支专门去扒函数体字面量的解析器,就在隔壁一行。** 同一种形状,一个接对了源,一个没有。
**处置:给一个动态前缀接源之前,先问「这个前缀的值是谁写出来的」,再去那个地方读。**
...
> ☞ 处置:显式 `optimize: false`，让这支闸**与 `NODE_ENV` 无关** ——
> 一支读数随环境变的闸,本身就不是一把尺。

**给下一支闸的三句话:**

1. ★ **写完一支闸,先问它「在 `NODE_ENV=production` 下还是同一支吗」** —— 本机跑一遍
   `NODE_ENV=production node scripts/<它>.mjs`,**两次的读数应当逐字相同**;
2. ★★ **故障注入要在【两种环境】各跑一遍。** 只在本机注入过的闸,
   只证明了它在本机会咬人;
...
> 因为 shell 会替你跑掉你写进字符串里的东西。**

**处置(可执行的两条):** ① 长量具在跑的时候,别的命令先问一句「它会不会碰 `.next`、库或锁」;
② **含反引号的文本不要经 `-c` 传给解释器** —— 写进文件再执行,或者用单引号的 heredoc。
★ 本仓库已经为「引号 / 转义把命令变成了别的东西」付过账(`check-i18n` 的正则、
Tailwind 的字面量类名),这是同一族在 **shell** 上的那一张脸。
...
## ★★ 一支扫描器的【切词】那一层,可以和它的【读取】那一层不一样瞎(FONT-3,2026-09-12)
...
> 「按空白切」这句话在一个**字面量 class 串**上是对的,在一个**模板表达式**上就不是 ——
> 而同一支扫描器同时喂着这两种输入,**它自己说不出自己刚才喂的是哪一种。**
>
> **处置(三条):**
> ① ★ **两种切法都跑一遍,并排报**(宽切:把 `'` `` ` `` `+` `{` `}` `(` `)` 也当分隔符)。
>    **两个数不一样,就是有一种写法你没想到** —— 差集直接就是要读的那几行。
> ② ★ **拿一个已知的正例去试切词那一层**,和 INPUT-2b 那条同一个办法:
>    找一个你**确定**含那个 token 的模板表达式,切词必须切得出来。
> ③ ★★ **落地不要按【行号】去对两支量具的名单** —— JSX 开标签的行号与 token 自己的
...
> ☞ **处置(两条):**
> ① ★ **报这一类计数时,把「其中有几格不渲染文字」一起报出来** ——
>    不带这一行,「57 → 5」读起来像「还差 5 张没修」,而实际是「4 张差在看不见的地方、
>    1 张差在这一刀没有裁的属性上」。
> ② ★ **然后【还是把它们改掉】,而且理由不是好看:一条规则不该留五个例外。**
>    FONT-3 给那四个空格子也发了同一个字号 token —— 屏幕上零变化,而**规则从此没有例外**,
>    下一个人不必再判一次「这一格算不算」。
...
> **它当时的 `font-variant-numeric`**。于是证明的匹配写成:
> 「**own text 在改前的等宽集合里**」,按回去的值**逐元素取自那一条改前读数**。
> ★ **而这条判据必须自证两件事,否则它就是一次挑选:**
> ① ★ **匹配到的个数要与改前读数里的个数并排报出来**(实测 7/7 · 45/45 · 12/12);
> ② ★ **按回去之后,页面上渲染成等宽的元素个数必须从 0 变回那个数**(实测 0→7 · 0→45 · 0→12)
>    —— 这一条证明那次注入**真的生效了**,而不是一次静悄悄的空操作。
> ☞ 这是 S5「一个渲染文字都没改」换来的东西:**文字没变,所以文字可以当身份。**
> 一刀如果同时改了文字,这条路就断了 —— **那时要先留一份改前的元素指纹,而不是事后去找。**
...
> 而这三个限定词在报告里最容易掉的就是**视口** —— 因为它读起来不像一个分母,
> 像一个"我在哪台机器上跑的"。
> ☞ 与「一个数被抄走时,它的【分母】掉了」是同一条法则:**这一次掉的是视口。**
>
> **处置(两条):**
> ① 一份按视口取样的读数,**报数的时候把视口写进那句话里**
>    (「phone 首屏 694 个里 26 个不过」,不要写「694 个里 26 个不过」);
> ② ★ **凡是判据里出现 `hidden md:` / `md:hidden` / `sm:` 这一族响应式类的页面,
>    单视口的读数按构造是【半份】** —— 要么两个视口都量,要么在报告里点名说"另一半没量"。
