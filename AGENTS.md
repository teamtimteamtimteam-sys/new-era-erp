...
### ★★ 一道会漏抓而不吭声的闸,比没有闸更坏 —— 它还发一张「干净」的证明(CONFIRM-1,2026-09-06)

**上面那条说的是【一次人工勘察】会被代码的词汇误导。这一条是它的下游:
【一个解析器】同样会,而解析器不会说自己漏了。**

`scripts/check-confirm-subject.mjs` 头一版跑出 **43 处**,`grep -c '<ConfirmButton'`
数出 **48 处**。差额查明是注释(正是上面那条),**代码确实是干净的**。
可要紧的不是这一次的结论,是**当时没有任何东西能证明它**:

> **一个解析器悄悄跳过五个调用点,和它抓到了五个坏调用点而代码本来就干净,
> 输出长得一模一样 —— 都是 `EXIT 0`。**

本仓库已经为这个形状记过好几笔("一次失败不是一个空集"、"解析出 0 个后缀的
resolver 必须失败"、"零必须是一次测量,不是一次缺席")。**这一条是同一条法则
落在【自制解析器】上**:凡是"扫一遍树、抽出 N 处、逐处判"的检查,
`N` 本身必须是一条断言,而不是一个报出来给人看的数。

**做法便宜:用一个【与解析器无关的】办法数一遍同样的东西,两个数对不上就自己红。**
这里是"非注释行里的 JSX 开标签数",与抽取主语走的是两条独立的路径。
**故障注入必须包括把解析器自己弄瞎那一格** —— 弄瞎之后它印的是
