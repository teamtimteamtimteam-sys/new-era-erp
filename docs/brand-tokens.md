...
> ### 删除清单(Tim 挑完之后)
> ★ **先读 `docs/variant-c-spec.md`(STYLE-1,2026-09-09)。** 那份文档把 variant C
> 从这一页的代码里【实测】成了一组带出处的数值(两个视口、逐元素的
> `getComputedStyle`)。**在它写下来之前,执行下面第 1 步会连同删掉 C 唯一一份
> 精确副本;现在不会了。** 也请顺带读它的 §6 —— 取样页**没有**覆盖的那些元素类型
> (文件上传、日期框、数字框、原生下拉、勾选框)**至今没有标准**,删掉这一页
> 不会让它们有,而重画一次取样是唯一的补法。
>
