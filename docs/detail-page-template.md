...
## ⑭-3 `/finance/invoices/[id]` —— 委托点名的完成判据,**+133px / 2 裁 → 0 / 0**

**这一页是两件不同的事,而把它们说成一件就是撒谎(§⑭-0 Q3):**

| | |
|---|---|
| **量得到的那一半** | 页面自己的两张表 + 标题右边那一排不换行的按钮。**这是那 133px。** |
| **可编辑的那一半** | `CreateCreditNoteControl` —— 6 列、`<select>` 与 `<input type=number>` 都在 `<td>` 里,货真价实的 B 类全行可编辑网格。**但它收在 `useState(false)` 后面**(`:40`),探针从不点开它,**所以它一个像素都不在那 133px 里。** |

**本刀做了量得到的那一半,并把可编辑那一半留给下一刀 —— 说清楚,不含糊。**

**做了什么:**
1. 明细行表(6 列)→ `InvoiceLinesTable.tsx`。手机留**摘要 · 金额**;
   行号/数量/单价/**AR 链接**进展开区。**AR 链接进展开区是刻意的**:
   它是每行一个的钻取入口,不是这一行的身份 —— 而它正是探针点名的
   `a.text-blue-600`,那个把整页顶宽的元素。
2. 结算表 → **直接用 CONV-9 的 `SettlementHistoryTable`,一个字都没新写。**
3. 外壳 → `ListPage`;标题右边那三个按钮进 `actions`,**而 `actions` 会换行**
   —— 转换前那一排是 `flex` 不换行,标题 + 三个按钮在 390px 上必然顶宽。

    实测 +133px / 2 张表被裁  →  **0 / 0**   ✅ 委托的完成判据达成

### ★★ 而这里有一条比这一页更要紧的发现:那张表**收敛对了** ★★

`SettlementHistoryTable` 是 CONV-9 在 **3 页**上收敛的(§⑫-4:AP · AR · 开支)。
这一页是**第 4 个调用点**,而它的列、i18n 键、连 `empty={t('finance.noOpenItems')}`
**都逐字相同** —— 本刀一个字都不用改那个组件。

> **它是在【不知道有这一页】的情况下被收敛的,而第四页原样装得进去 ——
> 那正是「这不是两张相似的表,是同一张」当初判对了的证据。**
> ☞ 顺带验证 CONV-5 §⑩-5 那条计数规矩:本刀这一页转了 **2 张表**,
> 而闸只 **+1**(121 → 122)—— 因为结算那一张【不是新调用点】。
> **闸数的是调用点,不是渲染次数。**

## ⑭-4 `/logistics/containers/[id]` —— +90px → 0 / 0,**而它是旧尺子藏起来的那一张**

**这一页此前【就在那 9 条 `textLen=76` 的 404 名单里】(§⑬-1e)。**
也就是说:**它一直是坏的,而 CONV-8 与 CONV-9 两刀都把它记成「可用」。**
修好尺子的全部回报,这一张就是。

**元凶不是表,也不是那个被点名的按钮 —— 是它旁边那个 `<select>`:**
挂运单那一行写的是 `flex items-end gap-2`,**没有 `flex-wrap`**;
而 `<select>` 按**最宽的那个 `<option>`** 定宽,option 的文本是
「单号 · 订单号 · 客户名」三段拼起来的。**客户名一长,整页被它顶出去,
而被挤出屏幕的是旁边那个 `bg-blue-600` 按钮 —— 探针点名的是【被挤的那个】,
不是【撑宽的那个】。**

> ☞ **这是读探针的一条规矩,值得写下来:`culprit` 是【溢出边界上的那个元素】,
> 不一定是【造成溢出的那个元素】。** CONV-9 §⑫-5a 说的是"元凶可能在表里";
> 这一条是它的下一层:**元凶可能是被点名者的【邻居】。**

    fix: flex → flex flex-wrap · select 加 max-w-full min-w-0
    实测 +90px / 0  →  **0 / 0**

**没做完的那一半,说清楚:** `ContainerPanels.tsx` 的单证表
(`:274` `<select>` 与 `:280` `<input>` **都在 `<td>` 里**)是**真的 B 类可编辑网格**,
CONV-8 把这一页归进可编辑七张是对的。**本刀没有转它** ——
那是一次带服务端动作的网格重写,做半截比留着更坏。**它留在 §⑭-6 的清单里。**

## ⑭-5 ★★★ 第四层盲区,而它一次能毁掉整跑:**跑过 `npm run build` 之后,每一条动态路由都 404** ★★★

**这是本刀最要紧的一条,而它是被 §⑬-1 那条新断言【当场抓住】的。**

同一棵树、同一批 id、相隔十分钟,两跑:

| | 404 条数 | 真的量到 | 旧口径会说 |
|---|---:|---:|---|
| `.next` 里有生产构建 | **16 / 16** | 0 | **16 / 16 USABLE** |
| `rm -rf .next` 之后 | **0** | **20 / 20** | 20 / 20 USABLE |

**每一条 404 页都 `ovf=0 clip=0`** —— 一张 404 既不溢出也没有表可裁,
**所以旧口径把整跑记成满分。**

> ★★【为什么这一条比前三层都毒】★★
> **「先跑 `npm run build` 确认门是绿的,再跑探针量手机」—— 这是任何人都会
> 采用的顺序,而那个顺序【恰好】制造出一份全绿的假数据。**
> 前三层(表没画出来 / 拿不到 id / 取到页面不收的 id)都只影响【几条】路由;
> 这一条影响【全部】,而且它由一个"我很谨慎"的动作触发。

**机制:** `next build` 写 `.next/BUILD_ID`;探针随后起的 `next dev` 在这份
生产产物上面跑,动态路由全部落进 `notFound()`。**`next dev` 自己【不写】
`BUILD_ID`(它只写 `.next/dev/`)—— 实测两遍,所以这是一条结构性的、不会漂的判据。**

**处置:探针开跑前检查 `.next/BUILD_ID`,在就【当场炸】并给出那一行命令。**
不自动删:一个悄悄删掉别人构建产物的探针,下一次会在别的地方被诅咒;
本仓库一贯是「响,而不是替人做决定」。

    故障注入:touch .next/BUILD_ID → EXIT 1,点名文件与补救命令
              rm .next/BUILD_ID   → EXIT 0,1/1 真的量到

> ☞ **对本刀自己那份数的影响:没有。** §⑬-1d 那份「CONV-9 的 19 张」基线
> 是在本刀**第一次 `npm run build` 之【前】**跑的(日志时序可查),
> 所以它是干净的。**而 CONV-9 自己那份数的顺序不可考** ——
> 它的 commit 只说三个门都绿,没说顺序。**这是 §⑭-7 要人确认的第一条。**

## ⑭-6 本刀之后还剩什么(按 CONV-8 的格式)

**16 张里做了 4 张,剩 11 张 + 1 张半。**

**只读,剩 6 张:**

| 页 | 表 | 备注 |
|---|---:|---|
| `/sales/customers/[id]` | 4 | 1 张在页里,3 张在 `StatementPanel`/`ChasePanel`/`ContactsPanel`。**已核实三张表的输入全在表【下面】的表单里,留 `DataTable`**(CONV-9 的分类站得住)。☞ `ContactsPanel` **被 `/suppliers/[id]/edit` 共用** —— 又一个「修一处好两页」 |
| `/logistics/forwarders/[id]` | 2 | 实测 0/0 |
| `/output/[id]/assays/[assayId]` | 2 | ★ 探针**测不到**(线上没有产出父的化验行,冒烟同样 SKIP)· 用得上 `padding` 槽 |
| `/inbound/[id]/assays/[assayId]` | 1 | 实测 0/0 · 用得上 `padding` 槽 |
| `/stocktakes/[id]` | 1 | 实测 0/0 · `CountList` **不是表**,不要硬套 |
| `/sales/orders/[id]` | 1 | 实测 0/0 |

**可编辑,剩 5 张 + 半张:**
`/logistics/containers/[id]` 的单证网格(**外壳与溢出已修,网格未转 —— 这是那半张**)·
`/hr/attendance/[id]`(探针测不到)· `/hr/reviews/[id]`(测不到)· `/my-reviews/[id]`(测不到)·
`/sales/quotes/[id]`(实测 0/0)· `/settings/roles/[id]`(实测 0/0,`PermissionMatrix` 是 B 类)·
**`/finance/invoices/[id]` 的 `CreateCreditNoteControl`(那半张,见 §⑭-3)**

**`/inbound/receive/done/[id]`:刻意未转,实测仍然 0 溢出 / 0 裁切。** ✅

**分组/小计能力:本刀新增 0 处,总数仍是 5 处 / 2 种形状。没建。**

## ⑮-0 ★ PRE-ACCOUNT-1(2026-09-04)开工那一轮提问,连问带荐,**以及 Tim 的回答** ★

§⑬-0a 记下 CONV-9 那一轮没有留下记录,§⑭-0 因此把 CONV-10 自己那一轮写了下来
(而 Tim 没有回)。**本刀这一轮 Tim 回了,所以这里连问带答一起记** ——
下一刀据此分得清「这是拍过板的」与「这是那一刀自己决定的」。

**★ 本刀不是转换刀。** 剩下的 12 张详情页按 Tim 的裁定**推迟到账号发放之后**,
它们今天量 0/0,转换买的是齐整,不是修复。**本刀一张页面都没有转换。**

| # | 问题 | 本刀的推荐 | **Tim 的裁定** |
|---|---|---|---|
| **Q1** | 两条幽灵授权:只删行,还是连产地一起修? | 先修产地(`survey-phone.mjs` 的 `cleanup()`),再删行 —— 而 Step 3 本身要反复跑那支探针 | **照推荐** |
| **Q2** | 线上四个**活着的** `survey-*` admin 账号算不算本刀的范围? | 算,同一次操作删掉,并把 `check-scratch-rows.mjs` 扩到看得见这一类 | **照推荐** |
| **Q3** | 风险方向与委托说的**相反**:幽灵授权谁也登不进来,而活着的一次性 admin 是一把**能用的钥匙**(密码写在仓库里、项目托管) | 四个活账号才是发放前的真暴露,两条幽灵是记账缺陷;两件都做,按这个顺序报 | **照推荐,并定为本刀的头条**。另:`guard_last_admin` 认幽灵这件事**记进文档,本刀不修** |
| **Q4** | Step 2 建闸、做普查、还是棘轮? | **棘轮** —— 实测 66 个文件手搓 `<table>`,闸会当场对着 66 个文件红;而普查拦不住 cut C 的新债 | **照推荐。**并追加一条:**基线只会缩短这条性质,要写进基线文件自己的注释里**,免得下一刀悄悄把它改大 |
| **Q5** | 规矩 (b)「库里缺能力先加进库」机器查得了吗? | 查不了,它没有机械特征;只建 (a) 的闸,(b) 写进 `docs/base-components.md` | **照推荐,并要求显式标注 UNENFORCED** —— 不许建假装在查它的机械 |
| **Q6** | `/my-reviews/[id]` 探针**根本量不到**(admin 是 404 契约),要不要给探针加第二个会话? | 加(Option A),但这是本刀的体量风险 | **加。**并把 Step 3 砍掉一半 —— 见 Q10 |
| **Q7** | 怎么种数据? | **本仓库没有"持久化种子"这回事**:`db/fixtures` 约 170 支**全部以 ROLLBACK 结尾**,冒烟造的评估在 `finally` 里删掉 | **照推荐。**委托那句「照这个仓库既有的方式种」**是错的**,裁定为:**只用即抛** —— 走产品 RPC 种、量、删,前台做,报破窗时长 |
| **Q8** | 种数据会踩到冒烟里那条**故意埋的**跳过断言(「开出第一份的那天,这条断言会逼人把它删掉」) | 只用即抛就不会触发;**不要**删那几行 | **照推荐** —— 那几行记的仍是真事实:线上还没有真底稿 |
| **Q9** | `/hr/attendance/[id]`:`open_attendance_period` 会给**每一个在册员工**铺一行(今天 21 行),等于宣称"八月考勤已经上账" | 可接受但要说明;也可以不做 | **不做。**为一份几分钟后就要删掉的记录写 21 行考勤断言,不值得。**记为「没有真关账就量不了」并推迟** |
| **Q10** | Step 3 比它写下来的体量大得多(三个子系统的种子 + 探针第二种会话 + 父子化验对) | 砍成六个人真会打开的那两张 | **砍。**`/my-reviews/[id]` 与 `/hr/reviews/[id]` 留下;`/hr/attendance/[id]` 与 `/output/[id]/assays/[assayId]` **与那 12 张一起推迟** |
| **Q11** | 「只修量红的」与「不许把页面转到模板上」可能是同一处改动 | 用最窄的改动就地修(一个 `flex-wrap`、一个 `min-w-0`);只能靠转换才修得好就停下来报告 | **照推荐** |

## ⑮-1 ★★ 委托的一条前提被**证伪**了:那不是缺数据,是**缺身份** ★★

委托把四张页面一起记成「库里没有行可解析 —— 这是 DATA 缺口,不是探针缺陷」。
**对其中一张是错的,而且喂多少数据都修不好它。**

`app/my-reviews/[id]/page.tsx:39-40`:
...
探针用的是**一个没有 `employees` 行的 admin**,于是 `current_user_employee()` 是 NULL,
`me` 为空,**这一页无论库里有没有那份评估都 404**。
冒烟自己早就把这句写下来过(`smoke-routes.mjs:1656`「`/my-reviews/[id]` 对 admin 是 404 契约」)。

**这不是推论,是量出来的 ——** 本刀把评估人会话摘掉重跑了一次:

```
注入:REVIEWER_ROUTES = new Set([])        ← 只把会话拿掉,数据照样种、id 照样解析
  [1/1] /my-reviews/[id]   ovf=0 clip=0    ← 【假绿的那个形状】
  NOT FOUND (HTTP 404): /my-reviews/[id] ← /my-reviews/09213525-…
  measured: 0   not-found (excluded): 1    → EXIT 1
还原 → shasum 逐字节相同(8cfc6506…)
```

> **一张 404 既不溢出、也没有表可裁 —— 所以 `ovf=0 clip=0`。**
> CONV-10 §⑭-5 量到的是同一个形状(构建产物导致的整跑假绿);
> 这一条是它的另一面:**身份不对也会给出同一份满分。**
> 两者的教训合成一句:**`ovf=0 clip=0` 只有配上 HTTP 200 才是一个结果。**

## ⑮-2 探针学会了第二种身份 —— 形状取自冒烟,**用完即抛**

`survey-phone.mjs` 现在:建评估人账号(**不授任何角色** —— RLS 的 `select as reviewer`
只看 `reviewer_employee_id`,这同时验证了"没有角色的经理也读得到自己的评估任务")、
两行临时员工、走**产品自己的**`open_probation_review` + `set_review_reviewer` 造那份评估,
量完在 `cleanup()` 里按 **评估 → 员工 → 账号** 的外键顺序删掉。

* **`REVIEWER_ROUTES`** 决定哪些路由换会话,`newTab(cookie)` 负责切;
* **每条记录带上 `session` 字段** —— 一份量到的数据必须说得出它是**以谁的身份**量的,
  否则下一刀分不清 404 是页面坏了还是身份不对;
* 员工行用 **`ZZ-SMOKE-` 前缀**(不是自己发明一个):`sweepScratch` 与
  `check-scratch-rows` **两支都兜得住**;两支脚本共用 `liveLock`,不可能同时跑,
  所以不存在被扫掉的风险 —— 那正是**账号**用 `survey-` 前缀的理由。

### ★ 一条量具自己的教训:**名字太长会把被测对象撑变形**

第一版抄了冒烟那个长名字(`【SMOKE 冒烟脚本临时行 · 勿动 · 随时可删】`),
`/hr/reviews/[id]` 量出 **+240px**。**那 240 里有 203 是探针自己的名字撑出来的** ——
那一页的评估人下拉框按最宽的一个 `<option>`(「工号 · 姓名」)定宽。
~~线上真实数据最宽一条是~~ 线上**非 `ZZ-` 那一批**最宽一条是 **27 个字符**(`EMP-2026-0001 · Choo Er Teh`),
★ **【全部线上数据都是测试数据 —— Tim,2026-09-20】** —— **那一行也是测试数据**,它与 `ZZ-` 那行的区别不是「真 / 假」,
而是**两种测试数据**。☞ 这一段的量法与结论**一个字都不受影响**(它量的是字符宽度),
改的只是**别把它读成「真实数据 vs 探针」那条线**。
而那个长名字是它的两倍还多。

    长名字   → +240px
    真实长度 → **+37px**   ← 这才是那一页真正的缺陷

> **一个把被测对象撑变形的量具,量到的是它自己。**
> 名字改成真实长度(`ZZ Survey 1`);「这是临时行」由 `ZZ-SMOKE-` 前缀负责说 ——
> 两支清扫认的都是前缀,不是名字。

## ⑮-3 四张里量了两张:**一张修到 0/0,一张本来就干净**

探针**跑在 `npm run build` 之前**(每一跑都先确认 `.next/BUILD_ID` 不存在,§⑭-5 那道拦)。

| 路由 | 会话 | HTTP | 表数 | 溢出 | 裁切 | 处置 |
|---|---|---:|---:|---:|---:|---|
| `/hr/reviews/[id]` | admin | **200** | 0 | **+37 → 0** | 0 | **修了**(共用组件,见下) |
| `/my-reviews/[id]` | **reviewer** | **200** | 0 | **0** | 0 | **量到就是干净的,一个字没动** |
| `/hr/reviews/cycles` | admin | **200** | 0 | **0** | 0 | 共用组件的第二页,复核没被弄坏 |

**推迟的两张,理由分开写:**

* `/hr/attendance/[id]` —— **没有真关账就量不了**。`attendance_periods` 只由
  `open_attendance_period` 写入,而它会给**每一个在册员工**铺一行(今天 21 行)。
  Tim 裁定不为一份几分钟后就删掉的记录宣称"八月考勤已上账"。
* `/output/[id]/assays/[assayId]` —— 需要**产出父 + 化验子**一对。
  `assay_results` 今天 4 行**全部挂在进料批上**,一行都没有 `output_batch_id`;
  冒烟同样 SKIP 它(`PROC-1b`:机制与屏幕先于第一张真单据落地)。

### 修的那一处:`SetReviewerControl` —— **一处改动修好两页**

元凶**不是**被点名的那个元素。探针点的是 `span.text-xs text-amber-700`
(那句"为什么按不动"),而撑宽的是它旁边的 `<select>`:
那一行写的是 `inline-flex items-center gap-2`,**没有 `flex-wrap`**,
而 `<select>` 按最宽的 `<option>`(「工号 · 姓名」)定宽。

> ☞ **这是 CONV-10 §⑭-4 那条规矩的第二例,足以把它当成一条常规:**
> **`culprit` 是【溢出边界上的那个元素】,不一定是【造成溢出的那个元素】。**
> `/logistics/containers/[id]` 是 select 撑宽、按钮被点名;
> 这一处是 select 撑宽、琥珀字被点名。**两次都要读邻居。**

```
fix: inline-flex → inline-flex flex-wrap … max-w-full
     select 加 max-w-full min-w-0
实测 +37px / 0  →  0 / 0
```

**而它是【共用的】:** `SetReviewerControl` 同时挂在 `/hr/reviews/[id]` 与
`/hr/reviews/cycles` 上,所以这一处改动一次修好两页 —— 已把 `cycles` 一并量过,0/0。
(与 CONV-10 那条「先找共用的属主再逐页改」同一条。)

> ⚠ **本刀在这里犯过一次错,记下来:** 那段说明文字第一版写成 JSX 注释
> `{/* … */}` 放在 `return (` 的**第一行**,于是 `return` 有了两个根节点 ——
> 三条路由全部 **HTTP 500**,而探针如实报了 `EXIT 1`。
> **注释挪到 `return` 外面**之后恢复。
> ☞ 值得记的是探针的表现:它**没有**把 500 记成"这页没有溢出",
> 而是退出分母 + 整跑变红 —— CONV-10 那道断言在一次真事故上兑现了。

## ⑮-4 本刀之后还剩什么

**详情页:12 张推迟的 + 本刀新推迟的 2 张 = 14 张。** 一张都没转换,与 Tim 的裁定一致。

* 只读 6 张(§⑭-6 原表)+ `/output/[id]/assays/[assayId]`(**需要一对父子化验行**);
* 可编辑 5 张半(§⑭-6 原表)+ `/hr/attendance/[id]`(**需要一次真的月度关账**);
* `ContactsPanel` / `/suppliers/[id]/edit` 的共用属主债,随那批一起走;
* **`th`/`td` 数不一致仍是 2 处**(本刀没有新增,也没有去猎第三处 —— 第三处出现时值得一道独立的闸);
* 分组/小计:仍是 **5 处 / 2 种形状**,没建。

## ⑮-5 本刀发现、**没有修**的三处(逐条说清楚为什么)

1. **`guard_last_admin` 认幽灵。** `db/tables/user_roles.sql:43` 只问"还有没有另一条
   未撤销的 is_system 授权",不问"那条授权背后有没有人" —— 一条幽灵授权因此
   **顶得了一个真管理员**。修法要跨 schema 读 `auth.users`,不是一行改动。
   记在 `docs/known-issues.md` GHOST-GRANTS 第四节与 `manual-walk-list.md` §32.2。

2. **★ 冒烟【没有】那道 `BUILD_ID` 拦,而探针有。**
   CONV-10 §⑭-5 量到:`.next` 里有生产构建时,`next dev` 会把每一条动态路由
   丢进 `notFound()`。它给**探针**加了开跑前的检查,**冒烟一个字都没加** ——
   而冒烟同样起 `next dev`。
   **方向上冒烟比探针安全**(它断言 HTTP 200,所以会**响亮地红**,不会假绿),
   但那意味着「build 之后跑冒烟」会浪费一整跑并给出一个误导的失败原因。
   **本刀自己就撞上了这个顺序**,处置是跑冒烟前 `rm -rf .next`(本刀用的就是这个顺序)。
   **没有顺手加那道拦**:改冒烟是改本仓库最重的一支脚本,而本刀的闸已经在跑
   —— 留给下一刀,判据与探针那一条逐字相同。

3. **一次性口令写在仓库正文里。** `survey-pass-1` / `survey-pass-2` /
   `smoke-pass-1..3` / `pdf-pass-1`。今天这些账号都是用完即删的(三支脚本的清理
   现在都会记账并让整跑变红),但**口令本身仍然是明文、在版本库里**。
   六个人拿到账号之后值得改成从环境变量读。记在 `manual-walk-list.md` §32.1。

> ☞ **另有一处措辞不准,一并记下不改:**`check-scratch-rows.mjs` 里
> 「2 小时以内」那一桶的收尾话是「下一次冒烟开跑时的 sweepScratch 会带走它自己那几张表的行」——
> 这句话**对幽灵授权与活着的一次性 admin 都不成立**(sweepScratch 看不见前者,
> 前缀也不匹配后者)。它是 CLEANUP-A 加幽灵检查时就带着的,本刀新增那一段沿用了同一个桶。
> **不在本刀改**:那是一句提示文字,而改它要连带重排两个桶的语义。
