...
**修法是一行:把那个 `@media` 块删掉,或者做真正的深色模式。**
`shadcn init` 会重写这个文件,所以**这条会被 ①a 顺手修掉** —— 但在那之前它是活的。
...
| 10 | **深色模式冲突** | **1**(`app/globals.css`) | **机械** | **低** | 删掉默认 `@media` 块,或做真深色模式。`shadcn init` 会覆盖这个文件。 |
...
| STEP 2 · e 手机 | ✅ PART C/e,含深色模式 live bug |
