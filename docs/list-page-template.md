...
>
> ★★ **CONV-9(2026-09-04)转了其中 19 张**,并给本篇的 `ListPage` 加了第二个槽
> **`padding`**(见 §⑩-15 第 3 条那个被划掉的条目)。调用点 89 → **111**。
> 它还重演了一次 CONV-8 那个「被自己的注释骗过去」的注入,**确认那个洞今天仍然补着**;
> 以及量到探针的**第三层盲区**:**一张 404 页会被记成「可用」**
> (9 条动态路由的 `textLen` 恰好都是 76)。详见那一篇 §⑫。
...
3. ~~**`ListPage` 的边距槽**(⑩-9):两处回归,按"第三次才建"不建。~~
   > ### ☞ 【CONV-9(2026-09-04)建了它 —— 到第 **7** 处】
   > CONV-9 在 35 张详情页里一次量到 **5 处**(合计 7 处),而**真正逼着建它的
   > 不是数量,是【方向】**:那 5 处里有 3 处写的是 `p-4 sm:p-8`,
   > 也就是**它们已经是为手机调过的**。硬套外壳的 `p-8` 会在 390px 上
   > **各夺走 32px 可用宽度** —— 那一刀的全部目的正是改善手机可用度,
   > 所以那不是"一处可接受的版式让步",是**亲手制造出自己要修的那个病**。
   > 槽是一个可选 prop(`padding`),不给就是 `p-8`,**既有调用点一个字都不用改**
   > —— 与 CONV-8 的 `breadcrumb` 槽逐字同一条判据。详见
   > [`docs/detail-page-template.md`](./detail-page-template.md) §⑫-3。
