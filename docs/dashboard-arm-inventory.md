...
### 保养那两支的【补救在哪一页】,以及它们的隐患(EQP-2d,2026-08-21)

**两支的 `itemHref` 都指向 `/finance/assets/[id]`。** 判据是本文件那条唯一的规矩:
**补救动作在不在那张页面上。** 补救是「给这台机器记一次保养」,而 EQP-2d 正是
把那张表单建在机器自己那一页上的 —— 所以门牌指向它,而不是指向
`/finance/assets` 那张列表。`item_id` 是 `fixed_assets` 的行(间隔行今天没有
自己的页面),与 `bank_unmatched` / `margin_cost_not_allocated` 取父行同一条。

**隐患,照既有的惯例点名(与 `safety_stock_below` / `output_unsold_aging` /
`work_order_overdue` 逐字同形):** 门牌落地的那一页上,**把间隔调大、或者把那条
间隔行删掉,这盏灯会立刻安静,而一次保养都没有做。** 这是判据之外的事,
**不是不给链接的理由** —— 补救确实在那一页上,而"在同一页上还能做别的事"
对上面三支同样成立。EQP-2d 在屏幕上把这件事拆开了一半:删行 =【未监控】
(读数消失、状态改名),`disposition='ignore'` =【盯着,不吵】(读数还在),
两个动作各有各的文案,而删除那一步的确认框直说了这个区别。

**「未监控」不上任何一支,而这【不是】一个疏漏。** 一台没有间隔行的机器
按定义不在这两支里(视图的谓词第一句就是 `s.monitored`)——
`operations_now` 装的是【等着人处理的事】,而"没人决定要盯它"是一个**状态**,
不是一件待办(与批次毛利被撤出看板同一条判据,见「考虑过又排除」)。
它画在机器自己那一页上;**全库范围的"哪些机器没人在盯"今天没有任何一处回答**,
已记进 `docs/known-issues.md`,带删除条件。
...
| **Retention due — awaiting confirmation (EQP-PAY-1)** | **DEFERRED, with a trigger — not rejected.** The arm is well-formed on all three properties: it is a genuine waiting state (a person confirms release and it clears), it declares `module.purchasing.view`, and it is naturally bounded (unreleased retentions only). Source would be `purchase_order_retention_status` where `retention_state = 'awaiting_confirmation'`. **It is not shipped because it cannot be SHOWN to discriminate today: live holds zero retention rows, so the arm would render empty for the foreseeable future — and an arm that is permanently empty is indistinguishable, on the page everyone lands on, from one that is broken.** A rolled-back fixture *can* make it produce one row and then zero (`db/fixtures/177` already moves the state that way), but that proves the **predicate** is right, not that the **arm** can be seen — and this file's standard is the second one. Making it visible today would mean putting a retention with a 13-month-old acceptance date onto live, i.e. adding test residue to a database whose existing test residue is already a named problem. **Trigger: the day the first `purchase_order_line_retentions` row exists on live, build the arm** (add its row to the table above in the same commit, per this file's own rule). Until then the prompt lives on the purchase order page — and that surface's weakness is stated rather than glossed: **someone has to open the order to see it, and a payment falling due twelve months out is precisely the kind that gets forgotten.** That weakness is the arm's whole value. Full reasoning: `docs/equipment-payment-milestones-and-retention.md` §7. |
