...
### `fx_rates` 的【中间价】—— METAL-3 之后它是一项日常义务,不再是可选的一种价

**要做的事:每个非本位币,每个期末,各录一条 `mid`。**(今天是 USD 与 CNY 两个。)

**为什么这一条是新的**:实测过,线上历史上一共只有【一行】中间价(USD,2026-07-31),
对 9 行 tt_buy/tt_sell —— 也就是说 mid 一直是"存在但没人维护"的一种价。METAL-3 起
它承重了两处:

1. **月结的重估**按币种逐个换算,每个非本位币都要期末中间价(月结页原来只问
   "有没有任意一条",两个币种时一条 USD 就让那一步显示成"可以做了";已改成逐币种,
   并把缺的那几个点名);
2. **以外币发布的金属行情**(SMM 按 CNY/吨)按【报价那一天】的中间价换算成 USD,
   两条腿:报价币 → 本位币 → USD。所以 CNY 与 USD 【都】要有 mid。

**没录会怎样**:重估在月结页上显示 `blocked` 并点名缺哪个币种;按那条报价计价会
以 `FX_RATE_MISSING|<币种>|<报价日>|mid` 拒绝 —— 吵,但看得见。
**而"等人处理"那一头也接上了**:看板的缺牌价支现在把【有报价、报价币非本位币的
那些天】也算进来(`fx_rate_gaps.gap_source = 'quote'`),于是缺一条中间价会先出现在
首页,而不是等它挡住一次报价。
...
| `accrued_annual_leave`(启用日在年中时) | 累积起点取 `GREATEST(入职月, 当年 1 月)` —— **`system_start_date` 不在其中**。若本库 10 月才有完整记录,3 月入职者 2026 年仍累积 10 个月,含未覆盖的 3–9 月。**补这个界限时务必写成三者取最晚**:`GREATEST(入职月, 假期年年初, 完整记录起始月)`,不要叠加削减 —— `db/fixtures/13` 已把"取交集"这条性质锁住,并在文件里留了这句提醒 |
