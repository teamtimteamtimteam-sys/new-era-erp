...
password to finish setting up your account."* The new password must be at least 8 characters. Until you set it, that screen shows no menus and no modules — the only
other thing on it is **Not you? Log out**.
...
activity you get a warning first — **You will be signed out shortly**, with a
**Continue working** button that keeps you exactly where you are.

If you are locked out entirely, an administrator can restore access. Nobody is permanently
locked out.

## 1.2 What you are looking at

After signing in you land on the home page. It is a starting point rather than a dashboard: the
company mark, a greeting, and a search box that is not built yet and says so — *"Search is not
built yet. This box is where it will live — for now it does nothing, so it does not take what
you type either."* Everything you do is reached from the module bar above it.

If your account holds no module permissions, the home page says so: *"You do not yet have
access to any module. Your account is signed in but no module has been granted to it. Ask an
...
Immediately after you set your password for the first time you are taken either to **My
profile**, if your account is linked to an employee record, or to a page headed **Your account
is ready**, which says: *"An administrator will finish setting up your access. Once your
account is linked to your employee record you will see your own profile, payslips and training
here."* That is not an error. It means the account exists and the link has not been made yet.

Three pages are yours alone and do not depend on module permissions:

- **My profile** (`/me`) — your own record, payslips and training. Its subtitle reads *"Your
  own record. Only you and HR can see this."* If your account is not yet linked to an employee
  record, it says so and tells you to ask an administrator.
- **My reviews** (`/my-reviews`) — performance reviews you conduct, if you conduct any.
...
Two things about that menu are worth knowing before you start looking for something.
...
**Some pages are not in any menu.** Twenty working pages have no menu entry of their own.
Every one of them is reached in one click from a page that does have a menu entry, and the
door is named wherever the page is described. The full list is in section 3.10. The one that catches people out most is **Field Receiving**, which is where goods are
received against a purchase order: it is a button on the Inbound page, not a menu entry.
...
Deleting a document does not remove it. The record stays, marked as deleted, together with
**who deleted it** and **why** — a reason is required, and the system refuses the deletion
without one.

Deleted records disappear from the lists and detail pages where they used to appear. They are
listed on **Settings → Deleted records**, which needs the `data.view_deleted`
capability. That page is a register, not a recycle bin: **there is no restore.** Nothing in the
system puts a deleted record back.
...
1. **Some documents cannot be deleted at all.** Invoices, credit notes, journal entries,
   ...
   ledger. For these, see 1.6 below and the module sections — the correction is a reversal or
   a credit note, never a deletion.
2. **Configuration rows are removed outright, not marked.** A public holiday, a storage
   location's allowed material classes, a line on a draft quotation, a maintenance interval on
   a machine, a metal on a pricing formula — removing one of these deletes the row. It is not
   listed on Settings → Deleted records, because there is nothing left to list.
3. **A step inside a task is removed outright.** On a task, the **Delete** control on a step
   removes it. That deletion is not recorded anywhere and cannot be undone. The confirmation
   dialog is the only place that says so.

One wording to be aware of: some delete confirmations still say *"(Soft delete: data is kept
and recoverable.)"* The first half is true — the data is kept. The second half is not
something you can act on: no screen recovers a deleted record.

## 1.6 Reversal, not correction

Most of Finance, and all of the stock ledger, is append-only. You do not edit a posted
document and you do not delete it. You post a second, opposite document.
...
This is the single most useful thing to know before you start entering documents. Wherever a
process step below has no undo, it says so in those words.
...
**Can you see this number?** A page you can open may still withhold particular values. The
cell reads **Restricted**. It is never blank and never zero. The most common case is money:
...
contents, the page says which — for example, on the contract form: *"Customers are not listed
...
and that string is what an administrator ticks under **Settings → Roles**. Part 4 lists all 39
of them.
...
The system almost never fills a date in for you, and where it refuses to, it says why. A
process date decides which metal prices a cost allocation uses. An invoice issue date decides
which accounting period the entry lands in. A ship date decides which period revenue lands in.
An arrival date is the day the goods actually arrived.

A date filled in for you would be right most of the time and silently wrong the rest of the
time, so blank means blank. Expect to type a date on almost every document, and expect a
refusal by name if you do not.
...
Every step below gives the same five things: who does it, which page it is on, what permission
it needs, what happens if the step before it was not done, and how to undo a mistake. Where a
step has no undo, it says so in those words.

Refusal sentences are quoted exactly as they appear on screen, so what is in front of you can
be matched to the page.
...
second is legitimate — a free sample, a spot delivery — but the system asks you to say which
it was.
...
- **If not done:** you can still receive, but you must choose a reason from **Source (no
  purchase order)**. The unexplained case is refused: *"This reason requires a written
  explanation: an unexplained 'other' says nothing."*
- **Undo:** an order with nothing received against it can be cancelled with a reason. See 2.2.

### Step 2 — Book the load in

There are two doors, and the Inbound page states the difference between them: *"Field Receiving
weighs one material at the scale — kilograms, no price, and it can receive against a purchase
order line. Add Inbound is the full entry: unit, unit price and stage."*

**Field Receiving** — one material, weighed at the scale.
...
- **Permission:** the Inbound page needs `module.inbound.view`; saving needs
  `module.inbound.edit`.
- **What you enter:** Supplier, Material, **Weighed Quantity (kg)**, **Arrival Date**,
  optionally **Declared quantity (optional)** and Notes. The declared quantity is what the
  supplier said was coming, and it is never prefilled from the purchase order: *"our order is
  not their declaration."* Left blank it means "not recorded", which is not zero.
...
- **If the arrival date is missing:** *"The arrival date is required — the stock movement
  ...
  saved. Enter the date and submit again."*
- **Undo:** delete the batch from its own page, giving a reason. That writes a write-off
  ...
  correct a typo — see below.

**Add Inbound** — the full entry, with unit, unit price and stage.
...
- **Permission:** `module.inbound.edit`. The screen says: *"Recording the condition of a load
  needs edit rights in the inbound module."*
- **What you record:** **Safety state** (tick everything true of the load — it can be both
  water-exposed and damaged; nothing is pre-ticked) and **Chemistry certainty**.
- **The blank is meaningful.** *"No safety state is recorded for this load. That means NOBODY
  HAS RECORDED ONE - it does not mean the load is safe."* And on the distinction between two
  chemistry answers: *"'Mixed' means you KNOW it is mixed - an established fact. If you could
  not tell what it is, that is 'Unknown, pending identification', not 'Mixed'."*
- **If not done:** the batch cannot be processed. Committing a processing run that consumes it
  ...
  — it does not mean the load is safe. Record it at Inbound → open that batch → 'Condition on
  arrival', then commit the run again."*
...
- **Not every batch answers these questions.** For material that is not battery material the
  panel says: *"Does not apply: a {kind} has no safety state and no chemistry to be certain
  about. Only battery material answers these axes."*
...
The batch page shows **Landed cost**: *"What this batch has cost so far. The purchase price is
what we owe the supplier; freight and processing cost are capitalised onto the batch separately
...
This system has a two-level purchase approval chain. It is built, it is configured, and it is
**not in force**. The approvals page says so directly: *"Approvals are NOT in force. Orders are
stamped approved by the system when they are raised; nobody decides them. This is a state the
system says out loud, not a silent permissiveness."*

So **a purchase order is created already approved**. There is no approval step in the process
below, because there is no approval to wait for. If you try to approve one anyway, the system
refuses: *"Approvals are not in force, so there is nothing to approve — this order was stamped
by the system when it was raised; nobody decided it."*
...
database, not a button.

If approvals ever are switched on, an order is then raised as a draft and must be approved
before goods can be received against it or a deposit released. The approvals page also states
what the switch would and would not do to existing orders: turning it on leaves existing orders
receivable, and turning it off never retroactively approves anything.
...
- **Choose the kind first.** An order is either **Materials** or **Equipment**, never both. The
  form says: *"One order is either materials or equipment — never both. Choose before you type:
  the rule is enforced when you submit, and a refusal after typing wastes the typing."* If you
  mix them, the refusal explains why: *"…One order is either all material or all equipment —
  mixing them makes its ordered quantity a number that adds kilograms to machines."*
...
- **Equipment orders are different.** Each line is one machine, and the machine's asset card
  must already exist: *"Pick a machine registered under Finance → Assets. A purchase order line
  never creates a machine; the card exists first."* An equipment order is never received into
  stock: *"This is an equipment order. A machine arriving is not a goods receipt — it creates
  no batch, has no assay and does not enter a location, so there is no 'receive' action here."*
- **GST.** Amounts on a purchase order are exclusive of GST: *"A purchase order is a
  commitment, not a tax point — input GST is recorded when the supplier's invoice is entered as
  an expense, using that supplier's default tax code."*
...
- **Undo:** **Amend** on the order, with a reason. *"Quantity, price and dates can be amended.
  ...
  date, and if no rate is on file the amendment is refused rather than guessing one. Amending a
  closed order is refused until you reopen it: *"This order is closed. Reopen it first - so the
  change is a recorded action, not a side effect."*
...
- **What it does:** posts to the ledger. This is where input GST is recorded. Enter the amount
  **net** of GST and pick a tax code: *"The tax is computed on top: a claimable code posts it to
  ...
  code must be picked here. It is never chosen for you — TX, ZP, EP, BL and OP all behave
  differently on the return."*
- **Non-resident payees.** If the payee is recorded as non-resident, the form asks whether tax
  must be withheld and will not accept a blank: *"If withholding genuinely does not apply,
  choose 'Not subject to withholding' — do not leave it blank."* An expense that attracts
  withholding cannot be recorded as already paid; record it unpaid and withhold at the payment.
- **Capital purchases.** Ticking **Capital expenditure (fixed asset)** posts to fixed assets and
  creates the register entry in the same transaction: *"the asset cannot exist without the
  payable behind it."*
- **If the equipment line has already been expensed:** refused. *"One line is expensed once —
  if the price changed, amend the order line."*
- **Undo:** **there is no undo.** An expense cannot be edited or deleted — the guard is
  `EXPENSE_IMMUTABLE`. The correction is a **reversal**, which posts an opposite entry and
  leaves both on the ledger.
...
- **Currency must match.** *"{document} is a {currency} document — settle it with a {currency}
  payment (got {other})."*
- **You cannot over-allocate.** *"Allocation to {doc} ({amount}) exceeds its open amount
  ({open})"*, and *"Total allocated ({total}) exceeds the payment amount ({amount})"*.
...
- **Deposits.** Where a deposit was paid on the order, releasing it against an invoice is a
  separate act on the invoice: *"A deposit was paid on purchase order {po}. Releasing it moves
  that money from prepayments onto this invoice — no cash moves."* The deposit is measured in
  base currency at the rate it was paid at; the release amount is in the invoice's currency, and
  the server converts.
- **Undo:** **there is no undo.** A payment cannot be edited or deleted — `PAYMENT_IMMUTABLE`.
  The correction is a **reversal**.

### Step 5 — Close the order

- **Page:** the order's own page, **Close order**.
- **Permission:** `module.purchasing.edit`.
- **Refused if:** the order is cancelled, or already closed (*"Purchase order {code} is already
  closed"*).
...
- **Undo:** **Reopen**, with a reason. Reopening returns the order to the status it held before
  it was closed. A reason is required.

### Cancelling an order

Cancelling is narrow, and deliberately so.
...
- **Refused once a deposit has been released:** *"Purchase order {code} already has a deposit
  released against it — cancelling it would leave that release pointing at a void order.
  Reverse the release first."*
- **A reason is required:** *"A reason is required to cancel purchase order {code}."*
- **There is no undo.** The confirmation says: *"The order is closed for good — it cannot be
  reopened. Who cancelled it and why are both recorded."*
...
and the run is not required to have a plan behind it.

### Step 1 — Write the work order (optional)

- **Page:** Operation → Work orders → **+ New work order**.
- **Permission:** `module.processing.edit`.
- **What it is:** *"A work order is a plan: what to process, how much, and when. What actually
  happened lives on the processing runs, and progress below is read from them — never stored."*
- **Planned inputs are by material, not by batch:** *"when a plan is written the batches often
  do not exist yet, and choosing them is a decision made on the day."*
- **Expected outputs are optional**, and every expectation row must say where its number came
  ...
  **Calibrated against real production**. There is no default: *"a missed entry is a failure,
  not a quietly supplied value that looks like an answer."* An empty expectation table means
  nobody estimated, which is not the same as expecting zero.
- **It saves as a draft:** *"Saved as a draft. Releasing it is a separate step."*
...
- **Refused if not a draft:** *"Work order {code} is {status} — only a draft can be released."*
- **Undo:** there is no un-release. A released order is closed or cancelled, not returned to
  ...
  Cancelling would say it never happened, and the material really did move."*
...
- **Permission:** `module.processing.edit`.
- **What you choose:**
  - **Work order (optional)** — only released orders are listed, and *"Inputs are NOT prefilled
    from the plan: a plan names materials, a run names batches, and a batch the system guessed
    would be a plausible wrong answer."*
  - **Operation** — *"Pick an operation — it decides what this machine accepts."* Some
    operations produce no new batch at all: *"same batch in, same batch out, state changed."*
  - **Process Date** — required. *"A process date is required — it decides the movement date
    and which metal prices the allocation uses."* The Save button is disabled while it is blank.
  - **Cost allocation basis** — prefilled from the company default in Finance settings, and
    changeable here. *"Pick a cost allocation basis — it decides each output batch's reported
    gross margin, so it is a choice, never a default."*
...
- **If the input carries a safety state that may not be fed:** *"Batch {code} carries safety
  ...
  discharged AND water-exposed is still water-exposed, and discharge does not cancel water."*
- **If the operation does not accept what the batch is carrying:** the refusal says so, and says
  what else might: *"This is NOT the same sentence as 'may not be fed' — another operation may
  well accept it: charged material goes through Deep discharge first, and packs that cannot be
  discharged go to the Battery processing line."*
...
- **Undo:** **Delete Processing Run** on the run's page, which asks *"Roll this processing run
  back?"* and confirms with **Roll back this run**. A reason is required. The consequence is
  stated on the dialog:
  *"This REVERSES the run: the consumed inputs are restored to their batches, the output batches
  are voided, and reversing movements are written to the stock ledger. It cannot be undone."*
  A reversed run stays listed against its work order, because it really was worked, but its
  consumption stops counting anywhere.

### Step 4 — Allocate the cost

This is a separate, later step, and it is the one most often left undone.
...
- **If not done:** the run sits committed and unallocated indefinitely, and the batches carry no
  ...
  carries them, no batch does."* Month-end close refuses while any run is in this state.
- **If the cost entries changed after the last allocation:** the page says *"Allocation is out
  of date — cost entries changed after the last allocation"*, and explains what re-running does:
  the difference posts to inventory for the in-stock share, to cost of goods sold for the sold
  share, and to inventory adjustment for the written-off share, dated today.
- **If allocating by metal value and no output batch has a priced metal:** *"No priced metal
  content on any output batch — record assay results, or add a price for at least one contained
  metal in Metal Prices (or allocate by weight)."*
- **Undo:** re-allocate. Allocation records themselves cannot be edited — `ALLOCATION_IMMUTABLE`.
...
- **Permission:** `module.processing.edit`. *"You do not have processing edit rights, so this
  cannot be changed — promising a batch to the plant is a processing decision."* Note that this
  is a processing right, not a sales right.
- **What it means:** *"Whether this batch is saleable stock, or is already earmarked as feed for
  a downstream operation. This is a separate axis from the sales state: a batch can be both 'in
  stock' and already promised to the plant."*
- **If earmarked:** *"selling, reserving and shipping will all be refused. This is NOT a
  statement that the thing may not be sold; release it back to saleable stock and it sells."*
...
- **If not done:** *"No safety state has been recorded. That means NOBODY HAS LOOKED, not that
  the batch is safe — this batch cannot be fed into any operation until someone records one
  here."*
...
There are two different ways material leaves this company, and they are not variants of one
another. Read the first paragraph of each before you decide which you are doing.
...
**The order is invoiced before it is shipped.** This is the opposite of the way most people
describe a sale, and the system enforces it per line. The invoicing panel says so on screen:
*"Order flow requires INVOICE BEFORE SHIPMENT."*

#### Step 1 — Raise the quotation

- **Page:** Sales → Quotations → **New quotation**.
- **Permission:** `module.sales.edit`. *"editing a quotation needs module.sales.edit."*
- **What it is:** *"A quotation is the document BEFORE the commitment. It touches no stock and
  no ledger; its one special power is converting into a sales order, copying this quote
  exactly."*
- **The customer does not have to be onboarded:** *"a name and a country are enough, which is
  the point: you quote people before they buy."*
- **Both dates are required and neither is defaulted.** *"The validity date is a commitment: a
...
- **Saving creates a draft.** *"Issuing is a separate step, and issuing is what sends it to the
  customer."*
- **Undo:** a quotation stays editable after issue — *"negotiating is what it is for."* Changing
  it after issue puts a banner on the page: *"This quotation has been changed since it was last
  issued — the copy the customer holds no longer matches. Re-issue it below to send the current
  version."* Each issue appends a version and older versions are kept.

#### Step 2 — Issue it

- **Page:** the quotation, **Issue PDF**.
- **Permission:** `module.sales.edit`.
- **If there are no lines:** *"Add at least one line first: an issued quotation with no lines is
  a blank page to the customer."*
- **The system does not know whether the customer received it.** *"There is no 'sent' flag — the
  system does not know what the counterparty received."*
...
- **If it expired:** *"This quotation expired on {date}, so it cannot be converted. To proceed,
  change the validity date and issue a new version — the customer should be looking at a price
...
- **What conversion does:** *"Creates a DRAFT sales order copying {code} exactly — same lines,
  quantities, prices, price provenance, currency and FX rate, nothing re-derived. The quotation
  then freezes: it has become an order, and the two must not drift apart."*
- **Declining requires a reason:** *"three months from now nobody will remember why this one did
  not close."*
...
- **If the order has no lines:** *"Order {code} has no lines — an order with nothing on it
  cannot be confirmed."*
- **Undo:** a confirmed order can be **cancelled**. It cannot be returned to draft.

#### Step 5 — Reserve the stock

Shipping consumes a reservation, so this step is not optional.

- **Page:** the order's own page, **Reserved stock** section, **Reserve**.
- **Permission:** `module.sales.edit`.
- **Why:** *"shipping consumes a reservation, because only a reservation says which batch and
  which location."*
...
- **Only output batches can be reserved:** *"Sales draw from output batches; incoming material
  has to be processed first."*
- **If the batch is earmarked for processing:** *"Batch {code} is earmarked as {purpose}, so it
  is not saleable stock and can be neither reserved nor shipped — this is NOT a statement that
  the thing may not be sold. Release the earmark on the output batch page, or use a different
  batch."*
...
- **If the batch does not hold that much:** *"Cannot reserve {qty} — only {available} is
  available in that batch and location. Reserving does not create stock; it only sets aside what
  is already there."*
- **What reserving does:** *"Moves this quantity into the committed bucket for this line. Sales
  and processing cannot touch it until it is released here, or shipped."*
- **Undo:** **Release**, with a reason — *"A reason is required — taking back a promise is
  exactly what nobody can reconstruct later."* *"Returns the stock to available. A partial
  release cancels this reservation and records a new one for the remainder, so each row stays a
...
- **Permission:** seeing the section needs `module.finance.view`; raising the invoice needs
  `module.finance.edit`. The panel says why: *"raising an invoice needs module.finance.edit —
  it is a posting document."*
- **If the order is still a draft:** *"Only a confirmed order can be invoiced — a draft is not
  yet a commitment."*
- **The issue date decides the posting period and is required:** *"The issue date decides the
  posting period — it is required and never defaulted."*
- **What it posts:** *"Posts AR against contract liability at the order's rate. Shipping requires
  this invoice. The credit limit is checked here — invoicing is where the exposure is created."*
...
- **A ship date is required:** *"The ship date is required — it is the day the goods physically
  left, and it decides which period the revenue lands in. It is never filled in for you.
  Nothing was saved."*
- **If the line is not on a live issued invoice:** *"Line {n} of order {code} is not on a live
  posted invoice. Order flow is invoice-before-shipment: raise the invoice first. Nothing was
  shipped."*
...
- **What it does:** writes a shipment (`SHP-`), moves the stock out, releases the contract
  ...
  shipped** or **Shipped**. Shipping creates no new receivable: *"the debt was recognised when
  the invoice was raised. The invoice stays the one and only AR row for this order."*
- **Undo:** **there is no undo.** A shipment cannot be edited or deleted, and the invoice behind
  it can no longer be voided. See the warning below.
- One thing on that button is out of date. Its confirmation says corrections *"would go through a
  credit note, which does not exist yet"*. Credit notes do exist — Finance → Credit notes is the
  register and one is raised from the invoice. Read that sentence as "corrections go through a
  credit note".
...
This is the single most likely place in the system to get stuck, and it is worth knowing before
...
- **What is required:** quantity, unit price, currency, exchange rate, sale date, and normally a
  customer. *"A sale date is required — it sets the FX rate used, the inventory movement date,
  and the period both journals post to. It is deliberately not defaulted server-side."*
...
- **If no customer is chosen:** the form warns rather than refusing: *"This sale will be
  recorded, but it will not be credit-checked and it will not count towards anyone's exposure —
  it belongs to nobody until a customer is attached to it later, and it cannot be invoiced to a
  customer before that."*
- **Credit control:** if the customer is on hold, or the sale would take them past their limit,
  the sale is refused and the panel shows the limit, the outstanding and the headroom before
...
- **Undo:** **there is no undo.** The sale record cannot be edited or deleted, and the stock
  ...
  to order-flow invoices only, and the invoicing page says so — *"This invoice posts nothing on
  its own — the receivable sits on the sales records behind its lines — so a correction here
  still goes through voiding the invoice or reversing the receipt."*
...
- **What you enter:** a counted quantity per batch, and optional notes. The page shows
  **Current**, **Book**, **Counted** and **Delta** side by side, and summarises: *"Counted {n} /
  {total} batches · {diffs} with differences"*.
- **Undo:** **Re-count** re-opens a line. Nothing has left the ledger yet.

### Step 3 — Review and post

- **Page:** the stocktake → **Review & Post** (`/stocktakes/[id]/review`).
- **Permission:** `module.stocktakes.edit`.
- **What posting does:** *"Posting writes one adjustment movement per differing batch and sets
  its stock to the counted quantity."* If there are no differences the page says so: *"No
  differences — posting will change nothing."*
...
**Month-end** page lists them as steps you can walk down.

- **Page:** Finance → Month-end (`/finance/month-end`).
...
Three of these say when they are blocked rather than merely outstanding:

- *"blocked: payroll not posted"* — employees cannot be marked paid before the payroll is posted.
- *"blocked: no mid rate on file for {currency} for {date} — enter it under FX first."*
- *"blocked: run the revaluation first"*.

**CPF is not a reason to hold the close.** *"CPF is due by the 14th of the FOLLOWING month and
its journal lands in that month — locking this month does not block it."*
...
- **Undo:** **Reopen**, on the same page, with a reason — *"Reopen this period? Its entries
  become editable again."* A reason is required, the close record is kept, and the lock date
  moves back. Reopening the same close twice is refused by name. Note that reopening a month
  does not reopen a closed financial year: an entry dated inside a closed year is still refused
  until the year itself is reopened.
...
*"Close the financial year ending {date}? Every P&L account is taken to zero and the net result
posts to 3100. The year then refuses new entries until deliberately reopened."*

**Undo:** **Reopen year**, with a reason — *"The closing entry is reversed (audit trail kept)
and the year accepts entries again."*
...
- **A quarter cannot be filed while its books are still open:** *"This quarter ends on {end} but
  the books are only closed up to {locked}. A filed return whose entries can still change is not
  a record of anything."*
- **The filing itself happens elsewhere:** *"Filing itself happens on the IRAS portal. This
  system records what was filed, when, and by whom — it does not submit anything."*
- **Undo:** **there is no undo.** *"A filed return is a record of what was filed and is never
  edited."* The correction is a **Raise a correction**, which creates a new return pointing at
  the one it corrects: *"The original stays exactly as it was filed — that is the whole point of
  keeping it."*
...
Where a page has no menu entry, the entry point is given. Section 3.10 lists all twenty in one
place.

**Every module's first menu entry is called Overview**, and there are seven of them. The page
itself is headed with the module's name rather than the word Overview, so "Purchasing →
Overview" opens a page headed **Purchasing**. Six of the seven carry the same explanation:
*"Where this module stands right now. Only facts that span several of its pages and that no
single page can state — the pages themselves are in the menu above."* Inventory's is older and
different in shape; see 3.6.
...
| Receiving discrepancies | yes | Where what arrived does not match what was ordered: short, over, declared against weighed, a different material than ordered, or an assay outside tolerance. Each against a threshold set under receiving settings. |
...
| From | To | By |
|---|---|---|
| Draft | Confirmed | Approving it — but see below |
...
**Final:** Cancelled. *"The order is closed for good — it cannot be reopened."* Closed is not
final: it reopens.

**Approval status** is a second, separate marker on the order: Pending · Approved · Rejected.
Both Approved and Rejected are final — neither can be changed once set. Because approvals are
switched off, every order is created with approval status **Approved** and status
**Confirmed**. It is never a draft in practice.
...
**What you record:** who the contract is with — *"A contract is with a supplier or with a
customer, never both. Picking a supplier makes this a buy-side contract; picking a customer
makes it sell-side. If the same company is both to you, that is two contracts, because they are
two agreements."* — a **Kind** (Supply, Offtake, Framework, Service, Other), a **Title**, an
**In force from** date, an optional **In force until** (blank means no fixed end, and means
exactly that), optional headline terms (signed date, currency, incoterm, payment terms in days,
document reference, notes) and a **Status**.

**Status is chosen at creation and is permanent.** The two choices are **Active** and
**Draft**. The form states what each means and that the choice cannot be revisited:

- Active — *"purchase and sales orders can be linked to this contract, and a linked order copies
  its terms."*
- Draft — *"Still being negotiated: the contract is recorded and readable, but NO order can be
  linked to it. Anything raised against it will be refused."*
- *"This cannot be changed afterwards. Nothing in this system moves a contract from one status
  to another, so whichever you pick here is what it stays. Pick Draft only if you are content
  for it to stay unusable."*
...
**After creation, the only thing you can do with a contract is read it.** The form says so
before you save: *"It CANNOT be edited, amended, or moved to another status afterwards, and it
cannot be deleted. There is also no screen yet for linking an order to a contract. Record it
only when you are content with what it says."*

**Undo: there is none.** A contract cannot be edited, re-statused or deleted. If a contract is
recorded wrongly, it stays in the register as recorded.

**Refusals when creating one, quoted exactly:**

- *"A contract is with exactly one party — either a supplier or a customer, not both and not
  neither. Nothing was saved. Pick one from the list."*
...
- *"The date the contract takes effect is required — without it the register cannot say whether
  the agreement was in force when an order was raised. Nothing was saved."*
...
- *"Recording a contract needs permission to edit whichever side it is with … and you do not
  have it for the party you chose."*

**What a contract is for.** The register page explains: *"A purchase order is not a contract. A
long-term supply agreement is — it is the relationship a document is raised under. Contract
terms (grade specification, insurance obligations, volume commitments) live here; a document
raised under a contract COPIES the terms in force at that moment, and later edits to the
contract do not reach it."* The register also reports grade specifications not met, measured
against the specification the document copied when it was linked.

**Read the coverage figures before the breach figures.** The page says why: *"Nothing forces a
document to carry a contract, and that is deliberate — spot purchases do not run under an
agreement. So read the figures above before reading anything below: 'no contract breached' can
also mean 'nobody linked anything'."*
...
Logistics is drawn as a dash with the note *"This module has no separate edit permission — its
View permission is the whole of it."*
...
**Freight is capitalised, not expensed.** The page states the trade-off: *"Freight is
capitalised into the cost of the material it carried, not expensed. The trade that comes with
it: a wrong apportionment now sits inside inventory instead of showing on the P&L."*

You choose the basis on which the freight is spread across batches — by weight or by value —
and the page explains when the two diverge: *"Weight and value diverge most exactly when it
matters most — a light, expensive batch travelling with heavy, cheap material. Pick the one the
invoice actually supports; if the forwarder itemised per batch, use that."*
...
*"Closing short is legal and recorded — a rule that forces you to shrink the plan to close it
would erase the variance instead."*
...
A run is created committed. The only move is **Delete Processing Run**, with a reason, which sets it
Reversed, restores the consumed inputs, voids the output batches and writes reversing
movements. **Reversed is final.**

A reversed run stays listed against its work order: *"A reversed run stays listed — it really
was worked against this order — but its consumption no longer counts anywhere."*

**Loss is recorded in categories, not as one number.** The loss panel explains why: *"Water and
volatiles leave but the METAL STAYS; dust and spillage take the metal WITH them; residue sent
for disposal is NOT A LOSS AT ALL. Collapsed into one number, recovery can never be right."*
Categorised amounts need not add up to the run's loss total, but may not exceed it.
...
rather than blank: *"Unacknowledged means THE NEXT SHIFT HAS NOT SAID THEY READ THIS — not 'this
field is empty'."*

Two things a handover deliberately does not carry. It does not record what the shift processed:
*"A processing run carries only a date, no time of day, so no run can be attributed to a
shift."* And it does not record incidents: *"They belong on the workplace incident and near-miss
register (not yet built)."* Equipment tick boxes point at downtime already recorded elsewhere
...
The order page lists the moves available from wherever it is, and says why it lists them that
way: *"Allowed transitions are listed per state, not as an exclusion — the database holds the
same table and always wins."* When there is nothing left: *"This order is in a final state — no
further transitions."*
...
raised: *"invoicing is where the exposure is created."* On a direct sale it is applied when the
sale is recorded. A customer on hold blocks both.

A sale recorded with no customer belongs to nobody. It can be attached to a customer afterwards
on the receivables page, and that attachment is **one-way**: *"it cannot be changed to a
different customer or undone afterwards."*
...
for receivables, payables, raw stock and finished stock, and the net position. It states what it
cannot answer rather than printing a zero: *"This leg cannot reach its basis right now, so it
has no figure — answering anyway would give a confident 0.00."*
...
| Cash forecast | *"What we expect to receive and pay, week by week, in the currency it actually moves in"*, over thirteen weeks. Amounts with no date are listed separately and are not in any week. |
| Price exposure | Material bought on a floating price against material sold on a fixed price. The purchase side is not modelled, and the page says so as a statement about the system rather than a zero. |
...
The margin page carries a distinction worth reading once: *"Cost here is the run's CURRENT
allocated unit cost multiplied by the quantity sold. The general ledger posts COGS once at the
moment of sale and never restates it… The two figures are different and both are correct."*
...
invoice cannot be edited and cannot be deleted — `INVOICE_IMMUTABLE`.

**Credit note**, code `CN-`. **No status column and no transitions — it is immutable from the
moment it is created.** It cannot be edited and cannot be deleted.

*"A credit note reduces what the customer owes on THIS invoice, after the invoice can no longer
be voided. It is not a refund: no money moves, and it cannot take the invoice below zero."*
...
reversal lands in… a defaulted date can never hit a locked period, so leaving it blank would
slip through where filling it in correctly does not."*
...
**Reversed is final.** Cannot be edited, cannot be deleted — `PAYMENT_IMMUTABLE`.

**Expense**, code `EXP-`. Its marker is a payment status: Unpaid or Paid. The correction is a
reversal. Cannot be edited, cannot be deleted — `EXPENSE_IMMUTABLE`.
...
a retention clock. Retention is never paid automatically: *"This retention is due. It is NOT
paid automatically — confirm that the machine gave no trouble during the retention period, and
state how much is released and how much is withheld."*

Deleting a maintenance interval means something specific: *"delete row = we no longer watch this
machine for this kind of work"*, which is not the same as leaving it monitored but off the
dashboard.
...
*"Its entries become editable again."* Closing the financial year is a separate act with its own
preview: *"Every P&L account is taken to zero and the net result posts to 3100. The year then
refuses new entries until deliberately reopened."*

**GST period. States:** Open · Filed. **Filed is final** — *"A filed return is a record of what
was filed and is never edited."* The correction is a new return that points at the one it
corrects. A GST period has no deleted marker.
...
The reconciliation identity is printed on the page: *"Closing balance + items on your books the
bank has not shown + items the bank shows that are not on your books = ledger balance."*
...
**FX Rates** has one sub-page with no menu entry, reached from the FX page: **Enter a week**,
a grid for filling gaps. Cells that already hold a rate are read-only there: *"To change one,
open it from the link in the cell and say why — that keeps the previous number on record."*
...
| Page | Menu | What it does |
|---|---|---|
| Overview | yes | Headed **Inventory**. Material balance across all processing runs, and current stock by material, backed by the movement ledger. Unlike the other module overviews this one is a stock listing rather than a set of cross-page facts. |
...
*"Read-only views over stock as it stands. Nothing here is stored — every figure is derived from
the movement ledger when you open the page."*
...
The snapshot values stock at landed cost — purchase price plus freight plus capitalised
processing cost — *"the same definition used by write-off, stocktake and the GL reconciliation."*
Without `data.view_prices` the value columns read **Restricted**: *"That is not zero —
quantities are complete; only the money is withheld from you."*

Stock with no location recorded is shown as its own group: *"This is a normal state, not missing
data — the goods are real, a transfer can assign a location at any time."*
...
**A note on exported files.** The stage column in a downloaded CSV contains the stored value,
which is in Chinese (`待加工` / `加工中` / `已加工完`), not the English shown on screen. The
screen and the file will not read the same for that column.
...
quoted or sold, and the refusal distinguishes that from a material whose form was never set:
*"This is NOT a statement that it may not be sold. Set the form under Materials first."*
...
**Approved, Rejected and Cancelled are all final** as far as the request is concerned. Cancelling
gives the days back: *"Cancelling releases the days back to the grants they came from; the ledger
keeps both entries."*

**Days are counted by the system**, not typed: *"Weekends and public holidays excluded, computed
by the database."* Where Monday-to-Friday counting is wrong — a six-day or shift schedule — there
is an explicit exception path that asks for the days and a reason. An exception can exceed a
leave type's standard days but can never exceed an accrued annual leave balance: *"that is a real
entitlement, not a policy."*

Refusals worth knowing before you submit:

- *"Not enough leave: {available} days available, {requested} requested."*
- *"You will have {n} days accrued by then, and you asked for {m}. Annual leave is earned monthly
  — you will have enough from {date}."*
- *"Annual leave cannot be taken during probation. It keeps accruing month by month from your
  start date, and whatever you have accrued becomes bookable once you are confirmed."*
...
balance is carried forward. Both are idempotent: *"Running it twice is refused rather than
doubling anyone up."*

**Leave types** and **Public holidays** are configuration: *"Day counts, certificate thresholds
and active flags are data — change them here, no release needed."* Public holidays are added year
by year as they are gazetted: *"Lunar dates are not computed — they come from the official
announcement."* Deleting a holiday removes the row outright.
...
permission — *"HR approves the claim; finance turns it into a payable."*
...
Code `CLM-`. **States:** Submitted · Withdrawn · Approved · Rejected. Decided or withdrawn, and
all three end states are final. An expense claim has no deleted marker.

### Payroll

**Payroll period**, code `PAY-`. **States:** Draft · Posted. **This one is reversible:**
**Post to ledger** and **Unpost** both exist, and unposting asks for a reason. It says what it
does: *"The journal entry will be reversed and the period returns to draft."*

Figures are prefilled from last month, with a warning: *"check every row against the provider
...
*"What we report to the payroll provider each month — overtime hours and unpaid leave. Nothing
here is calculated into pay; the provider does that."*
...
record. A probation review cannot be raised without a probation end date on file: *"That date is
what the confirmation decision turns on and what the reminders count down to, so it is not
invented here."*

Goals carry a target and a unit, and both are set together: *"A goal without a unit can never
take a number later — neither the self-assessment nor the reviewer can add the unit afterwards."*
...
permission: *"Everything dated that you can already see elsewhere, on one month. It is a place to
look, not a place to edit — click an item to go to the page that owns it."* Where a source could
not be read the page says the month is incomplete rather than showing it as empty.
...
same database function the settlement path calls, *"so this tool and an invoice cannot
disagree."*

**Reminders** gathers waiting signals from every module: *"Each item appears only if your
permissions reach its own module."* It separates three answers carefully — a real zero
(*"You can see these, and right now they have nothing"*), a permission wall (*"These are not
zeros — this account cannot answer whether they have anything"*), and nothing visible at all.
...
**Make this a team task** promotes a personal task, and once other people have worked on it the
promotion cannot be reversed: *"Someone else has been on this task, so it stays a team task —
turning it personal would leave them unable to read what they worked on."*

Promotion needs your login to be linked to an employee record. If it is not: *"Ask HR to link
your account, then this becomes available."*

**Steps.** A task can carry steps, and a step can carry one level of sub-steps. Steps are
reordered and renamed in place. **Deleting a step removes it outright** — this is the one place
in the system where a delete is not recorded and cannot be undone. A step that has sub-steps is
refused rather than removed with its children.

**Deleting a task** marks it, as elsewhere.
...
not be created."* The initial password is handed over in person — *"The account can sign in as
soon as it is created. Hand the initial password to the person yourself — they will be required
to change it the first time they sign in."* An account can be linked to one employee record, and
an employee already linked elsewhere is not offered.
...
from CSV. It is the only action in the system that can insert hundreds of rows at once. Each
...
Twenty working pages are not listed in any menu. Each is one click from a page that is.
...
`KPI scoring` is reached from HR → KPI and is a sub-page of it in the same way.
...
hold. A role created this afternoon is indistinguishable, to every gate in the system, from one
that has been there since the beginning.

So read the table below as a description of how access is arranged today, not as a law about
who may do what. New roles can be created, existing ones renamed or re-granted, and people moved
...
made. Everything you create through the interface is an ordinary role. That is a deliberate
floor, not a missing feature.

## 4.4 What the capabilities are

There are 39 capabilities, in three kinds.
...
| `action.bulk_import` | Load materials, counterparties, employees, departments and storage locations from a CSV file. The only action that can insert hundreds of rows at once. |

There is one further capability, `module.tasks.view_all`, which would let its holder read other
...
Roles marked **held** have at least one person in them today. The rest are defined and waiting
for one — a role with no holder is not a role that does not work.
...
prices, sales, banking and review content. **Does not hold `action.manage_permissions`**, so
cannot change roles or grants, and does not see the deleted-records register.
...
holds no edit capability at all. The separation is the point — the role that approves a large
order cannot also raise one.

Note that approvals are switched off, so this role approves nothing today. See 2.2.
...
Edit on inbound, output, inventory, stocktakes and tasks. View on logistics. No prices, no
sales figures, no purchasing, no processing.
...
Sees prices and sales figures. **No finance capability**, so a holder of this role can raise and
confirm an order and reserve stock against it, but cannot raise the invoice — and because the
order flow is invoice-before-shipment, cannot ship it either. That step needs finance.
...
every order and no money, and that is a deliberate arrangement rather than a gap.
