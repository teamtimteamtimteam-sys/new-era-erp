...
```
FETCH_OWN_EXIT=0
local  HEAD  = 75899e46c91e1cc1fe6a87b06f8774ea99d581a9
origin/main  = 75899e46c91e1cc1fe6a87b06f8774ea99d581a9
MATCH ✓
```
...
```
gh api repos/…/deployments?per_page=1  → id 6344829856
gh api repos/…/deployments/6344829856/statuses
  {"state":"success","created_at":"2026-09-09T07:33:02Z"}
```
→ **有。**
...
```
deployment sha = 75899e46c91e1cc1fe6a87b06f8774ea99d581a9
this cut sha   = 75899e46c91e1cc1fe6a87b06f8774ea99d581a9
MATCH ✓
```
→ **是。**

★ **为什么要分开问:** 「有一次成功的部署」与「成功的那一次是我这一版」是两句话。
只问第一句,会在**上一版**部署成功、而本版根本没触发时给出一个绿灯。
实测本刀就撞见过这个窗口 —— 推完之后头 165 秒,最新的成功部署 sha 仍然是
`75a1122`(上一刀那一版),本刀这个提交的部署还没建出来。**只问第一句的话,那时候就"通过"了。**
...
|---|---|
| **部署 id** | **6344829856** |
| **sha** | **75899e46c91e1cc1fe6a87b06f8774ea99d581a9** |
| **success 时间** | **2026-09-09T07:33:02Z** |
| environment | Production |
| environment_url | `https://new-era-rl83avfve-tim-s-projects7.vercel.app` |

| | |
|---|---|
| 开工前 HEAD | `75a1122f70855f62c753c66f08a5abf3987d9027` |
| **本刀提交后 HEAD** | **`75899e46c91e1cc1fe6a87b06f8774ea99d581a9`** |
| 结束时 `origin/main` | 同上(fetch 比对过) |
| 结束时树 | **干净**(`git status --porcelain` 0 行) |

> 本节由紧随工作提交之后的一个提交补齐 —— 部署要先有提交才谈得上,
> 而交回报告必须与工作同一个提交进仓库(委托书 §9)。
> 本仓库对这个办法已有两次先例(`df64373` NARROW-COVERAGE-1 §12.3、
> `75a1122` SMALL-BATCH-1 §11)。
