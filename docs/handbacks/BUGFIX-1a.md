...
| ★★ **而这一次推送【把构建弄红了】** | ★★ **Vercel 对 `dd9be64…` 的 Production 构建 `state=failure`。**<br>★★ **病因是【本刀自己那支闸】** —— `scripts/check-generated-css.mjs` 的覆盖断言,第二条路**按行**数规则;而 Vercel 给整条 `npm run build` 导出 `NODE_ENV=production`,`@tailwindcss/postcss` 的 `optimize` 默认值就是它,于是**生成出来的 CSS 被压缩、几乎没有换行**,那条路一条都数不到:`判据数出 922,独立计数是 0`,退 2。<br>★ **不是依赖、不是平台、不是网络** —— 逐条排除的读数在下面 §16.2,**原样留着**。 |
| ★★ **修法那一次提交 —— `55baada` 带的就是它** | ★★ **`55baadab52d2e6fa0c24c0c8c8b50123ec312981`**(`BUGFIX-1a fix: check-generated-css was environment-dependent and broke the Vercel build`)<br>★ **两处**:① 覆盖断言的第二条路改成**按字符**走(认转义、认字符串、认注释、认花括号,**不认换行**);② ★★ **显式 `optimize: false`** —— 这一格是修①的时候才露出来的,它比①贵得多:`NODE_ENV=production` 下那支闸**对着自己存在的唯一理由报绿**(优化器把那条非法声明悄悄丢掉了)。<br>★ **改完的读数:六格故障注入矩阵,两种环境各三格,6 / 6 全过**(§16.2 末尾)。 |
| ★ **中间还有一次【纯文档】提交** | `54a1051`(`BUGFIX-1a docs: the deploy FAILED — evidence, what I ruled out, and why I did not guess a fix`)—— **失败那一刻的读数与排除过程**,逐字留着。 |
| ★★ **这件事已经写进 `AGENTS.md`** | ★ **`AGENTS.md`「★★★ 一道闸在【本机】与在【Vercel】上不是同一道闸 —— `NODE_ENV=production` 会换掉它读的东西(BUGFIX-1a,2026-09-12)」那一节** —— 两条教训、给下一支闸的三句话,并把代价照直记着:**一次失败的生产部署**。 |

### 16.2 §7.7 部署 —— ~~★★ **失败了。这一刀【没有】上线。** ★★~~ · ★★ **【已被取代 —— `55baada` 上线了,2026-09-12】** ★★

> ### ★★ 先把【现在】的结论说完,失败那一刻的整段记录原样留在下面 ★★
> ★★ **`55baadab52d2e6fa0c24c0c8c8b50123ec312981` 的 Production 部署【成功了】,这一刀在线上。**
>
> ### ★★ 而这一条读数【是谁读的】,必须说清楚 ★★
> ★★ **它来自 Tim —— 2026-09-12,他自己在 Vercel 面板上看到的,并转述给我。**
> ★★ **它【不是】本机发出的一次部署查询。** 这台机器够不到 Vercel(五项逐条实测全部 ABSENT,
> 就在下面那一段里),而按 **Tim 2026-09-12 立的那条常设规矩:一刀的终端活【到推送为止】,
> 部署由 Tim 自己看** —— 见 `AGENTS.md`「一刀的终端活【到推送为止】」与
> `docs/forward-queue.md` 同一条。
> ☞ **于是这一格写的是【一份转述】,不是一次测量** —— 两者在报告里长得很像,而它们不是一回事。

| 项 | 读数 | ★ **谁读的** |
|---|---|---|
| 部署的对象 | ★ **`55baadab52d2e6fa0c24c0c8c8b50123ec312981`**(完整 40 字符)· `Production` | ★ **本机**:`HEAD` / `origin/main` / `ls-remote` 三方逐字相同 |
| ★★ **部署状态** | ★★ **成功** | ★★ **Tim · 2026-09-12 · Vercel 面板** |
| ★★ **五条修复在生产上的样子** | ★★ **Tim 在线上把【五条全部】走了一遍,五条都好了** —— **X**(开发服务器的 CSS 故障)· **a**(日历读不到 Container ETA)· **c**(请假来源印出 i18n 键)· **e**(日期选择器关不掉)· **g**(多行框越打越宽) | ★★ **Tim · 2026-09-12 · 生产站点** |
| ★ **于是 §15 里那一条【等 Tim 手上确认】的也就关上了** | ★ **item e 的原生日期浮层"关得掉"这件事,本刀测不到**(浮层不在 DOM 里,无头浏览器不弹),§15 把它记成**等 Tim 手上确认**。★ **Tim 说五条都验过了 —— 它闭合了。** | ★ **Tim**(机制那一半仍是本刀 PROVEN 的,见 §15b「§3 E」) |
| ⚠ **本机【没有】做的事,逐条** | ★ **没有等部署** · **没有把 deployment id 绑到 SHA** · **没有查 `state`** · **没有请求生产别名** · **没有复现那次失败的构建**。<br>☞ 照 2026-09-12 那条新规矩:**推送完,报三方完整 SHA,停。** | —— |

---

> ### ★ 下面这一整段是【第一次推送 `dd9be64` 那一刻】的记录 —— **逐字留着,一个字都不改**
> ★ **它在那一刻是对的**:`dd9be64` 的那次 Production 构建**确实**失败了,这一刀在那一刻**确实**没有上线。
> ★ **改掉它等于抹掉那一刻** —— 与本仓库处理「被取代的读数」逐字同一条办法
> (划掉,不是删掉;下一个人才看得见它曾经是什么)。
> ☞ 而且它带着**病因是怎么查出来的**那一整段,**那一段正是 `55baada` 的由来** ——
> 连同那一趟**买到零**的冷复现,它是 2026-09-12 那条新规矩的价钱。
...
#### ★★ 拿到日志之后 —— **病因查清了,而它就是我自己那支闸** ★★

★ **Tim 把构建日志贴了回来。失败的那一行逐字:**

```
08:40:24.767 ✗ check-generated-css:**覆盖断言失败 —— 这一次读数不作数。**
08:40:24.768     按 AST 数出的规则 ↔ 按文本数出的规则:判据数出 922,独立计数是 0 —— 差 922。
08:40:24.798 Error: Command "npm run build" exited with 2
```

☞ **不是依赖、不是平台、不是网络 —— 是 `scripts/check-generated-css.mjs` 自己。**
★ 而且**它前面 23 条检查全绿**,`check-select-columns` 都没轮到跑。

#### ★ 机制:三件单独看都无害的事叠在一起

| # | 事实 | 读数 |
|--:|---|---|
| 1 | ★ **Vercel 给整条 `npm run build` 导出 `NODE_ENV=production`** | 日志里 `Running "npm run build"`,环境由 Vercel 给 |
| 2 | ★ `@tailwindcss/postcss` 的 `optimize` **默认值就是 `NODE_ENV === 'production'`** | 于是**在 Vercel 上它把生成出来的 CSS 压缩了**,本机没有 |
| 3 | ★★ 那支闸的覆盖断言,第二条路是**按行**数规则(`一行以 { 收尾`) | **压缩之后整份 CSS 几乎没有换行,它一条都数不到** |

★★ **本机一条命令就复现了,读数与 Vercel 逐字相同:**

```
$ NODE_ENV=production node scripts/check-generated-css.mjs
✗ …按 AST 数出的规则 ↔ 按文本数出的规则:判据数出 922,独立计数是 0 —— 差 922。   EXIT=2
```

#### ★ 修法(两处,而第二处比第一处贵得多)

**① 第二条路改成【按字符】走,不认换行。**
认转义、认字符串、认注释、认花括号 —— 但**不对排版做任何假设**。
★ 这一支自己在路上栽了两次,两次都写进了它的注释:

| 栽在哪 | 读数 | 修法 |
|---|---|---|
| CSS 的**反斜杠转义在字符串之外照样有效**,而 Tailwind 的任意变体类名里全是它(`.\[\&_svg\:not\(\[class\*\=\'size-\'\]\)\]\:size-3`)。把 `\'` 当成字符串开头,从那里起整段被吞掉 | **少数 107 条**(1104 ↔ 997) | 先认 `\`,跳两个字符 |
| 压缩输出把 `/*! tailwindcss … */` 直接贴在 `@layer properties{` 前面,于是前言以 `/` 开头而不是 `@` | **多数 1 条**(922 ↔ 923) | 前言里先把注释剔掉(**不是**遇到注释就重置前言 —— 那会把 `@media /* c */ screen {` 数成样式规则) |

**② ★★ `optimize: false` —— 而这一条是修①的时候才露出来的,它才是真正要命的那一格。**

把那支闸放到 `NODE_ENV=production` 下重跑**故障注入**,结果:

```
inj1(往 app/ 放一个带那种散文的临时文件)  NODE_ENV=production → ★★ EXIT=0
```

☞ **它对着【自己存在的唯一理由】报绿。** 因为**优化器会把那条非法声明悄悄丢掉**。
★★ **而那正是这条缺陷当初的样子:`next build` 只是一句 warning,`next dev` 每条路由 500。**
**坏掉的是【没有优化过】的那一份 CSS,所以要判的也是那一份。**
☞ 于是显式 `optimize: false`,让这支闸**与 `NODE_ENV` 无关**。

**③ 顺带被 eslint 冻结闸抓住的一条:** 新写的字符扫描器留了一个没人读的 `depth` 变量,
`check-lint` 当场报「1 条**新的**文件+规则组合」并退 1。**已删掉。**
★ **那道闸做对了它该做的事**,而这也说明:**生产模式的整条构建链必须在本机跑一遍才算验过。**

#### ★ 修完之后的读数 —— 六格故障注入矩阵,两种环境各三格

| | `NODE_ENV` 未设 | `NODE_ENV=production` |
|---|---|---|
| 干净的树 | ★ **EXIT 0**(1104 条规则 / 1453 条声明) | ★ **EXIT 0**(**逐字同一个数**) |
| 注入①:`app/` 下的散文 | ★ **EXIT 1**,点名生成后第 1912 行第 24 列 | ★ **EXIT 1**,**同一个位置** |
| 注入②:拿掉 `@source "../app"`(弄瞎它) | ★ **EXIT 2**,「覆盖断言失败」 | ★ **EXIT 2**,同上 |

★ **6 / 6 全过。** ☞ **这支闸现在与环境无关 —— 而"与环境无关"本身是一条要量的性质,不是一句话。**
