...
### ★★ 终点:**已关闭 —— Tim 确认部署完成(他没有给钟点)。**(APR-3 记,2026-09-22)

★★ **上界 `2026-09-22 22:38:15 CST`,而它是【三种上界里最松的那一种】,照直标出来。**
出处:APR-3 这一次会话里对着库跑的 `select now()`(经 Management API,以 `postgres`)——
也就是**数据库服务器自己的钟**,不是"写这行字时的本地时间"。

> ### ☞ 说清楚为什么只有这一种,因为 APR-1 那一招在这里【不管用】
> APR-1 的上界是**量出来的**:`finance_settings_history` 里那唯一一行
> (`12:25:06`),而那一行**只能由部署之后的代码写出来**。
> ★ **那一招不能推广,而 APR-2 正好是它不管用的那一类:**
> APR-2 的部署**不需要任何人做任何动作** —— 它改的是几支函数与一块面板,
> 没有哪条路径会因为"新代码上线了"而往库里写一行。
>
> **实测(以 `postgres` 读基表,2026-09-22 22:38):自 APR-2 的迁移
> `2026-09-22 21:29:01+08` 以来,库里【一个字节都没有被应用写过】:**
>
> | 表 | 最新一行 |
> |---|---|
> | `finance_settings_history` | 1 行,`2026-09-22 12:25:06`(★ APR-2 之【前】) |
> | `approval_log` | 14 行,最新 `2026-09-11 22:32:01` |
> | `purchase_orders` / `work_orders` / `expense_claims` / `stocktakes` / `payments` / `expenses` | 全部 ≤ `2026-09-20` |
>
> ☞ **所以【没有更紧的上界】,而这句话本身是一次测量的结果,不是一次放弃。**
> ★ **教训,记下来给下一刀:** 一份报告能不能给出一个**量出来的**破窗终点,
> 取决于那一刀的部署后面**恰好有没有一次会写库的人工动作** ——
> 而那是运气,不是方法。**方法只有一个:部署那一端的时刻由够得到 Vercel 的人给。**
