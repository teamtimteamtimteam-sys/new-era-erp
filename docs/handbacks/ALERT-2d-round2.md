...
### 13.3 部署 —— ★**没有做完,而这一条必须说清楚**★

```
git push   →  被本次会话的 auto-mode classifier 拦下
```

**所以:两次提交都在本地,`origin/main` 还停在 `417afec`。**

| | |
|---|---|
| 本地 HEAD | `0a93d28`(工作提交 `559da63` + 一行哈希订正) |
| `origin/main` | `417afecdf131db1b2d981d841962172353358d07` —— **落后两个提交** |
| 部署 id | **还没有** —— 没推就没有部署 |
| state | **不适用** |

> **可以说的:** 树是干净的;`GATE_EXIT=0`;`BUILD_OWN_EXIT=0`;**本刀没有迁移,
> 所以【没有破窗】** —— 生产此刻跑的是旧代码配旧库,两者一致,不存在
> 「旧代码 + 新库」那个窗口。这一条与 IOD-2 那次的处境**正好相反**,值得说明白:
> 那次危险来自库先走了一步;这一刀一条 SQL 都没有,所以推迟推送**什么都不破坏**,
> 它只是让这些改动还没到人手上。
>
> **不可以说的:** 这一刀"上线了"。**没有。**
...
(本仓库记过这件事:同一条命令在会话里被拦、由 Tim 重新发出就过 ——
`ANON-0` / `FIX-2b` 都是这样。**我没有自作主张去试它的变体。**)

推完之后还欠三件,按 AGENTS.md 的顺序:

1. `scripts/wait-for-deploy.sh` 等 `state=success`(它在本地补全 SHA,不会拿缩写去
   问出一个空集);
2. **另外独立复核一次** —— `gh api repos/<owner>/<repo>/deployments/<id>/statuses`,
   确认判据检查的确实是 `state=success`,不是"有没有一条记录";
3. 记下部署 id 与 CST 的 success 时刻。
   ☞ 而**记录不来不等于部署没发生**(GRN-1b):先看 Vercel 面板。
