...
| 树 | 干净(`git status --porcelain` 0 行) |
| `messages/` diff | **0 行** —— 没有新增、改动或删除任何 i18n key |

### 部署

| | |
|---|---|
| deployment id | **6346862808** |
| sha | `ad39bb36053d81ffdf406da228be4da6098f0170` —— **与本地 HEAD 逐字相同** |
| state | **success** |
| success 时刻 | **2026-09-09 09:40:14Z**(17:40:14 CST) |

**两个问题分开问过,照 §1.F:**
1. **120s 时问「这个 sha 有部署记录吗」** —— **没有**(空数组)。
   **这正是那条常数说的那一幕:窗口里最新的成功部署还是【上一刀的 sha】。**
2. **继续等到记录出现(+25s),再问「最新那次成功部署的 sha 是不是 ad39bb3」** ——
   `id=6346862808 · sha=ad39bb36… · state=success`。**等到 sha 对上才收工。**

### ★ 窗口长度 —— 第四个数据点

```
推送 09:37:21Z  →  success 09:40:14Z  =  173s
```
...
☞ **诚实地更新一下那个常数的描述:** 委托书 §1.F 写的是「165 / 155 / 150,方差不到 10%」。
加上第四个点之后是 **150–173s,方差约 15%**。
**它没有推翻那条常数** —— 「从 120 秒开始问、等 sha 对上」照样是对的做法,
而且这一刀正是在 120s 那一问上拿到空数组的。
**要改的只有那句「不到 10%」:今天该写成【两分半到三分钟,方差 15%】。**
☞ 与本仓库那条「写下来的成本必须是量过的成本」一致:第四个点量出来比前三个都长,
就照直记,不去平均它。

### ★ 本刀会不会让下一份委托书的哈希又落后一格

**会 —— 而且原因还是那一个:这一节(§12)是部署【之后】才写得出来的,
所以它必然是一条【docs-only 的后续提交】,而下一份委托书多半写在它落地之前。**
☞ **这是第五次了。它不是失误,是这个流程的形状。**
