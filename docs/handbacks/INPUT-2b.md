...
| 冒烟整跑 **871 秒**(ROUND 2 实测) | 否(只作估价) | 见 §R3-7.3 | 本轮实跑另报 |
| `--mode=drift` 整跑 **1669 秒**(ROUND 2 实测) | 否(只作估价) | 见 §R3-9 | 本轮实跑另报 |
...
| 单条补量(`/purchasing` + 陪跑) | ★ 见 §R3-9 | ★ ROUND 3 实测 |
| 并入 + 两条 compare + 行高比对器 + 宽度报告 | ★ 见 §R3-9 | ★ ROUND 3 实测 |
| R2 / select 判据探针(58 × 2 视口 + lanes ×2 + 截图) | ★ 见 §R3-9 | ★ ROUND 3 实测。ROUND 2:634 |
| 版式修复那一圈(改前量 + 改 + 量 + (ii) 实验 + 撤 + 再量) | ★ 见 §R3-9 | ★ ROUND 3 实测 —— **四次 2 路由 × 2 视口的探针,每次 ~19 秒** |
| ★ **冒烟整跑** | ★ 见 §R3-7.3 | ★ ROUND 3 实测。ROUND 2:871 |
| `npm run build` | ★ 见 §R3-7.2 | ★ ROUND 3 实测(ROUND 2 **没跑过**) |
| `db/gate.py` | ★ 见 §R3-7.4 | ★ ROUND 3 实测(ROUND 2 **没跑过**) |
| 部署登记等待 | ★ 见 §R3-7.7 | ★ ROUND 3 实测(ROUND 2 **没跑过**) |
...
> | | |
> |---|---|
> | ★ **流程部分** | ★ 见 §R3-9 那张表的合计 —— ★ **而估价要按【冷 `.next`】的 drift(1669 秒)算,不是本轮那个 727** |
