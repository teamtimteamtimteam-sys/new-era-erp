...
| **收工后 HEAD** | `9f64bf1cdcad5ac241eefb2b41e3157b26c2621f`(TABLE-STYLE-1;本报告在其后一个纯文档提交里)|
| 收工后树 | **零行(树干净)** —— 见 §13 |
...
☞ **而这笔账是【对得上的,不是差不多】:** 在 `ba360c9` 上按「块内 / 块外」分别数
`<table>`/`<th>`/`<td>` 上的钉字号 —— **块内 186 处,块外 10 处,合计 196**。

* **块内的 186,与 TABLE-CONVERT-0 §4 那一栏加总出来的 186【逐个相等】。**
* **块外的 10 处,逐行点名如下,而它们与本刀在这三段里剥掉的 5 / 2 / 3【逐段相等】:**

```
app/finance/balance-sheet/page.tsx:120 :129 :141 :143 :157     ← 5 处(sectionBlock)
app/finance/cashflow/page.tsx:102 :106                          ← 2 处(<Row>)
app/finance/pnl/page.tsx:116 :125 :139                          ← 3 处(sectionBlock)
```

☞ **所以普查那个数没有错,是它的【量法】按 `<table>` 块统计 —— 而这三段行不住在块里。**
一个"差不多对得上"的解释在这个仓库里不算解释;这一笔是逐处点名对上的。
...
### 13.1 `db/gate.py` —— **它自己的退出码 0**

| | |
|---|---|
| **`GATE_OWN_EXIT`** | **0** |
| **wall-clock** | **535 秒**(委托书给的范围 180–700s,**落在里面,不需要放宽**) |
| 起 / 止(UTC) | `2026-09-09 16:12:41` → `16:21:36` |
...
★ 闸自己印的 wall-clock 是 **425s**(它只算三个判词那一段);**535s 是整支脚本**
(含 `check_grants.py` 那一段)。**两个数都记着,因为它们量的不是同一段。**

### 13.2 `npm run build` —— **它自己的退出码 0**
...
**元检查(`check-instrument-selfproof`)那一行,原文照抄:**
...
== 格子钉死的字号(<table>/<th>/<td> 标签 + 列描述符的 className)(app/,不含组件库自己)==
   基线:124 个文件在册。本次扫到 418 处。
   分账:标签上 125 处(<table>/<th>/<td>) · 列描述符里 293 处(className: '…')
✓ 没有新增的格子钉死的字号(…)。
```

★ **一处与本刀无关的读数,记下来免得下一个人当成本刀干的:**
构建里 `check-cjk-rendered` 报「✓ 少了 1 条(基线可以变短):`app/components/pdf/Wordmark.tsx :: 渐变`」。
**那个文件本刀一个字节没动**(不在 `git diff` 里,最后一次改动是 `ad624e3` COD-1)。
它是一条**advisory**(✓ 不是 ✗),不让构建变红;**本刀没有去收紧那份基线** ——
收紧一份不是自己造成的基线,正是这一族记过的越界。

### 13.3 推送 · 部署

| | |
|---|---|
| 提交 | **`9f64bf1cdcad5ac241eefb2b41e3157b26c2621f`** |
| `date -u`(推送前) | `Wed 9 Sep 2026 16:25:51 UTC` |
| `date -u`(推送后) | `Wed 9 Sep 2026 16:25:55 UTC` |
| **推送窗口** | **4 秒** |
| **核验方式** | ★ **`git fetch origin` 之后比哈希,不是读 `git push` 的输出** |
| `git rev-parse HEAD` | `9f64bf1cdcad5ac241eefb2b41e3157b26c2621f` |
| `git rev-parse origin/main` | `9f64bf1cdcad5ac241eefb2b41e3157b26c2621f` —— **相等** |
| 部署 id | **6354495764**(Production) |
| **部署 state** | **`success`** |
| 部署 created / success 时刻 | `2026-09-09T16:28:56Z`(= `2026-09-10 00:28:56 CST`) |
| 环境 URL | `https://new-era-g28dbseun-tim-s-projects7.vercel.app` |
| 等到 success 用时 | **145 秒**(`wait-for-deploy.sh` 自己的退出码 **0**) |

**★ 两个问题分开问过,而且都用【完整 40 位】SHA(缩写会让 GitHub 静默返回空数组):**

* **问题一(有没有这条记录):**
  `deployments?sha=9f64bf1c…` → `id=6354495764 sha=9f64bf1cdcad5ac241eefb2b41e3157b26c2621f env=Production`
* **问题二(它自己的 state 是什么):**
  `deployments/6354495764/statuses` → **`state=success`**

**三个 SHA 并排,全部相等:**

```
local HEAD   : 9f64bf1cdcad5ac241eefb2b41e3157b26c2621f
origin/main  : 9f64bf1cdcad5ac241eefb2b41e3157b26c2621f
部署记录 sha : 9f64bf1cdcad5ac241eefb2b41e3157b26c2621f
```

### 13.4 收工时的树

**代码那一刀(`9f64bf1`)推完之后,实测:**

```
$ git status --porcelain
(零行)
$ git fetch origin && git rev-parse HEAD && git rev-parse origin/main
9f64bf1cdcad5ac241eefb2b41e3157b26c2621f
9f64bf1cdcad5ac241eefb2b41e3157b26c2621f     ← 相等
```

**★ 一句必须说清的话:本报告【自己】那个提交的哈希,写不进它自己。**
先写一个数、再提交,提交出来的哈希就不是那个数了(本刀试过一次,那一版
记的 `656751f…` 在 `--amend` 之后【已经不存在】—— 一个查不到的哈希比没有更坏)。
☞ 所以这一栏记的是**代码提交**的收工状态,那也正是部署与闸真正对着的那个树;
报告提交是它之后的**纯文档跟进**(与本刀开工时面对的 `ba360c9` 是同一种形状),
它的哈希由收工那条 `git log --oneline -2` 给出,**不由本文声称**。

**树干净 · HEAD == origin/main · 部署 state=success。**
