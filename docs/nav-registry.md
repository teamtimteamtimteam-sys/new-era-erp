...
> ★★【UI-1c(2026-09-05)更正这三个数,而更正它的正是上面这条判据】★★
> **今天是:删除 10 个 · 保留 1 个 · 【回来 1 个】(设置)。**
>
> 这不是推翻本节,是本节的判据遇上了改变了的事实。UI-1a 把 `settings` 移出
> `BAR_MODULE_IDS`,于是设置那七个目标**不再被顶栏菜单 offer**:实测桌面上
> `roles` / `reference` / `approvals` / `deleted` / `import` **五张一次点击都到不了**
> (受影响的是 `admin` 与 `cco` 两个人)。按本节那条判据,
> 「每一个目标都已经被 offer」不再成立,那一行于是重新成立。
>
> 回来的是 `app/settings/SettingsSubnav.tsx`(挂在**一个** `app/settings/layout.tsx`
> 上,不是七处手插),而它与被删的那一份**在两点上不同**:目标**从注册表派生**
> (`getModuleAccess()`,不是写死的数组),进不去的**画成「· 受限」**而不是省略。
>
> **真正的修法是它旁边那一件**:头像下拉的「设置」从一条跳转变成七行子菜单,
> 与 `ToolsMenu` 对称 —— 有了它,这条 strip 是【定位】而不是【唯一入口】。
> 完整推理见 `docs/information-architecture.md` §23.1–§23.2。
>
> ★ 顺带一条给下一个读委托书的人:UI-1c 的委托书把
> `app/purchasing/Subnav.tsx` 写成「树里已经存在的样板」—— **它是上表里被删的
> 十个之一。** 一句曾经为真的话,隔两刀之后被当成现在仍然为真的来读。 ★
