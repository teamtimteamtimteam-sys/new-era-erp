...
#### SUP-TYPE-1a(2026-08-18):不欠冒烟,也不欠 --reach —— 而这是查过的,不是省掉的

SUP-TYPE-1a 是**纯数据库那一半**:一个列、一个触发器、两支视图谓词。
它**没有动任何页面、没有加路由、没有改导航或 `lib/modules.ts`、没有改任何权限守卫**。
按 AGENTS.md 写下的触发条件,冒烟与 `--reach` 这一刀都不满足。

**但有一件事要留给 SUP-TYPE-1b(选择器那一半)**:
`suppliers.supplies_goods` 现在会让收货【按名拒绝】
(`RECEIPT_AGAINST_NON_GOODS_VENDOR`),而**收货表单目前仍然把所有在册供应商
列进下拉**(实测:`app/inbound/new/page.tsx:32`、`app/inbound/receive/page.tsx:22`
只过滤 `deleted_at`)。也就是说**页面此刻会提供一个服务端保证会拒的选项** ——
正是 AGENTS.md「页面不许摆出一个服务端保证会拒的按钮」那一条。
这是 1b 的活,记在这里是因为**在 1b 落地之前,那条路径是可点、且必然报错的**;
