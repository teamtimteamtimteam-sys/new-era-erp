...
## 预留区的两个屏幕缺口(SO-3b fu5 顺带查出来,没有在同一刀里修)

两条都在 `app/sales/orders/[id]/ReservationSection.tsx`,都是 SO-3b 发货落地之后
留下的,**都会在重建之后依然存在**,所以记在这里而不是"到期消失"那一份里。

**① 已发的预留仍然列成"可释放",而那个按钮必然失败。** 那一节的活预留判据只有
`released_at === null` 一个条件,不看 `consumed_at`。发过货之后,那条预留还带着
一个「释放」按钮,而 `release_reservation` 必拒(`SO_RESERVATION_ALREADY_SHIPPED`)。
AGENTS.md 那条"永远不要给一个服务端注定拒绝的动作画一个提交控件"说的就是它。

**这一条没有让屏幕上的【数字】出错,而那正是它没有被顺手改掉的理由**:那一节显示的
"已预留 X / 行数量"是对【所有未释放】的预留求和 = 活预留 + 已发,而这恰好等于
fu5 之后服务端的判据 `line_spoken_for()`。所以页面与服务端此刻是**同口径**的;
动它要同时决定"已发的那部分在屏幕上叫什么",而那是一句要新的文案键的话。

**② `partially_shipped` 的单子看不到预留控件。** 那一节按 `status === 'confirmed'`
显示控件,而 `reserve_stock` 与 `create_order_invoice` 自 SO-3b fu1 起【都收】
`partially_shipped`(只认 confirmed 会让任何多行订单在第一次发货之后走不下去 ——
fixture 68 第一次跑就撞上了)。数据库放开了,屏幕没有跟着放开。

**后果有两面,两面都要说**:一面是**多行订单在界面上走不完**(第一次发货之后,
剩下的行再也预留不了,只能走 RPC);另一面是它**恰好挡住了 fu5 修掉的那个缺陷的
界面路径** —— 双重发货此前只能由直连 RPC/API 走到,点不出来。修 ② 的时候要记住
第二面已经由 fu5 在数据库这一侧关掉了,所以放开屏幕不再重新打开那条路。
