...
#### LME-1a(2026-08-18):出处这一列现在是真的 —— 而屏幕还不知道

LME-1a 是**纯数据库那一半**:`metal_prices.source` 从"自由文本 + 默认 manual"
变成四取一的封闭集合(**且没有默认值**),新增 `source_reference` 与
`quote_delayed` 两列,`upsert_metal_prices` 收下三个新参数并按名拒绝漏填。

**留给 LME-1b(录入屏那一半)的:**
1. `/metal-prices` 的录入表单**还没有出处这个控件** —— 而
   `upsert_metal_prices` 现在【必填】它。也就是说**那张表单此刻提交会被拒**
   (`QUOTE_SOURCE_REQUIRED`),错误是具名的、不是机器码,但它坏着。
   **这是本刀有意接受的一个破窗**:两条路都不好 —— 给函数一个默认值就等于
   把"这一列是空话"原样保留下来;而在同一刀里改屏幕又超出了"DB 半"的范围。
   选了前者不做、后者不做,把它写在这里,并且【1b 必须紧接着做】。
2. 三个新参数(`p_source` / `p_source_reference` / `p_quote_delayed`)需要
   对应的三个控件:出处四选一(不含 unknown)、凭据自由文本、当天/延迟三态。
3. `published_index` 必须同时选指数 —— 表单要把这条配对规则表达出来,
   而不是让人提交之后才撞上 `QUOTE_SOURCE_INDEX_REQUIRED`。

**冒烟欠着(第十刀)**:`select 1` 实测 2.6–3.5 秒。
`--reach` 不欠:没动导航、没加路由、没改 `lib/modules.ts` 或权限守卫。
