...
下一次网络正常时补跑,并且要**按名**回答两个问题:
1. 三个新页面在冒烟里是 `ok` 还是 `SKIP`?(`/processing/orders/[id]` 目前登记在
   `EXPECTED_SKIPS` 里 —— 线上零张工单;手走开出第一张的那天它会响。)
2. `--reach` 里,三个新页面【进得去吗】?子导航加了一项,而
   `/processing/orders/[id]` 是动态路由,走查对动态路由不做"打得开却走不到"的断言
   (SAL-B6 的教训),所以那一条仍要由人确认。
补跑那次提交把本条删掉。
