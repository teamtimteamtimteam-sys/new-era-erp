...
| 判词 | 数量 | 处置 |
|---|---|---|
| FAILS OPEN | 5 | **已全部修掉**(见下) |
| SHOWS FALSE DATA | 64 | 未修:只误导、不放行、不毁数据。按页面重要性排期 |
| HARMLESS | 80 | **结案,不再复查** |
...
- `app/finance/invoices/page.tsx`(5 处)—— 整个文件没有任何 error 检查,失败即
  干净的 200「没有发票」。
- `app/finance/close/page.tsx:90` —— 期末关账预览显示 0 分录 + 绿色「已平」,
  就在关账按钮正上方。
- `app/finance/revaluation/page.tsx:20,36` —— 预览空而按钮仍可点;少算前期重估行
  会让每个账面金额与调整数偏大(**这一处会过账,不只是显示**)。
