...
出处列没有任何屏幕在读。** 全库唯一读 `processing_metal_recovery` 的是
`/processing/[id]`,它的 select 里没有 `input_source` / `output_source` ——
批次页上每一行含量的出处看得见(PROC-1b 的出处列),但加工单回收率表里
"这个百分比除的是哪一种数"仍然只存在于视图里。一列没人读的视图列,与一个
没有屏幕的机制,是同一个缺口矮一层。补上它是加工单页的一次显示改动,
不牵动任何机制 —— 记在这里,别在下次有人问"出处怎么看不到"时才被重新发现。
