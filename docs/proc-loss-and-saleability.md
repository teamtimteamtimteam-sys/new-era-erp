...
## 【分类的依据仍然悬着:等 NEA 的报表格式】(PROC-WIRE-1A 补记,2026-08-31)

> **这四个类别是【保守版】,不是 NEA 的类别。** Tim 手上**没有** NEA 实际的
> 物料平衡报表格式,所以本刀选的是"物理上分得开、而且 `metal_fate` 答得出来"
> 这条自守的判据,**不是**"监管要求的那几格"。
>
> **后果要说清楚:那一天到来时,大概率不是"再加几行",而是【重新归类】** ——
> 一条 NEA 的格子可能横跨我们两个类别,也可能把 `dust_spill` 劈成两半。
> `processing_run_losses` 上已经记下的那些行,**要按新格子重新映射**,
> 而那件事今天没有映射表可写。
>
> **但加类别这件事本身是【数据】,不是代码** —— `loss_categories` 是
> RUNTIME CONFIG,加一行就够,这正是把它做成字典换来的东西。
> **真正要小心的是把【已经分好的历史】搬过去,那才是那天的成本。**
>
> * **触发条件:** 拿到 NEA 的物料平衡报表格式那一天。
> * **在那之前:** 不要因为"看起来像"就去猜某一格对应哪一类 ——
>   猜错的映射与对的映射长得一模一样,而它算得出回收率。
