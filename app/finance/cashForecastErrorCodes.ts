...
// 不在集合内的交给共用兜底 lib/machine-text.ts —— 尤其 FX_RATE_MISSING:它是 THE FX RULE 那一族的码,
// 措辞归它自己,这里不去复述。
...
export async function localizeCashForecastError(message: string): Promise<string> {
    ...
    if (!match) return await fallbackForRawError(raw, 'localizeCashForecastError@app/finance/cashForecastErrorCodes.ts')
    const t = await getTranslations()
    if (match[1] === 'PERMISSION_DENIED') return t('permissions.errDenied')
    if (!CASH_FORECAST_ERROR_CODES.has(match[1])) return await fallbackForRawError(raw, 'localizeCashForecastError@app/finance/cashForecastErrorCodes.ts')
