...
// ★ PAY-REQ-1(Tim 2026-09-23):恒为 unpaid —— 借费用科目/贷 2000 应付(成为 AP 单据)。
//   「已付」那一支退休了:钱离开之前要先批,付款走付款申请(record_expense 对 'paid' 按名拒)。
...
export async function createExpense(
    ...
    const { data, error } = await supabase.rpc('record_expense', {
        ...
        p_supplier_id: counterparty.kind === 'supplier' ? counterparty.id : undefined,
        p_employee_id: counterparty.kind === 'employee' ? counterparty.id : undefined,
