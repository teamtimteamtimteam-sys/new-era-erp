...
//
// ★★ TABLE-CONVERT-2(2026-09-10):**"按兵不动"到此为止 —— 这张表转了。**
//   理由与资产台账同一条(见那个文件的抬头):ReopenForm 是一个自带状态的
//   'use client' 子组件,不是靠并列数组提交的行内输入,所以它不需要
//   "行级编辑态"契约,列描述符把它画出来就够了。
//   ☞ 表本身搬到了 ./CloseHistoryTable.tsx;本页是 server component。
...
import CloseHistoryTable, { type CloseHistoryRow } from './CloseHistoryTable'
...
export default async function ClosePage({
    ...
    // ★ TABLE-CONVERT-2:关账历史那张表搬进了 CloseHistoryTable('use client')。
                    ...
                    <>
                        <div className="bg-gray-50 rounded p-4 flex flex-wrap gap-x-8 gap-y-2 text-sm items-center">
                            ...
                            <div>
                                ...
                                <span className="font-medium">{formatAmount(preview.debits, baseCurrency)}</span>
                            </div>
                            <div>
                                <span className="text-[color:var(--brand-muted-text)] mr-1">{t('finance.colCredits')}:</span>
                                <span className="font-medium">{formatAmount(preview.credits, baseCurrency)}</span>
                            </div>
                            <span
                                className={
                ...
                <div className="bg-gray-50 rounded p-4 mb-4 space-y-3 text-sm">
                    <div className="flex flex-wrap gap-x-8 gap-y-1">
                        <div>
                            ...
                            <span>{yp.expected_year_end}</span>
                        </div>
                        <div>
                            <span className="text-[color:var(--brand-muted-text)] mr-1">{t('finance.yearClose.netResult')}:</span>
                            <span className="font-medium">{formatAmount(yp.net_result, baseCurrency)}</span>
                        </div>
                    </div>
