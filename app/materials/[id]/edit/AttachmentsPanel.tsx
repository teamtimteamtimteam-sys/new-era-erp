...
export default function AttachmentsPanel({
    ...
    return (
        <section className="mt-8 pt-8 border-t">
            ...
            {rows.length === 0 ? (
                <p className="text-sm text-gray-500 mb-6">{t('materials.attachments.empty')}</p>
            ) : (
                <table className="w-full border-collapse border border-gray-300 mb-6">
                    <thead className="bg-gray-100">
                        <tr>
                            <th className="border border-gray-300 px-2 sm:px-4 py-2 text-left">{t('materials.attachments.colName')}</th>
                            <th className="border border-gray-300 px-2 sm:px-4 py-2 text-left">{t('materials.attachments.colCategory')}</th>
                            <th className="hidden sm:table-cell border border-gray-300 px-4 py-2 text-left">{t('materials.attachments.colType')}</th>
                            <th className="hidden sm:table-cell border border-gray-300 px-4 py-2 text-left">{t('materials.attachments.colSize')}</th>
                            <th className="hidden sm:table-cell border border-gray-300 px-4 py-2 text-left">{t('materials.attachments.colCreated')}</th>
                            {/* ★ TABLE-PHONE-1:「操作」这一列在手机上【照画】,不叠进去 ——
                                一个在手机上够不着的下载/删除,与没有这两个动作是同一回事
                                (DBLOCK-1 那条道理用在版式上)。 */}
                            <th className="border border-gray-300 px-2 sm:px-4 py-2 text-left">{t('materials.attachments.colActions')}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {rows.map((row) => (
                            <tr key={row.id}>
                                <td className="border border-gray-300 px-2 sm:px-4 py-2 break-all">
                                    ...
                                    <div className="sm:hidden mt-1 space-y-0.5 text-xs text-gray-600">
                                        <div>
                                            <span className="text-gray-500">{t('materials.attachments.colType')}: </span>
                                            {row.file_type ?? '—'}
                                        </div>
                                        <div>
                                            <span className="text-gray-500">{t('materials.attachments.colSize')}: </span>
                                            {formatBytes(row.file_size)}
                                        </div>
                                        <div>
                                            <span className="text-gray-500">{t('materials.attachments.colCreated')}: </span>
