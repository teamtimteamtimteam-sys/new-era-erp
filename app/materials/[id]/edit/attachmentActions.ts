...
// 物料附件的服务端动作,端口自 customers 的 attachmentActions.ts。
// 上传流程:浏览器端先把文件直传 Storage(私有桶 material-attachments),
// 再调用 recordAttachment 把元数据写入 material_attachments 表。
...
export async function recordAttachment(input: RecordAttachmentInput) {
    ...
    if (!storagePath || !fileName) {
        return { error: t('materials.attachments.recordError', { message: 'missing path or name' }) }
    }
    ...
    if (!isAllowedAttachmentType(input.fileType)) {
        ...
        return { error: t('materials.attachments.errType') }
    }
    ...
    const { error } = await supabase.from('material_attachments').insert({
    ...
    if (error) {
        return { error: t('materials.attachments.recordError', { message: error.message }) }
    }

    revalidatePath(`/materials/${input.materialId}/edit`)
    return { success: true }
}
...
export async function getAttachmentDownloadUrl(storagePath: string, fileName?: string) {
    ...
    if (error || !data?.signedUrl) {
        return { error: t('materials.attachments.downloadError', { message: error?.message ?? 'no url' }) }
    }
...
export async function deleteAttachment(id: string, materialId: string) {
    const t = await getTranslations()
    const supabase = await createClient()
    const {
        data: { user },
    ...
    if (error) {
        return { error: t('materials.attachments.deleteError', { message: error.message }) }
    }
