...
export type Column<T> = {
    ...
    /**
     * ════════════════════════════════════════════════════════════════════════
     * ★★★ POLISH-1(2026-09-12,Tim 的裁定 R7)· **单值列** —— 表头可以折行 ★★★
     * ════════════════════════════════════════════════════════════════════════
     * 【它声明的是什么】**这一格里装的是一个值,不是一段文字** —— 一个数、
     *   一个短的固定词(状态、代码、日期)。
     *
     * 【它做什么 —— 只有一件事】这一列的表头**不再套 `sm:whitespace-nowrap`**。
     *   ☞ 于是一个长表头不再逼着整列去凑它的宽度。
     *
     * 【为什么这是【唯一】做的那一件事 —— 两条都是量出来的】
     *   ★ 规矩(spec §4.3a)说的是「单值列不得宽过同表最窄的文字列」。
     *     那是一条**不等式**,而在 `table-layout: auto` 下它**强制不了**:
     *     表比内容宽的时候,富余是按比例分下去的,压内容改不了渲染宽度。
     *     ☞ 实测:18 张表里 **12 张**对任何内容侧的改动【纹丝不动】,
     *       因为它们的 `tableW` 恰好等于外壳宽(1376)—— 那是富余,不是需求。
     *   ★★ 而**硬封顶**那个变体(给表头一个 `max-width` = 最窄文字列的宽度)
     *     实测**踩了停止条件 (c)**:`/finance/claims` 在 390px 上
     *     **滚动范围 0 → 20** —— 因为那个「最窄文字列」在手机上已经被
     *     DataTable 丢掉了(宽度 0),于是封顶值是 0。
     *     ☞ 它还要求每一列挑一个**手选的像素值**,而本仓库的标准里
     *       每一个数都必须是**推出来的**,不是挑出来的。
     *   ☞ 所以落地的是「允许折行」这一半:**没有任何一个挑出来的数字,
     *     而且两个视口 18 张表【一张都没有开始横滚、一张都没有多出范围】。**
     *
     * 【它在哪里有用,说白】只在**宽度真的稀缺**的时候 —— 也就是表比外壳宽、
     *   富余为负的那些表。实测有效的是 3 张(见 spec §4.3a 的名单),
     *   最重的一张 `/finance/packs` 的滚动范围 **365 → 122**。
     *   **表有富余的时候,「这一列比那一列宽」不是缺陷,是富余。**
     *
     * ⚠ **不要把 `sm:whitespace-nowrap` 整条去掉。** 它有一条量出来的理由:
     *   1280px 下「库存状态」会被折成每行一个字(见下面那段注释)。
     *   这个 prop 是**逐列的选择退出**,不是把那条规则删掉。
     */
    singleValue?: boolean
    className?: string
}
                                ...
                                return (
                                    <th
                                        ...
                                        className={cn(
                                            ...
                                            // ★ POLISH-1 R7:声明了 `singleValue` 的列【不套这一条】。
                                            //   逐列选择退出,不是把这条规则删掉 —— 理由见 Column.singleValue。
                                            !c.singleValue && 'sm:whitespace-nowrap',
