...
//
// ════════════════════════════════════════════════════════════════════════════
// ★★【SEARCH-3(2026-09-13):上面那句「一颗 <button>」到此为止 —— 它是输入框了】★★
// ════════════════════════════════════════════════════════════════════════════
//   Tim 的 U1:**你在【你点的那一格】里打字,结果贴着它往下展开。**
//   ☞ 于是这一格是一个包着 `<input>` 的 `<label>`(点标签 = 聚焦控件,纯 HTML),
//     而结果是一个贴着它下边缘、与它同宽的下拉。**尺寸仍然一个像素都没动**:
//     `.box` / `.glyph` / `.prompt` 三个类名原样留着,实测 390px 上
//     `358x49.02`,与 SEARCH-1 / SEARCH-2b 的读数逐字相同。
//   ☞ Tim 的 U2:那一格的光标是**文字光标**(`.box { cursor: text }`)——
//     它是一个输入框,不是一个链接。
//
// ★★【U3:那一行问候语此前【压在搜索面上面】,而修法在 `.shell` 的 z-index 上】★★
//   机制,一句话:`.shell` 是 `position: relative; z-index: 1`,**所以它是一个
//   层叠上下文**;面板写在它里面,面板那个 `z-[150]` 因此被关在里面,对外只值
//   `z-index: 1`。而 `.greeting` 也是 `z-index: 1`,**并且排在 `.shell` 后面** ——
//   同级同值,后来者在上。☞ 于是问候语画在面板上面。
//   ★ 实测(probe-nav-geometry 的 N16,改前):重叠处取 9 个点,
//     **9 个点最上面的都是那一行问候语,0 个是搜索面。**
//   ☞ 修法是把 `.shell` 抬到 `z-index: 2` —— 一句 CSS,而它由 N16 兜底。
//
    ...
    return (
        <div className={styles.stage}>
            ...
            <SearchEntry
                ...
                // ★ SEARCH-3:`.prompt` 今天挂在【那个输入框】上,不再挂在一个
                //   `<span>` 上 —— 而它的两条承重规则(截断、字号)一个字都没改,
                //   高度因此也没改(实测 390px 上 358x49.02,与 SEARCH-2b 逐字相同)。
                inputClassName={styles.prompt}
            />
