...
const SALES_ORDER_ERROR_CODES = new Set([
    ...
    // APR-5b:发货要 CFO 先放行(Q3)、客户冻结时不发(Q6)、不超过 开票 − 取消 − 已发(Q8)
    ...
    'SHIPPING_RELEASE_ORDER_NOT_SHIPPABLE',
    'SHIPPING_RELEASE_NO_LINES',
    'SHIPPING_RELEASE_LINE_NOT_INVOICED',
    'SHIPPING_RELEASE_LINE_ALREADY_RELEASED',
    'SHIPPING_RELEASE_NO_OTHER_DECIDER',
    'SHIPPING_RELEASE_NOT_FOUND',
    'SHIPPING_RELEASE_NOT_SUBMITTED',
    'SHIPPING_RELEASE_NOT_OPEN',
    'SHIPPING_RELEASE_REJECT_REASON_REQUIRED',
    // 审批引擎的三句(决定放行时会撞上):审批没开、自己提的、不是二级
...
export async function localizeSalesOrderError(message: string): Promise<string> {
    ...
    if (!match || !SALES_ORDER_ERROR_CODES.has(match[1])) return await fallbackForRawError(raw, 'localizeSalesOrderError@app/sales/orders/salesOrderErrorCodes.ts')
