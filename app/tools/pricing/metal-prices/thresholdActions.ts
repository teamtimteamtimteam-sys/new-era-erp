...
//
// 闸门在数据库:pricing_settings 的 UPDATE 策略要 action.metal_prices
// (改阈值与录行情是同一件工作,同一个码;ROLE-1 Batch 2b 之前是 module.pricing.edit)。
// 这里不复述那条判断。
...
export async function updateAnomalyThreshold(
    ...
    // 衣服:0 与"你没有权限"在屏幕上一模一样。页面对无权者把这个钮画成按不动的
    // (ROLE-1 Batch 2b 起,PermissionGate),但一次伪造的提交不该拿到一句谎话。
